...
int GMRFLib_QM(int thread_id, gsl_matrix *result, gsl_matrix *x, GMRFLib_graph_tp *graph, GMRFLib_Qfunc_tp *Qfunc, void *Qfunc_arg, int *nt_opt)
{
	...
	for (int k = 0; k < nt; k++) {
		indx[k] = Calloc(len, int);
		values[k] = Malloc(len, double);
		dot_values[k] = Malloc(ncol, double);
	}
...
{
	double *y = NULL;
	Malloc_init(graph->n, 1);
	y = Malloc_get(graph->n);
