...
double GMRFLib_ddot_optimized(int n, double *__restrict x, double *__restrict y)
{
	...
	if (__builtin_expect(n <= 4, 0)) {
		...
		double r = 0.0;
		switch(n) {
		case 4: r += x[3] * y[3]; __attribute__((fallthrough));
		case 3: r += x[2] * y[2]; __attribute__((fallthrough));
		case 2: r += x[1] * y[1]; __attribute__((fallthrough));
		case 1: r += x[0] * y[0];
		}
		return r;
...
{
	if (n <= 0) return 0.0;
	if (n <= GMRFLib_DOT_GROUP_NLIM) return GMRFLib_ddot_idx_optimized(n, v, a, idx);
