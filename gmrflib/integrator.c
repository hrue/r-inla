...
/*
  CHANGE/HRue: this is now in integrator.h:
...
/*
  CHANGE/HRue: this now in integrator.h:
  
  int adapt_integrate(integrand f, void *fdata, unsigned dim, const double *xmin, const double *xmax, unsigned maxEval, double
...
static unsigned evalR0_0fs4d(integrand f, void *fdata, unsigned dim, double *p, const double *c, double *sum0_, const double *r1,
			     double *sum1_, const double *r2, double *sum2_)
{
	double maxdiff = 0;
...
static unsigned rule75genzmalik_evalError(rule * r_, integrand f, void *fdata, const hypercube * h, esterr * ee)
{
	...
	/*
	 * Evaluate function in the center, in f(lambda2,0,...,0) and f(lambda3=lambda4, 0,...,0).  Estimate dimension with largest error 
	 */
...
static rule *make_rule75genzmalik(unsigned dim)
{
	...
	/*
	 * Because of the use of a bit-field in evalR_Rfs, we are limited to be < 32 dimensions (or however many bits are in unsigned). This is 
	 * not a practical limitation...long before you reach 32 dimensions, the Genz-Malik cubature becomes excruciatingly slow and is
	 * superseded by other methods (e.g. Monte-Carlo). 
	 */
...
static unsigned rule15gauss_evalError(rule * r, integrand f, void *fdata, const hypercube * h, esterr * ee)
{
	...
	const double xgk[8] = {				       /* abscissae of the 15-point kronrod rule */
		...
		0.000000000000000000000000000000000
		    /*
		     * xgk[1], xgk[3], ... abscissae of the 7-point gauss rule. xgk[0], xgk[2], ... to optimally extend the 7-point gauss rule 
		     */
	};
			       ...
			       esterr * ee)
{
	...
	/*
	 * another possibility is to specify some non-adaptive subdivisions: if (initialRegions != 1) partition(h, initialRegions,
	 * EQUIDISTANT, &regions, f,fdata, r); 
	 */
		    ...
		    double *err)
{
	...
	ee.val = ee.err = 0.0;				       /* CHANGE/HRue: avoid compiler warning */
...
int main(int argc, char **argv)
{
	...
	printf("Usage: %s <dimensions> <tolerance> <which_integrand> <maxeval>\n"
	       "   defaults: <dimensions>=2, <tolerance>=1e-2\n"
	       "             <which_integrand>=0 (should be 0..6)\n" "             <maxeval>=0 (unlimited)\n", argv[0]);
