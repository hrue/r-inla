...
#pragma omp parallel for reduction(+: tmp2) num_threads(GMRFLib_openmp->max_threads_inner)
			for (int iii = 0; iii < nidx; iii++) {
				...
				loglFunc(thread_id, NULL, &ll, &x[ii], 1, ii, x, NULL, loglFunc_arg);
				tmp2 += d[ii] * ll;
			}
			Calloc_free();
		} else {
			...
			for (int ii = 0; ii < n; ii++) {
				if (d[ii]) {
					loglFunc(thread_id, NULL, &logl, &x[ii], 1, ii, x, NULL, loglFunc_arg);
					tmp2 += d[ii] * logl;
				}
			}
		}
	...
	if (!logdens) {
		...
		if (d) {
			...
			for (ii = 0; ii < ns; ii++) {
				i = node_map[ii];
				if (d[i]) {
					loglFunc(thread_id, NULL, &logl, &x[i], 1, i, x, NULL, loglFunc_arg);
					tmp += d[i] * logl;
				}
			}
			val += tmp;
	...
	if (cpo_density) {						\
		if (d[idx]) {						\
			...
			{						\
				...
				loglFunc(thread_id, NULL, logcor, x_user, np, idx, fixed_mode, NULL, loglFunc_arg); \
				for(_i=0; _i < np; _i++) {		\
					logcor[_i] *= d[idx];		\
				}					\
				if (_debug && np) {			\
	...
	if (d[ii] || ai_par->cpo_manual) {				\
		...
		if (dic) {						\
			deviance_theta[ii][dens_count] = GMRFLib_ai_dic_integrate(thread_id, ii, dens[ii][dens_count], \
										  (ai_par->cpo_manual ? 1.0 : d[ii]), loglFunc, loglFunc_arg, xx_mode, NULL, NULL); \
		}							\
	}
	...
	if (d[ii]) {							\
		if (po) {						\
			GMRFLib_ai_po_integrate(thread_id, &po_theta[ii][dens_count], &po2_theta[ii][dens_count], &po3_theta[ii][dens_count], \
						ii, dens[ii][dens_count], d[ii], loglFunc, loglFunc_arg, xx_mode, NULL, NULL); \
		}							\
	}
	...
	if (d[ii] || ai_par->cpo_manual) {				\
		...
		if (dic) {						\
			deviance_theta_local[ii] =			\
				GMRFLib_ai_dic_integrate(thread_id, ii, dens_local[ii], (ai_par->cpo_manual ? 1.0 : d[ii]), loglFunc, loglFunc_arg, xx_mode, NULL, NULL); \
		}							\
	}
	...
	if (d[ii]) {							\
		if (po) {						\
			GMRFLib_ai_po_integrate(thread_id, &po_theta_local[ii], &po2_theta_local[ii], &po3_theta_local[ii], \
						ii, dens_local[ii], d[ii], loglFunc, loglFunc_arg, xx_mode, NULL, NULL); \
		}							\
	}
...
#define COMPUTE_LOCAL  COMPUTE_CPO_AND_DIC_LOCAL; COMPUTE_PO_LOCAL;
	...
	if (GMRFLib_inla_mode == GMRFLib_MODE_CLASSIC) {
		...
		need_Qinv = (GMRFLib_inla_mode == GMRFLib_MODE_CLASSIC) && (compute_n || ai_par->compute_nparam_eff);
	...
	if (dic) {
		...
		for (j = 0; j < compute_n; j++) {
			...
			if (d[ii]) {
				...
				loglFunc(thread_id, NULL, &logl, &x_tmp, 1, ii, x_vec, NULL, loglFunc_arg);
				...
				if (loglFunc(0, NULL, NULL, NULL, 0, ii, NULL, NULL, loglFunc_arg) == GMRFLib_LOGL_COMPUTE_CDF) {
					loglFunc(0, NULL, &sig, &((*density)[ii]->user_mean), -1, ii, NULL, NULL, loglFunc_arg);
					sig = (sig <= 0.5 ? -1.0 : 1.0);
				} else {
					double xx[2], ld[2] = { 0.0, 0.0 };
					xx[0] = (*density)[ii]->user_mean;
					xx[1] = xx[0] + 0.01 * (*density)[ii]->user_stdev;
					loglFunc(0, NULL, ld, xx, 2, ii, NULL, NULL, loglFunc_arg);
			  ...
			  void *loglFunc_arg, double *x_vec)
{
	...
	if (density->type == GMRFLib_DENSITY_TYPE_GAUSSIAN) {
		...
		loglFunc(thread_id, NULL, loglik, x_user, np, idx, x_vec, NULL, loglFunc_arg);
...
#pragma GCC ivdep
		...
		loglFunc(thread_id, NULL, loglik, xp, np, idx, x_vec, NULL, loglFunc_arg);
#pragma GCC ivdep
		for (i = 0; i < np; i++) {
			loglik[i] *= d;
		}
