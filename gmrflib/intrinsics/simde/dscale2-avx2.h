{
	__m256d va = simde_mm256_set1_pd(a);
	int i = 0;
	for (; i <= n - 16; i += 16) {
		__m256d x0 = simde_mm256_loadu_pd(&x[i]);
		__m256d x1 = simde_mm256_loadu_pd(&x[i + 4]);
		__m256d x2 = simde_mm256_loadu_pd(&x[i + 8]);
		__m256d x3 = simde_mm256_loadu_pd(&x[i + 12]);
	...
	for (; i <= n - 4; i += 4) {
		__m256d vx = simde_mm256_loadu_pd(&x[i]);
