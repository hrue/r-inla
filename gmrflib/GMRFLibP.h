...
#       define Calloc_init(n_, m_)						\
	size_t calloc_m_ = (m_);					\
	size_t calloc_mem_align_ = GMRFLib_memory_alignment / sizeof(double);	\
...
#       define iCalloc_init(n_, m_)						\
	...
	size_t icalloc_mem_align_ = GMRFLib_memory_alignment / sizeof(int);	\
...
#       define Malloc_init(n_, m_)						\
	size_t malloc_m_ = (m_);					\
	size_t malloc_mem_align_ = GMRFLib_memory_alignment / sizeof(double);	\
...
#       define P2(x, y)    if (1) { printf("[%s:%1d] " #x " = [ %.16f ] " #y " = [ %.16f ]\n",__FILE__, __LINE__,(double)(x), (double)(y)); }
...
#       define GMRFLib_CACHE_LEN_NUMA() (GMRFLib_MAX_THREADS2() * (1 + GMRFLib_NUMA_NODES()) + 2 * GMRFLib_MAX_THREADS())
...
#       define CODE_BLOCK_INIT()						\
	...
	int t_num__ = (need_work__ ? (nt__ == 1 ? 0 : tid__) : 0); \
