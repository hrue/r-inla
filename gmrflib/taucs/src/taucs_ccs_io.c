...
int taucs_ccs_write_ijv(taucs_ccs_matrix * m, char *ijvfilename)
{
...
int taucs_dtl(ccs_write_ijv) (taucs_ccs_matrix * m, char *ijvfilename) {
	int i, ip, j, n;
	taucs_datatype Aij;
	FILE *f;
...
taucs_ccs_matrix *taucs_ccs_read_ijv(char *ijvfilename, int flags)
{
...
taucs_ccs_matrix *taucs_dtl(ccs_read_ijv) (char *ijvfilename, int flags) {
	FILE *f;
	taucs_ccs_matrix *m;
	int *clen;
	int *is;
...
taucs_ccs_matrix *taucs_ccs_read_mtx(char *mtxfilename, int flags)
{
...
taucs_ccs_matrix *taucs_ccs_read_ccs(char *ccsfilename, int flags)
{
...
taucs_ccs_matrix *taucs_dtl(ccs_read_ccs) (char *filename, int flags) {
	...
	/*
	 * int* clen; int* is; int* js; taucs_datatype* vs; int ncols, nrows, nnz; int i,ip,j,k,n; 
	 */
	/*
	 * taucs_datatype dv;
	 */
...
#ifdef OSTYPE_win32
	f = open(filename, _O_WRONLY | _O_CREAT | _O_BINARY, _S_IREAD | _S_IWRITE | _S_IEXEC);
#else
