...
#pragma omp critical (Name_4afa98d4e0e7cc3aec97acb922c2fa7fb65a660f)
	...
	if (mean == NULL) {
		...
		*logdens = A - problem->sub_logdens;
		...
		GMRFLib_ai_log_posterior(thread_id, &ldens, problem->mean_constr, b, c, mean, d, loglFunc, loglFunc_arg, graph,
						  ...
						  GMRFLib_preopt_tp *preopt, GMRFLib_idx_tp *d_idx)
{
	...
	int num_threads = (GMRFLib_OPENMP_IN_SERIAL()? GMRFLib_ADAPTIVE_NUM_THREADS() : GMRFLib_openmp->max_threads_inner);
...
#undef CODE_BLOCK
		...
		if (!lproblem) {
			...
			int ret;
			ret =
			    GMRFLib_init_problem_store(thread_id, &lproblem, x, bb_use, cc_use, mean, graph, Qfunc, Qfunc_arg,
						       constr, store, &stiles_idx, NULL);

			if (ret != GMRFLib_SUCCESS) {
				catch_error = 1;
			}
		} else {
			GMRFLib_init_problem_store(thread_id, &lproblem, x, bb_use, cc_use, mean, graph, Qfunc, Qfunc_arg, constr,
						   store, &stiles_idx, NULL);
		}
...
#define SET_MODE							\
	...
	double *hessian = NULL, *theta = NULL, *theta_mode = NULL, *x_mode = NULL, log_dens_mode = 0, log_dens, *z = NULL, **izs =
	    NULL, *stdev_corr_pos = NULL, *stdev_corr_neg = NULL, f, w, w_origo, tref = 0.0, tu, *weights = NULL, *adj_weights =
...
#pragma omp critical (Name_4edc3808412cc155a1bc15b9d4cd319a85e133b9)
					...
					if (stupid_mode_iter >= ai_par->stupid_search_max_iter) {
						...
						fprintf(stderr, "\n\n");
						break;
						// GMRFLib_ASSERT(stupid_mode_iter < ai_par->stupid_search_max_iter,
						// GMRFLib_EMISC);
					}
				}
			}
		...
		if (stupid_mode_iter ||
		    ((GMRFLib_smtp == GMRFLib_SMTP_STILES) && (ai_store->problem->stiles_idx->in_group > 0))) {
			// if we come from studpid_search_mode, then we need to reset the mode

			// if using sTiles, then we need to make sure 'in_group=0' for ai_store. we turn off, temporary adaptive
			// mode. This cause an issue for gcpo calculations and an error will be triggered there, unless we do this

			int adaptive = -1;
			if (GMRFLib_smtp == GMRFLib_SMTP_STILES) {
				adaptive = GMRFLib_openmp->adaptive;
			...
			SET_MODE;
			if (GMRFLib_smtp == GMRFLib_SMTP_STILES) {
				GMRFLib_openmp->adaptive = adaptive;
			}
		}
...
#pragma omp parallel for num_threads(nt)
			for (int k = 0; k < nhyper; k++) {
				...
				if (GMRFLib_smtp == GMRFLib_SMTP_STILES) {
					if (!(s->problem->stiles_idx)) {
						s->problem->stiles_idx = Calloc(1, GMRFLib_stiles_idx_tp);
					}
					Memcpy(s->problem->stiles_idx, &stiles_idx, sizeof(GMRFLib_stiles_idx_tp));
				}
...
#pragma omp parallel for num_threads(1)
	...
	if (nlin) {
		// we have to shift indices for the preopt format. Any indices belonging to Predictor and/or APredictor will then
		// become
...
#define GMRFLib_OPENMP_ENSURE_IN_PARALLEL_EXPR gcpo_groups = GMRFLib_gcpo_build(thread_id, ai_store, preopt, gcpo_param, fl, d_idx, ai_par->gcpo_fixed_nodes)
		GMRFLib_OPENMP_ENSURE_IN_PARALLEL_RUN();
#undef GMRFLib_OPENMP_ENSURE_IN_PARALLEL_EXPR
...
#pragma omp parallel for private(log_dens, dens_count, tref, tu, ierr) num_threads(nt)
	for (int k = 0; k < design->nexperiments; k++) {
		...
		if (GMRFLib_smtp == GMRFLib_SMTP_STILES) {
			Memcpy(ai_store_id->problem->stiles_idx, &stiles_idx, sizeof(GMRFLib_stiles_idx_tp));
		}
		...
		if ((nhyper > 0 || design->nexperiments > 1) && !ai_par->mode_fixed) {
			if (design->std_scale) {
		...
		double *c_corrected = Calloc(graph->n, double);
...
#pragma omp parallel for num_threads(GMRFLib_openmp->max_threads_inner) schedule(static)
		...
		if (!early_stop[dens_count]) {
			if (ai_par->vb_enable && (ai_par->vb_strategy == GMRFLib_AI_VB_MEAN || ai_par->vb_strategy == GMRFLib_AI_VB_VARIANCE)) {
				GMRFLib_ai_vb_correct_mean_preopt(thread_id, dens, dens_count, c, d, prior_mean, ai_par, ai_store_id, graph,
								  (tabQfunc ? tabQfunc->Qfunc : Qfunc),
								  (tabQfunc ? tabQfunc->Qfunc_arg : Qfunc_arg), loglFunc, loglFunc_arg, preopt,
								  d_idx);
			}
			...
			if (ai_par->vb_enable && (ai_par->vb_strategy == GMRFLib_AI_VB_VARIANCE)) {
				GMRFLib_ai_vb_correct_variance_preopt(thread_id, dens, dens_count,
								      ...
								      (tabQfunc ? tabQfunc->Qfunc : Qfunc),
								      (tabQfunc ? tabQfunc->Qfunc_arg : Qfunc_arg), loglFunc, loglFunc_arg,
								      preopt, c_corrected, d_idx);
			}
		}
...
#pragma omp parallel for num_threads(GMRFLib_openmp->max_threads_inner) schedule(static)
		...
		if (misc_output->configs_preopt && cpo) {
			...
			GMRFLib_dfill(3 * preopt->Npred, NAN, cpodens_moments);
		}
			...
			for (int ii = 0; ii < d_idx->n; ii++) {
				...
				if (cpo) {
					...
					failure_theta[i][dens_count] = GMRFLib_ai_cpopit_integrate(thread_id,
												   ...
												   &pit_theta[i][dens_count], i,
												   cpodens, d[i], loglFunc,
...
#pragma omp critical (Name_8a7254c4a570078955ae0e221dd0594e23386e57)
			{
				 ...
				 */
				fprintf(ai_par->fp_log, " ] ldens= %6.3f, [%1d] accept, compute,",
					log_dens_orig - log_dens_mode, omp_get_thread_num());
				fprintf(ai_par->fp_log, " %.2fs%s\n", tu, (early_stop[dens_count] ? " (interupt)" : ""));
			}
		}
	...
	*density = Calloc(preopt->n + preopt->mnpred, GMRFLib_density_tp *);
	...
	if (nearly) {
		if (ai_par->fp_log) {
#pragma omp critical (Name_4b8306c3c423d37d341e318c658d40a1ba84da71)
			{
				fprintf(ai_par->fp_log, "number of early stops = %1d\n", nearly);
			}
		}
	}
...
#pragma omp parallel for num_threads(GMRFLib_openmp->max_threads_outer) schedule(static)
		for (int j = 0; j < preopt->Npred; j++) {
			...
			for (int jjj = 0; jjj < dens_max; jjj++) {
				...
				// gcpo_theta[jjj][j]->marg_theta_correction = DMAX(-15.0,
				// gcpo_theta[jjj][j]->marg_theta_correction);
...
#pragma omp parallel for num_threads(GMRFLib_openmp->max_threads_outer)
	...
	if (dic) {
		...
		GMRFLib_dfill(ndev, NAN, e_deviance);
		GMRFLib_dfill(ndev, NAN, e_deviance_sat);
		GMRFLib_dfill(ndev, NAN, deviance_e);
...
#pragma omp parallel for num_threads(GMRFLib_openmp->max_threads_outer) reduction(+ : deviance_mean,  deviance_mean_sat, mean_deviance, mean_deviance_sat) schedule(static)
		for (int j = 0; j < d_idx->n; j++) {
			...
			loglFunc(thread_id, lcache_idx, &logl, &x_tmp, 1, ii, x_vec, NULL, loglFunc_arg);
			...
			// neither of these options are fail-safe. I cannot see how to do this fail-safe without really mapping to
			// the
			// real data doing the comparison there. But this information is not available at this level
			double sig = 0.0;
			if (loglFunc(0, lcache_idx, NULL, NULL, 0, ii, NULL, NULL, loglFunc_arg) == GMRFLib_LOGL_COMPUTE_CDF) {
				loglFunc(0, lcache_idx, &sig, &x_tmp, -1, ii, NULL, NULL, loglFunc_arg);
				sig = (sig <= 0.5 ? -1.0 : 1.0);
			} else {
				double xx[2], ld[2] = { 0.0, 0.0 };
				xx[0] = x_tmp;
				xx[1] = xx[0] + 0.01 * (*density)[ii]->user_stdev;
				loglFunc(0, lcache_idx, ld, xx, 2, ii, NULL, NULL, loglFunc_arg);
				...
				for (int kkk = 0; kkk < probs->n; kkk++) {
					...
					if (ai_par->fp_hyperparam) {
						...
						fprintf(ai_par->fp_hyperparam, " %.10g %.10g\n",
							hyper_ldens[k] + log_dens_mode + log_jacobian, probs->val[kkk]);
					}
				}
			}
			if (ai_par->fp_hyperparam) {
	...
	if (deviance_theta) {
		for (int i = 0; i < preopt->Npred; i++) {
			if (deviance_theta[i]) {
				for (int j = 0; j < dens_max; j++) {
					Free(deviance_theta[i][j]);
				}
			}
			Free(deviance_theta[i]);
		}
		Free(deviance_theta);
	}
	if (free_ai_par) {
		...
		if (gcpo_param->build_strategy == GMRFLib_GCPO_BUILD_STRATEGY_PRIOR) {
			...
			if (gcpo_param->remove) {
				...
				for (int j = 0; j < gcpo_param->remove->n; j++) {
					if (!visited[j]) {
						err++;
						printf("\n[%1d] %s:%1d: *** error *** ->remove[%1d]=[%s] is not found, abort!\n\n",
			...
			if (gcpo_param->keep) {
				...
				for (int j = 0; j < gcpo_param->keep->n; j++) {
					if (!visited[j]) {
						err++;
						printf("\n[%1d] %s:%1d: *** error *** keep[%1d]=[%s] is not found, abort!\n\n",
			...
			GMRFLib_init_problem(thread_id, &problem, NULL, NULL, c, NULL,
					     preopt->preopt_graph, preopt->gcpo_Qfunc, preopt->preopt_Qfunc_arg,
					     preopt->latent_constr, &stiles_idx, NULL);
			...
			if (GMRFLib_smtp == GMRFLib_SMTP_STILES) {
				GMRFLib_stiles_idx_tp stiles_idx = { 0, -1, 0 };
				GMRFLib_stiles_set_idx(&stiles_idx, 1);
				if (build_ai_store->problem->stiles_idx) {
					if (build_ai_store->problem->stiles_idx->within_group != stiles_idx.within_group ||
					    build_ai_store->problem->stiles_idx->in_group != stiles_idx.in_group) {
						P(build_ai_store->problem->stiles_idx->within_group);
						P(stiles_idx.within_group);
						P(build_ai_store->problem->stiles_idx->in_group);
						P(stiles_idx.in_group);
						assert(build_ai_store->problem->stiles_idx->in_group == stiles_idx.in_group);
						assert(build_ai_store->problem->stiles_idx->within_group == stiles_idx.within_group);
					}
					*(build_ai_store->problem->stiles_idx) = stiles_idx;
				}
			}
		}
...
#pragma omp simd
		...
		if (GMRFLib_smtp == GMRFLib_SMTP_STILES) {
			...
			nrhs = IMIN(GMRFLib_stiles_get_block_size(), 1 + selection->n / nt_outer);
		} else {
...
#define BLOCK 16
		GMRFLib_idx_tp **A_idx4 = NULL;
		if (gcpo_param->min_overlap > 0) {
			A_idx4 = Calloc(dn, GMRFLib_idx_tp *);
			...
			for (int i = 0; i + BLOCK -1 < dn; i += BLOCK) {
				...
				GMRFLib_idx_create_x(&(A_idx4[node]), m);
				for (int j = 0, off = 0; j < BLOCK; j++) {
					int nnode = d_idx->idx[i + j];
					int nn = A_idx(nnode)->n;
					Memcpy(A_idx4[node]->idx + off, A_idx(nnode)->idx, nn * sizeof(int));
					off += nn;
				}
				A_idx4[node]->n = m;
				GMRFLib_sort_i(A_idx4[node]->idx, m);
			}
		}
...
#       define NLOC 6
		...
		int show_timer = 1 || gcpo_param->verbose;
#endif
...
#pragma omp parallel for num_threads(nt_inner) if (nt_inner > 1)
			for (int ii = 0; ii < sel->n; ii++) {
				...
				int min_overlap = gcpo_param->min_overlap;
				if (min_overlap == 0) {
					d_idx_local = d_idx;
				} else {
					// bitmap is the bitmap for all (latent) variables making up linpred[node], and the union
					// of each nodes neigbours
...
#pragma omp threadprivate(ttn)
						...
						if (0) {
							...
							for (int knode = 0; knode < dn; knode++) {
								...
								if (unlikely(node == nnode) || GMRFLib_idxval_nmatch(A_idx(nnode), bitmap) >= min_overlap) {
									GMRFLib_idx_add(&d_idx_local, nnode);
								}
							}
						} else {
							// unroll the code in blocks of BLOCK. the advantage is to check that consequtive
				...
				if (d_idx_local->n == 1) {
					...
					while (!levels_ok) {
						...
						groups[node]->n = 0;
						size_t siz_g = sMIN((size_t) d_idx_local->n, (size_t) (levels_magnify * (IABS(gcpo_param->num_level_sets) + 4L)));
						...
						gsl_sort_largest_index(largest, siz_g, cor_abs, (size_t) 1, (size_t) d_idx_local->n);

						double sumw = W(node);
						double cor_abs_prev = 1.0;
						int i_prev_l = (int) largest[0];
						int i_prev = d_idx_local->idx[i_prev_l];
						GMRFLib_idxval_add(&(groups[node]), i_prev, cor_abs_prev);
						for (size_t i = 1; i < siz_g && !levels_ok; i++) {
							int i_new_l = (int) largest[i];
							int i_new = d_idx_local->idx[i_new_l];
							...
							// printf("count_safe %d i %zu groups[%1d]->n = %1d, d_idx_local->n = %1d, levels_ok = %1d\n",
							// count_safe, i, node, groups[node]->n, d_idx_local->n, levels_ok);
						}
						if (levels_ok) {
							if (gcpo_param->verbose || detailed_output) {
						...
						if (new_node < gcpo_param->friends_n) {
							for (int j = 0; j < gcpo_param->friends[new_node]->n; j++) {
								...
								if (LEGAL(new_node2, Npred)
									...
									int idx_l = GMRFLib_idx_find(new_node2, d_idx_local);
									if (idx_l >= 0) {
...
#       undef NLOC
		...
		// if the number of given groups are to short compared to Npred, then pad with empty groups. if its longer, then
		// that is an error.
...
#define A_idx(node_) (preopt->pAA_idxval ? preopt->pAA_idxval[node_] : preopt->A_idxval[node_])
	...
	// if 'serial', we will (likely) be within a parallel region with on thread
	int serial = (GMRFLib_OPENMP_IN_PARALLEL_ONE_THREAD() || GMRFLib_OPENMP_IN_SERIAL());
...
#pragma omp critical (Name_6bbd084d591e27d9aacdb520c3e898945c242003)
		...
		loglFunc(thread_id, &cache_idx, logcor, x_user, np, idx, NULL, NULL, loglFunc_arg);
...
#pragma omp critical (Name_0713ff01bf46f0328663d7242f8e788872085a66)
	...
	loglFunc(thread_id, &cache_idx_numa, loglik, x_user, GMRFLib_INT_GHQ_POINTS, idx, x_vec, NULL, loglFunc_arg);
...
#define SHOW_TIME(msg_)							\
	...
	// save time: only compute MM the first time, and keep MM and its factorisation fixed during the iterations. the motivation 
	// 
	// is that the
	...
	GMRFLib_ai_add_Qinv_to_ai_store(ai_store);	       /* add Qinv if its not there already */
...
#pragma omp critical (Name_1169f76e685daed4d69fb5a745f9e95b4f5f633b)
			{
				...
				for (int k = 0; k < (int) (sizeof(fps) / sizeof(FILE *)); k++) {
					...
					if (max_corr_flag) {
						...
						fprintf(fpp, "\t*** You can change the emergency value (current value=%.2f) by \n",
							ai_par->vb_emergency);
						fprintf(fpp, "\t*** \t'control.inla=list(control.vb=list(emergency=...))'\n\n");
					}
					fprintf(fpp, "\t*** Please (re-)consider your model, priors, confounding, etc.\n");
				}
			}
			emergency = 1;
			break;
...
#define FUN_DERIV(theta_) (1.0)
	...
	// the gradient of variance wrt theta (additive) is -var()^2, which sugguests c_like = 1/var()^2, but it seems a little to
	// much. we add
...
#       define STORAGE_TYPE_CAST
	...
	for (iter = 0; iter < niter + 1; iter++) {	       /* yes, it should be '+1': we 'break' at the top */
		...
		if (iter == 0) {
			...
			// with iter==1 we know we used ai_store->problem, which should not be free'd, so we need to wait until
			// iter>1
			if (iter > 1) {
				GMRFLib_free_problem(problem);
			}
			...
			int retval = GMRFLib_init_problem(thread_id, &problem, NULL, NULL, c_add, x_mean, graph, Qfunc, Qfunc_arg,
							  ai_store->problem->sub_constr, &stiles_idx, NULL);
			if (retval != GMRFLib_SUCCESS) {
				problem = NULL;
				break;
			}
...
#pragma omp critical (Name_665f8b032e79706ed21a89a78c139f0eac1f454a)
				{
					...
					fprintf(fp,
						"\t\tNumber of nodes corrected for [%1d] |gradient|[%.4g] max.step.sigma[%.4g](%s)\n",
...
#pragma omp critical (Name_0ddd01862f572e8e2021d8c931021738790dccc7)
	...
	for (int iter = 0; iter < max_iter; iter++) {
		...
		loglFunc(thread_id, &cache_idx, loglik, x_user, np, idx, x_vec, NULL, loglFunc_arg);
				   ...
				   double *gcpodens_moments, double *ll_info, double *lpred_mean, double *lpred_variance, double *c_corrected)
{
	...
	// mo->configs_preopt[id]->iprior etc, but we just store the matrices in a vector, in the same order as the indices were
	// stored.
...
#pragma omp parallel for num_threads(nt)
	for (int i = 0; i < nlin; i++) {
		...
		GMRFLib_solve_l_sparse_matrix_special(vv, &(problem->sub_sm_fact), problem->sub_graph, from_idx, from_idx + len - 1, 1, problem);
		v = Calloc(len, double);
...
#pragma omp critical (Name_1763564c120acdb32a710c9e67c37ab1e0f5eea3)
	...
	retval = loglFunc(thread_id, &cache_idx, NULL, NULL, 0, idx, x_vec, NULL, loglFunc_arg);
...
#pragma omp simd
	...
	if (compute_cpo) {
		loglFunc(thread_id, &cache_idx, prob, xp, -np, idx, x_vec, NULL, loglFunc_arg);	/* no correction for 'd' here; should we? */
	} else {
		GMRFLib_dfill(np, 0.0, prob);
	}
	loglFunc(thread_id, &cache_idx, loglik, xp, np, idx, x_vec, NULL, loglFunc_arg);
...
#pragma omp simd
		...
		loglFunc(thread_id, &lcache_idx, loglik, xp, np, idx, x_vec, NULL, loglFunc_arg);
