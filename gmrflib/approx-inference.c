...
#pragma omp parallel for private(k, i, j)
			for(k = 0; k<klen; k++) {
				...
				if (ij_from <= ij_to) {
					...
					(*cov)[i + j * nlin] = (*cov)[j + i * nlin] = ddot_(&ij_len, v_i, &one, v_j, &one);
				} else {
					(*cov)[i + j * nlin] = (*cov)[j + i * nlin] = 0.0;
				}
				...
				if (nc) {
					...
					(*cov)[i + j * nlin] = (*cov)[i + j * nlin] - correction;
					(*cov)[j + i * nlin] = (*cov)[i + j * nlin];
				}
			}
