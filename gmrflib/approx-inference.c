...
int GMRFLib_init_GMRF_approximation_store__intern__old(GMRFLib_problem_tp ** problem, double *x, double *b, double *c, double *mean,
...
int GMRFLib_default_ai_param(GMRFLib_ai_param_tp ** ai_par)
{
	...
	*ai_par = Calloc(1, GMRFLib_ai_param_tp);
	...
	// LM parameters
	(*ai_par)->optpar_lambda0 = 0.001;
	(*ai_par)->optpar_tr_increase = 10;
	(*ai_par)->optpar_tr_decrease = 10;
	(*ai_par)->optpar_max_tries = 15;
...
#pragma omp critical
	...
	if (!(ai_store->problem)) {
		...
		GMRFLib_EWRAP1(GMRFLib_init_GMRF_approximation_store__intern(&(ai_store->problem),
																	 ...
																	 ai_store->store, ai_store->aa, ai_store->bb, ai_store->cc,
																	 ai_par->gaussian_data, ai_par->cmin));
...
{
	/*
	 * This is copy of the original routine but with optional two last arguments.  It has now been modified to include a "safer" optimizer.
	 */
	if (0 && optpar){
		optpar->fp = optpar->fp?optpar->fp:stdout;  //Debug hack
	}
	
	if (1) { //Run the Levenberg-Marquardt method
		...
		*idxs = NULL, nidx = 0;
		...
		//Compute intial value of the density.  Used for stopping criterion. 
		double ldens_old = 0.0;
		double logdens_old = 0.0;
		double ldens_new = 0.0;
		double logdens_new = 0.0;
		/*	GMRFLib_ai_log_posterior(&ldens_old, x, b, c, mean, d, loglFunc, loglFunc_arg, fixed_value, graph,
		 Qfunc, Qfunc_arg, constr);
		 if (problem) // if !problem, logdens_old =0.0. This probably always happens!
		 {
		 GMRFLib_evaluate(problem);
		 logdens_old = ldens_old - problem->sub_logdens;
		 }
		 */
...
if (free_cc) Free(cc); Free(mode); Free(idxs); Free(cc_trust); }
		...
		double lambda=0.0;
		if(!gaussian_data) {
			lambda = 0.0; //optpar->lambda0; <- Here we have NO REGULARISATION until something goes wrong!
		}
		...
		int ret = 0; // Used to see if the factorization succeeded.
...
#pragma omp parallel for private(i) schedule(static)
			...
			do {
				...
				for (i =0; i <n; i++) {
					...
					minmin = DMIN(minmin,lambda*fabs(cc[i]+ Qfunc(i,i,Qfunc_arg)));
					maxmax = DMAX(maxmax,lambda*fabs(cc[i]+ Qfunc(i,i,Qfunc_arg)));
				}
				...
				if (!lproblem) {
					if (GMRFLib_catch_error_for_inla) {
						...
						//						int ret;  // using this elswhere as well. Declaring it outside loop.
						...
						if (ret != GMRFLib_SUCCESS) {
							catch_error = 1;
							GMRFLib_free_problem(lproblem); //lproblem needs to be undeclared, otherwise we don't get here!
							lproblem = NULL;
						}
					} else {
				...
				if (catch_error) {
					...
					if ((lambda < DBL_EPSILON) || gaussian_data) {
						//This shouldn't happen fo Gaussian data, but the case is added for super-duper safety.
						lambda = optpar->lambda0;
					}
					...
					//	printf("%i th try failed!  New lambda=%g\n",number_of_tries,lambda);
			...
			} while (catch_error );  //added !lproblem just in case GMRFLib_catch_error_for_inla=0
			...
			//Note: Another option would be to have a mode_prop which is not adopted if there is an increase...
			...
			int do_nothing=0; //shrink the trust region.
			
			
			/*Compute new value of the log-density*/
			if (lambda > DBL_EPSILON) {
				// Gah! We have to re-compute the factorization without regularization
				
				ret = GMRFLib_init_problem_store(&lproblem, lproblem->mean_constr, bb, cc, mean, graph, Qfunc, Qfunc_arg, fixed_value, constr,
												 GMRFLib_NEW_PROBLEM, store);
				if (ret!=GMRFLib_SUCCESS) {  //What happens if the factorisation fails?
					// Do nothing to the regularisation. Or we may expand... don't really know which is best.
					do_nothing=1;
					
					logdens_new = logdens_old;  //HAVARD: IS THERE AN ERROR INTEGER?!
				} 
			}  
			if (!do_nothing) {
				GMRFLib_evaluate(lproblem);
				GMRFLib_ai_log_posterior(&ldens_new, lproblem->mean_constr, bb, cc, mean, d, loglFunc, loglFunc_arg, fixed_value, graph,
										 Qfunc, Qfunc_arg, constr);
				
				logdens_new =  ldens_new - lproblem->sub_logdens;
			...
			if (optpar && optpar->fp)
				fprintf(optpar->fp,"Iter %i: ldens_old = %g, ldens_new = %g :\n", iter,ldens_old, ldens_new);
			
			if (iter == 0) {
				err_previous = err;
				do_nothing = 1; // don't shrink or expand  on the first iteration.
			...
			if (!do_nothing && (lambda > DBL_EPSILON) && (logdens_new < logdens_old)) {
				
				lambda /= optpar->tr_decrease;
				ldens_old = ldens_new;
				//	printf("Decreasing TR radius!  New lambda=%g\n",lambda);
				if (optpar && optpar->fp) //Don't say we're going down!
					fprintf(optpar->fp,"Decreasing TR radius!  New lambda=%g\n",lambda);
			}
			
			if (!do_nothing && (ldens_old > ldens_new)) {
				if (lambda < DBL_EPSILON) {
					lambda = optpar->lambda0;
				}
				lambda *= optpar->tr_increase;
				//printf("Increasing TR radius!  New lambda=%g\n",lambda);
				if (optpar && optpar->fp)
...
#pragma omp parallel for private(i) schedule(static)
			...
			//When the density is computed properly, a "it hasn't moved much" criterion will be useful!
		...
		//printf("Gaussian Approximation in %i iters \n",iter);
		
		/* Recompute GA if lambda is too big - this should be fl(eps)... */
		if (lambda > 100*DBL_EPSILON) {
...
#undef FREE_ALL
		...
		// RUN THE OLD CODE
		int ret;
		ret = GMRFLib_init_GMRF_approximation_store__intern__old(problem, x, b, c, mean,
																 d, loglFunc, loglFunc_arg, fixed_value,
																 graph, Qfunc, Qfunc_arg,
																 constr,  optpar,
																 blockupdate_par,  store,
																 aa, bb,cc,  gaussian_data,  cmin);
		return ret;
	}
			...
			for (i = 0; i < compute_n; i++) {
				...
				if (0) {
					...
					for (jj = 0; jj < graph->n; jj++) {
						printf("mean/sd %d %.12g %.12g\n", jj, p->mean_constr[jj], sqrt(*GMRFLib_Qinv_get(p, jj, jj)));
					}
