...
#pragma omp parallel for private(i) num_threads(GMRFLib_openmp->max_threads_inner)
		...
		GMRFLib_ai_store_config_preopt(misc_output, nhyper, theta_local, log_dens_mode, log_dens_orig, ai_store_id->problem, mean_corrected, preopt, Qfunc, Qfunc_arg);
...
#pragma omp parallel for num_threads(GMRFLib_openmp->max_threads_outer)
	for (int ii = 0; ii < preopt->mnpred + graph->n; ii++) {
		...
		if (ii < preopt->mnpred) {
			...
			GMRFLib_density_combine(&dens_combine, NULL, dens_max, lpred[i], adj_weights);
			(*density)[i] = dens_combine;
		} else {
			i = ii - preopt->mnpred;
			GMRFLib_density_tp *dens_combine = NULL;
			GMRFLib_density_combine(&dens_combine, NULL, dens_max, dens[i], adj_weights);
			(*density)[ii] = dens_combine;	       /* yes, its 'ii' */

			if (tfunc && tfunc[i]) {
				GMRFLib_density_tp *dens_c = NULL;
				GMRFLib_density_combine(&dens_c, NULL, dens_max, dens_transform[i], adj_weights);
	...
	if (dlin && nlin) {
		...
		for (j = 0; j < nlin; j++) {
			...
			GMRFLib_density_combine(&dcombine, NULL, dens_max, dtmp, adj_weights);
			(*dlin)[j] = dcombine;
		}
		Free(dtmp);
		...
		if (ai_par->vb_max_correct > 0) {
			for (i = 0; i < graph->n; i++) {
				if (ABS(gsl_vector_get(delta_mu, i) / sd[i]) > ai_par->vb_max_correct) {
					gsl_vector_set(delta_mu, i, ai_par->vb_max_correct * sd[i] *
...
{
	GMRFLib_ENTER_ROUTINE;
	Calloc_init(4 * graph->n + 2 * preopt->mnpred + 2 * preopt->Npred);
...
#define SHOW_TIME(msg_)							\
	...
	GMRFLib_tabulate_Qfunc_core(&prior, a->latent_graph, GMRFLib_preopt_Qfunc_prior, Qfunc_arg, NULL, NULL, NULL, 1);
...
#define CODE_BLOCK							\
	for (int jj = 0; jj < vb_idx->n; jj++) {			\
		...
		double *res = CODE_BLOCK_WORK_PTR(2);			\
		GMRFLib_ai_update_conditional_mean2(cmean, ai_store->problem, j, ai_store->problem->mean_constr[j] + 1.0, NULL); \
		for (int i = 0; i < graph->n; i++) {			\
	...
	RUN_CODE_BLOCK(GMRFLib_MAX_THREADS, 3, graph->n);
#undef CODE_BLOCK
...
#define CODE_BLOCK							\
		for (int ii = 0; ii < d_idx->n; ii++) {			\
			CODE_BLOCK_SET_THREAD_ID;			\
			int i = d_idx->idx[ii];				\
			vb_coof[i] = GMRFLib_ai_vb_prepare(i, dens_local[i], d[i], loglFunc, loglFunc_arg, x_mean); \
		}
...
#undef CODE_BLOCK
		...
		GMRFLib_Qx2(tmp, x_mean, graph, prior->Qfunc, prior->Qfunc_arg, c);
		for (i = 0; i < graph->n; i++) {
			tmp[i] += preopt->total_b[GMRFLib_thread_id][i];
			gsl_vector_set(B, i, tmp[i]);
		}
		SHOW_TIME("compute KLD via _Qx2");
		...
		for (i = 0; i < graph->n; i++) {
			...
			double ddx = ABS(dx[i]) / sd[i];
			if (ddx < ai_par->vb_max_correct) {
				x_mean[i] += dx[i];
			} else {
				num_trunc++;
				x_mean[i] += ai_par->vb_max_correct * SIGN(dx[i]) * sd[i];
			}
		}
		SHOW_TIME("solve");
				   ...
				   GMRFLib_preopt_tp *preopt, GMRFLib_Qfunc_tp * Qfunc, void *Qfunc_arg)
{
	...
	if (!(mo->configs_preopt[id])) {
		...
		GMRFLib_graph_tp *g;
		GMRFLib_graph_duplicate(&(mo->configs_preopt[id]->graph), preopt->preopt_graph);
		GMRFLib_graph_duplicate(&(mo->configs_preopt[id]->prior_graph), preopt->latent_graph);
...
#define NEXTRA 19
	...
	if (interpolator == GMRFLib_AI_INTERPOLATOR_GRIDSUM) {
		 ...
		 */
		double *dens = NULL, *theta_tmp = NULL, *work = NULL, *npoints_j = NULL, ldens_max;
