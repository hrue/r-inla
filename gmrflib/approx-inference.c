...
#define FREE_ALL if (1) { if (free_b) Free(b); if (free_c) Free(c); if (free_d) Free(d); \
	...
	int vb = (getenv("INLA_VB_FIT") ? 1 : 0);
	FIXME("RUN WITH VB");
	
	double *lpred_var = NULL;
	if (vb) {
		lpred_var = Calloc(Npred, double);
	}
...
#undef CODE_BLOCK
			...
			if (vb) FIXME("RUN WITH VB");
			FIXME1("CHECK lpred_var");
#define CODE_BLOCK							\
			for (int i_ = 0; i_ < nidx; i_++) {		\
				int idx = idxs[i_];			\
				GMRFLib_ai_vb_fit_gaussian(thread_id, &(aa[idx]), &(bcoof[idx]), &(ccoof[idx]), NULL, idx, d[idx], \
							   loglFunc, loglFunc_arg, mode, linear_predictor[idx], sqrt(lpred_var[idx])); \
				cc_is_negative = (cc_is_negative || ccoof[idx] < 0.0); \
				if (ccoof[idx] == cmin && b_strategy == INLA_B_STRATEGY_SKIP) { \
					bcoof[idx] = 0.0;		\
				}					\
...
#pragma omp simd
		...
		if (vb) {
			if (1 || iter <= 1) {
				...
				GMRFLib_preopt_predictor_moments(NULL, lpred_var, preopt, lproblem, NULL);
			}
			for(int i = 0; i < Npred; i++) {
				printf("compute SD(lpred)[%1d] =  %g\n", i, sqrt(lpred_var[i]));
			}
		}
...
#pragma omp critical (Name_0ddd01862f572e8e2021d8c931021738790dccc7)
	...
	for (int iter = 0; iter < max_iter; iter++) {
		...
		double prior_sd = 4 * s, prior_mean = fit_mean, prior_var_inv = 1.0 / SQR(prior_sd);
		...
		double step_len = DMIN(1.0, (1.0 + iter) / 2.0);
