					  ...
					  GMRFLib_preopt_tp * preopt)
{
	...
	Calloc_init(6 * graph->n + 2 * preopt->mnpred + 4 * preopt->Npred + 1 * vb_idx->n);
	double *x_mean = Calloc_get(graph->n);
	double *pmean = Calloc_get(preopt->mnpred);
	double *pvar = Calloc_get(preopt->mnpred);
...
#define DELTA_LIMIT (10.0)
	...
	for (int iter = 0; iter < niter + 1; iter++) {	       /* yes, +1 */
		...
		Memset(c_add, 0, graph->n * sizeof(double));
		for (int jj = 0; jj < vb_idx->n; jj++) {
			int j = vb_idx->idx[jj];
		...
		int retval = GMRFLib_init_problem(thread_id, &problem, NULL, NULL, c_add, x_mean, graph, Qfunc, Qfunc_arg, ai_store->problem->sub_constr);
		if (retval != GMRFLib_SUCCESS) {
			problem = NULL;
			break;
		}
		...
		if (iter == 0) {
			...
			printf("[%1d]Iter [%1d/%1d] diff.sigma = %.12f (%s)\n", tn, iter, niter, diff_sigma, (pass ? "PASS" : "FAIL"));

			if (pass && ai_par->vb_verbose) {
				printf("\t[%1d]Iter [%1d/%1d] VB correct with strategy [variance] in total [%.3f]sec\n",
				       tn, iter, niter, GMRFLib_cpu() - tref);
				printf("\t\tNumber of nodes corrected for [%1d] diff.sigma[%.4f]\n", (int) vb_idx->n, diff_sigma);
				if (pass) {
					for (int jj = 0; jj < vb_idx->n; jj++) {
						int j = vb_idx->idx[jj];
...
#define DOT_PRODUCT_SERIAL_OLD(VALUE_, ELM_, ARR_)			\
		if (1) {						\
			double value_ = 0.0;				\
			double *vv_ = ELM_->val;			\
			double *aa_ = ARR_;				\
...
#define COV_ETA_LATENT_OLD(value_, k_, i_) DOT_PRODUCT_SERIAL(value_, A[k_], cov_latent[i_])
		...
		for (int ii = 0; ii < vb_idx->n; ii++) {		\
			...
			if (iter == 0 || update_hessian) {		\
				for (int jj = ii; jj < vb_idx->n; jj++) { \
					...
					trace *= 2.0;			\
					double val = (mell + 0.5 * (trace + ldet)) * param_correction_i * param_correction_j; \
					if (ii == jj) val = DMAX(0.0, val); \
