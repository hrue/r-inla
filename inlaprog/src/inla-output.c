...
#pragma omp parallel for num_threads(1)
	...
	if (mb->nf) {
		...
		GMRFLib_openmp_implement_strategy_special(div, IMAX(1, GMRFLib_MAX_THREADS() / div));
#pragma omp parallel for num_threads(div)
	...
	Dinit_core(mb->predictor_n + mb->predictor_n, fnm);
	Free(fnm);
	...
	if (mb->density_lin) {
		...
		inla_output_detail(mb->dir, &(mb->density_lin[ii]), mb->lc_order, mb->nlc,
				   1, mb->lc_output[ii], newdir2, mb->output->return_marginals, NULL, NULL, NULL, newtag2, NULL, local_verbose);
	...
	if (mb->density_hyper) {
		GMRFLib_openmp_implement_strategy_special(1, 1);
	...
	if (div <= GMRFLib_MAX_THREADS()) {
		...
		GMRFLib_openmp_implement_strategy_special(IMAX(1, GMRFLib_MAX_THREADS() / 4), 4);
	}
...
int inla_output_detail_gcpo(const char *dir, GMRFLib_gcpo_tp *gcpo, GMRFLib_gcpo_param_tp *param, int verbose)
{
	...
	GMRFLib_sprintf(&nndir, "%s/%s", ndir, "gcpo.dat");
	{
		int count = 1 + n * 4;
		for (i = 0; i < n; i++) {
			count += 2 * gcpo->groups[i]->n;
		}
		Dinit_core(count, nndir);
		if (verbose) {
...
#pragma omp critical (Name_f2b4e6c2e966340fa5f0b42b80cc8d45f1910623)
	...
	{
		Dinit_core(1 + ISQR(mo->nhyper), nndir);
		D1W(mo->nhyper);
	...
	{
		Dinit_core(mo->len_reordering, nndir);
		...
		for (id = 0; id < GMRFLib_MAX_THREADS(); id++) {
			if (mo->configs_preopt[id]) {
				if (!header) {
					...
					fwrite((void *) mo->configs_preopt[id]->i, sizeof(int), (size_t) mo->configs_preopt[id]->nz, fp);
					fwrite((void *) mo->configs_preopt[id]->j, sizeof(int), (size_t) mo->configs_preopt[id]->nz, fp);
					fwrite((void *) mo->configs_preopt[id]->iprior, sizeof(int), (size_t) mo->configs_preopt[id]->prior_nz, fp);
					fwrite((void *) mo->configs_preopt[id]->jprior, sizeof(int), (size_t) mo->configs_preopt[id]->prior_nz, fp);
					fwrite((void *) &nconfig, sizeof(int), (size_t) 1, fp);	/* yes!!! */
				...
				for (i = 0; i < mo->configs_preopt[id]->nconfig; i++) {
					...
					if (mo->configs_preopt[id]->config[i]->ll_info) {
						...
						fwrite((void *) (mo->configs_preopt[id]->config[i]->ll_info),
						       sizeof(double), (size_t) (4 * mo->configs_preopt[id]->Npred), fp);
					} else {
						double zero = 0.0;
		...
		case 0:
			if (output->kld) {
				...
				if (inla_computed(density, n)) {
					...
					Dinit_core(n * 2, nndir);
					GMRFLib_density_tp *gd = NULL;
					GMRFLib_density_create_normal(&gd, 0.0, 1.0, 0.0, 1.0, GMRFLib_FALSE);
					for (int i = 0; i < n; i++) {
						if (density[i]) {
							double kld;
							if (0) {
		...
		case 1:
			if (output->nquantiles) {
				if (inla_computed(density, n)) {
					...
					Dinit_core(n * (1 + 2 * output->nquantiles), nndir);
					for (int i = 0; i < n; i++) {
						double xp, p;
						if (density[i]) {
							D1W((locations ? locations[i % ndiv] : i));
		...
		case 3:
			if (output->ncdf) {
				if (inla_computed(density, n)) {
					...
					Dinit_core(n * (1 + 2 * output->ncdf), nndir);
					for (int i = 0; i < n; i++) {
						double xp, x, p;
						if (density[i]) {
							D1W((locations ? locations[i % ndiv] : i));
...
int inla_parse_output(inla_tp *mb, dictionary *ini, int sec, Output_tp **out)
{
	...
	if (!(mb->gcpo_param)) {
		...
		mb->gcpo_param->build_radius = iniparser_getint(ini, inla_string_join(secname, "GCPO.BUILD.RADIUS"), 1);
	...
	if (mb->verbose) {
		...
		if (use_defaults) {
			...
			printf("\t\t\t\tbuild.radius=[%1d]\n", mb->gcpo_param->build_radius);
