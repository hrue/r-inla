...
int loglikelihood_gaussian(int thread_id, double *logll, double *x, int m, int idx, double *UNUSED(x_vec), double *y_cdf, void *arg, char **arg_str)
{
	/*
	 * y ~ Normal(x, stdev)
	 */
				...
				char **UNUSED(arg_str))
{
	...
	if (m > 0) {
		for (i = 0; i < m; i++) {
			...
			logll[i] = cons + (a - 1.0) * log(xx) + (b - 1.0) * LOG_1mp(xx) + PREDICTOR_INVERSE_LINK_LOGJACOBIAN(eta);
		}
	}
...
int loglikelihood_poisson(int thread_id, double *logll, double *x, int m, int idx, double *UNUSED(x_vec), double *y_cdf, void *arg, char **arg_str)
{
...
#pragma omp simd
			for (int i = 0; i < m; i++) {
				...
				double mu = E * lambda;
				double p0 = exp(-mu);
				logll[i] = y * x[i] + ylEmn - mu - LOG_1mp(p0);
			}
		} else {
			for (int i = 0; i < m; i++) {
				double lambda = PREDICTOR_INVERSE_LINK(x[i] + off);
...
int loglikelihood_rcpoisson(int thread_id, double *logll, double *x, int m, int idx, double *UNUSED(x_vec), double *UNUSED(y_cdf), void *arg,
			    char **UNUSED(arg_str))
{
	if (m == 0) {
		return GMRFLib_SUCCESS;
...
int loglikelihood_0poissonS(int thread_id, double *logll, double *x, int m, int idx, double *UNUSED(x_vec), double *y_cdf,
			    void *arg, char **UNUSED(arg_str))
{
	if (m == 0) {
		return GMRFLib_LOGL_COMPUTE_CDF;
...
int loglikelihood_0binomial(int thread_id, double *logll, double *x, int m, int idx, double *UNUSED(x_vec), double *y_cdf,
			    void *arg, char **UNUSED(arg_str))
{
	if (m == 0) {
		return GMRFLib_LOGL_COMPUTE_CDF;
...
int loglikelihood_0binomialS(int thread_id, double *logll, double *x, int m, int idx, double *UNUSED(x_vec), double *y_cdf,
			     void *arg, char **UNUSED(arg_str))
{
	if (m == 0) {
		return GMRFLib_LOGL_COMPUTE_CDF;
	...
	if ((int) y == 0) {
		...
		if (m > 0) {
			for (i = 0; i < m; i++) {
				...
				mu = E * lambda;
				p0 = exp(-mu);
				logll[i] = LOG_1mp(p) + y * log(mu) - mu - normc - LOG_1mp(p0);
			}
...
int loglikelihood_mix_gaussian(int thread_id, double *logll, double *x, int m, int idx, double *x_vec, double *y_cdf, void *arg, char **arg_str)
{
	return (loglikelihood_mix_core
...
int loglikelihood_exp(int thread_id, double *logll, double *x, int m, int idx, double *UNUSED(x_vec), double *y_cdf, void *arg, char **arg_str)
{
	/*
	 * y ~ Exponential
	 */
...
#define SAFEGUARD(value_, m_) for(int i_ = 0; i_ < (m_); i_++) value_[i_] = TRUNCATE(value_[i_], 0.0, 1.0 - p_min)
	...
	if (m > 0) {
		...
		if (!ISZERO(truncation)) {
			loglfun(thread_id, prob_truncation, x, -m, idx, x_vec, &truncation, arg, arg_str);
			for (i = 0; i < m; i++) {
		...
		{
			loglfun(thread_id, log_dens, x, m, idx, x_vec, NULL, arg, arg_str);
			for (i = 0; i < m; i++) {
				logll[i] = log_dens[i] - LOG_1mp(prob_truncation[i]);
			}
		}
			break;
		...
		{
			if (!ISZERO(lower)) {
				loglfun(thread_id, prob_lower, x, -m, idx, x_vec, &lower, arg, arg_str);
				SAFEGUARD(prob_lower, m);
			}
			for (i = 0; i < m; i++) {
				// logll[i] = log(1.0 - (prob_lower[i] - prob_truncation[i]) / (1.0 - prob_truncation[i]));
				logll[i] = LOG_1mp(prob_lower[i]) - LOG_1mp(prob_truncation[i]);
			}
		...
		{
			if (!ISINF(upper)) {
				loglfun(thread_id, prob_upper, x, -m, idx, x_vec, &upper, arg, arg_str);
				SAFEGUARD(prob_upper, m);
			}
			for (i = 0; i < m; i++) {
				// logll[i] = log((prob_upper[i] - prob_truncation[i]) / (1.0 - prob_truncation[i]));
				logll[i] = log(prob_upper[i]) + LOG_1mp(prob_truncation[i] / prob_upper[i]) - LOG_1mp(prob_truncation[i]);
			}
		}
			break;
		...
		case SURV_EVENT_INTERVAL:
		{
			...
			for (i = 0; i < m; i++) {
				...
				logll[i] = log(prob_upper[i]) + LOG_1mp(prob_lower[i] / prob_upper[i]) - LOG_1mp(prob_truncation[i]);
			}
		}
			break;
		...
		case SURV_EVENT_ININTERVAL:
		{
			...
			for (i = 0; i < m; i++) {
				...
				logll[i] = log_dens[i] - (log(prob_upper[i]) + LOG_1mp(prob_lower[i] / prob_upper[i]));
			}
		}
			break;
...
int loglikelihood_weibull(int thread_id, double *logll, double *x, int m, int idx, double *UNUSED(x_vec), double *y_cdf, void *arg, char **arg_str)
{
	/*
	 * y ~ Weibull.
	 */
