	...
	if (jj < 0) {
		...
		if (model->transform != SPDE2_TRANSFORM_IDENTITY) {
			...
			case SPDE2_TRANSFORM_LOGIT:
			{
				for (int kk = 0; kk < 1 + nb; kk++){
					int off = 2 + kk * 3;
					dij[off] = cos(M_PI * map_probability(dij[off], MAP_FORWARD, NULL));
				}
			}
			break;
		...
		double *d_i = dij;
		for (int kk = 0; kk < 1 + nb; kk++){
			int j = (kk == 0 ? ii : model->graph->lnbs[ii][kk-1]);
			// we might store this in another way
			double *vals = (double *) *map_ivp_ptr(&(model->vmatrix->vmat[ii]), j);
			double *v = vals + (kk == 0 ? 3 * nc : 6 * nc);
			double *d_j = dij + kk * 3;

			values[kk] = d_i[0] * d_j[0] * (d_i[1] * d_j[1] * v[0] + d_i[2] * d_i[1] * v[1] + d_j[1] * d_j[2] * v[2] + v[3]);
			//printf("ii %d values[%1d] =  %g  (REF %g)\n", ii, kk, values[kk],inla_spde2_Qfunction(thread_id, ii, j, NULL, arg));
		}
	...
	*smodel = model;
	...
	for (i = 0; i < model->n; i++) {
		...
		for (int jj = 0; jj < model->graph->lnnbs[i]; jj++) {
			...
			// we dod not need to store 'i' for each (i,j)... FIX LATER MAYBE
