...
int inla_read_data_likelihood(inla_tp * mb, dictionary * ini, int sec)
{
	...
	if (ds->data_id == L_GAUSSIAN) {
		...
		idiv = 3;
		a[0] = ds->data_observations.nb = Calloc(mb->predictor_ndata, double);
	} else if (ds->data_id == L_TEST_BINOMIAL_1) {
		idiv = 3;
		a[0] = ds->data_observations.nb = Calloc(mb->predictor_ndata, double);
...
int loglikelihood_skew_normal(double *logll, double *x, int m, int idx, double *x_vec, void *arg)
{
	...
	if (m > 0) {
		for (i = 0; i < m; i++) {
			ypred = PREDICTOR_INVERSE_LINK(x[i] + OFFSET(idx));
			xarg = (y - ypred) * sprec;
			logll[i] = LOG_NORMC_GAUSSIAN + M_LOG2E + 0.5 * (lprec - SQR(xarg)) + inla_log_Phi(shape * xarg);
		}
	} else {
...
#pragma omp threadprivate(psi_old)
	...
	if (m > 0) {
		...
		assert(status == GSL_SUCCESS);
		for (i = 0; i < m; i++) {
			p = PREDICTOR_INVERSE_LINK(LINK_CORRECTION(x[i] + OFFSET(idx), psi) / LINK_CORRECTION_SD(psi));
			if (p > 1.0) {
				/*
				 * need this for the link = "log" that was requested...
				 */
...
int inla_parse_data(inla_tp * mb, dictionary * ini, int sec)
{
	...
	} else if (ds->data_id == L_POISSON || ds->data_id == L_GPOISSON ||
		   ds->data_id == L_ZEROINFLATEDPOISSON0 || ds->data_id == L_ZEROINFLATEDPOISSON1 ||
...
#define SetInitial(id_, val_) mb->f_initial[mb->nf][id_] = val_
	...
	double **log_prec = NULL, **log_prec0 = NULL, **log_prec1 = NULL, **log_prec2, **phi_intern = NULL, **rho_intern = NULL, **group_rho_intern = NULL,
	    ...
	    **mean_x = NULL, **log_prec_x = NULL, ***ar_theta = NULL, ***pacf_intern;
...
#pragma omp critical
	...
	} else if (mb->f_id[mb->nf] == F_R_GENERIC) {
		/*
		 * R_GENERIC
		 */
#if !defined(WINDOWS)
...
#pragma omp critical
		...
		case F_AR:
		{
			...
			val += mb->f_nrep[i] * (ldens * ngroup + normc_g);
...
#pragma omp critical
		...
		case F_BESAG2:
		{
			...
			SET_GROUP_RHO(2);
			// N is 2*graph->n here. 
			val += mb->f_nrep[i] * (normc_g + LOG_NORMC_GAUSSIAN * (mb->f_N[i] / 2.0 - mb->f_rankdef[i])
