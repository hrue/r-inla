 ...
 *
 */
static const char RCSId[] = "$Id: inla.c,v 1.698 2010/04/09 04:10:54 hrue Exp $";
...
double inla_all_offset(int idx, void *arg)
{
	int i, debug = 0;
	double val = 0.0;
	inla_tp *mb = (inla_tp *) arg;

	if (debug)
		printf("call inla_all_offset with idx %d\n", idx);

	if (idx >= mb->predictor_n)
		return 0.0;

	for (i = 0; i < mb->noff; i++) {
		val += mb->off_func[i] (idx, mb->off_func_arg[i]);
	}
...
int inla_make_iid3d_graph(GMRFLib_graph_tp ** graph, inla_iid3d_arg_tp * arg)
{
	...
	for (i = 0; i < n; i++) {
		...
		GMRFLib_ged_add(ged, i, i + 2 * n);
		GMRFLib_ged_add(ged, i + n, i + 2 * n);
	}
	GMRFLib_ged_build(graph, ged);
	GMRFLib_ged_free(ged);
...
{
	/*
	 * this is for the first component only 
	 */
	...
	if (i == j) {
		...
		GMRFLib_ASSERT_RETVAL(i == j, GMRFLib_ESNH, 0.0);
	}
	return 0.0;
}
double Qfunc_2diid_wishart_part11(int i, int j, void *arg)
{
	/*
	 * this is for the second component only 
	 */
...
double Qfunc_2diid_wishart_part01(int i, int j, void *arg)
{
	/*
	 * this is for the cross-terms only, and these are called with i==j!!!! meaning the two components within the same group. 
	 */
...
int inla_make_3diid_graph(GMRFLib_graph_tp ** graph, inla_3diid_arg_tp * arg)
{
	 ...
	 * not in use 
	 */
	FIXME("NOT TESTED AND NOT IN USE.");
	...
	GMRFLib_ged_init(&ged, NULL);
	for (i = 0; i < 3 * arg->n; i += 3) {
		GMRFLib_ged_add(ged, i, i + 1);
		GMRFLib_ged_add(ged, i + 1, i + 2);
		GMRFLib_ged_add(ged, i, i + 2);
	}
	GMRFLib_ged_build(graph, ged);
	GMRFLib_ged_free(ged);
...
double Qfunc_3diid_wishart_part00(int i, int j, void *arg)
{
	// assert(i == j);
	return Qfunc_3diid_wishart_core(0, 0, arg);
}
double Qfunc_3diid_wishart_part11(int i, int j, void *arg)
{
	// assert(i == j);
	return Qfunc_3diid_wishart_core(1, 1, arg);
}
double Qfunc_3diid_wishart_part22(int i, int j, void *arg)
{
	// assert(i == j);
	return Qfunc_3diid_wishart_core(2, 2, arg);
}
double Qfunc_3diid_wishart_part01(int i, int j, void *arg)
{
	// assert(i == j);
	return Qfunc_3diid_wishart_core(0, 1, arg);
}
double Qfunc_3diid_wishart_part02(int i, int j, void *arg)
{
	// assert(i == j);
	return Qfunc_3diid_wishart_core(0, 2, arg);
}
double Qfunc_3diid_wishart_part12(int i, int j, void *arg)
{
	// assert(i == j);
	return Qfunc_3diid_wishart_core(1, 2, arg);
}
double Qfunc_3diid_wishart_core(int node, int nnode, void *arg)
{
	/*
	 * This function returns the ij'th element of thre precision matrix of the three elements in the 3d-vector. The parameterisation is given in the covariance
	 * matrix, so we need to compute the precision matrix. We store Q to avoid to compute it all the time...
	 */
...
#pragma omp threadprivate(hold)
	...
	inla_3diid_arg_tp *a = (inla_3diid_arg_tp *) arg;
	double prec0, prec1, prec2, rho01, rho02, rho12, vec[6];
	int fail;

	if (!hold) {
		hold = Calloc(1, Hold_tp);
	...
	if (memcmp((void *) vec, (void *) hold->vec, 6 * sizeof(double))) {
		...
		fail = inla_wishart3d_adjust(&vec[3]);
		if (fail != GMRFLib_SUCCESS) {
			rho01 = vec[3];
			rho02 = vec[4];
			rho12 = vec[5];
		}
		gsl_matrix_set(hold->Q, 0, 0, 1.0 / prec0);
		gsl_matrix_set(hold->Q, 1, 1, 1.0 / prec1);
		gsl_matrix_set(hold->Q, 2, 2, 1.0 / prec2);
		gsl_matrix_set(hold->Q, 0, 1, rho01 / sqrt(prec0 * prec1));
		gsl_matrix_set(hold->Q, 0, 2, rho02 / sqrt(prec0 * prec2));
		gsl_matrix_set(hold->Q, 1, 2, rho12 / sqrt(prec1 * prec2));
		gsl_matrix_set(hold->Q, 1, 0, gsl_matrix_get(hold->Q, 0, 1));
		gsl_matrix_set(hold->Q, 2, 0, gsl_matrix_get(hold->Q, 0, 2));
		gsl_matrix_set(hold->Q, 2, 1, gsl_matrix_get(hold->Q, 1, 2));

		GMRFLib_gsl_spd_inverse(hold->Q);
	}
	return gsl_matrix_get(hold->Q, node, nnode);
}
double Qfunc_3diid_wishart(int node, int nnode, void *arg)
{
	inla_3diid_arg_tp *a = (inla_3diid_arg_tp *) arg;
	int n = a->n, i, j, ig, jg;
	...
	return Qfunc_3diid_wishart_core(ig, jg, arg);
}
double Qfunc_iid3d(int node, int nnode, void *arg)
{
	/*
	 * This function returns the ij'th element of thre precision matrix of the three elements in the 3d-vector. The parameterisation is given in the covariance
	 * matrix, so we need to compute the precision matrix. We store Q to avoid to compute it all the time...
	...
	inla_iid3d_arg_tp *a = (inla_iid3d_arg_tp *) arg;
	typedef struct {
		double vec[6];
		gsl_matrix *Q;
	} Hold_tp;
...
double priorfunc_wishart2d(double *x, double *parameters)
{
	 ...
	 * Q ~ Wishart(r, R^{-1} )
	 * 
	 * input is x = [ tau1, tau2, rho ] which parameterise Q.
	 * 
	 * prior parameters are p = [ r, R00, R11, R12 ]
	 * 
	...
	Q = gsl_matrix_calloc(2, 2);

	gsl_matrix_set(R, 0, 0, parameters[1]);
	gsl_matrix_set(R, 1, 1, parameters[2]);
	gsl_matrix_set(R, 0, 1, parameters[3]);
	gsl_matrix_set(R, 1, 0, parameters[3]);

	/*
	 * tau1 and tau1 are the *MARGINAL* precisions, which it should be.  The jacobian is computed like this:
	 * 
	 * //with(LinearAlgebra); //with(Student[VectorCalculus]); //S := matrix(2,2, [ 1/tau1, rho/sqrt(tau1*tau2), rho/sqrt(tau1*tau2), 1/tau2] ); //Q :=
	 * inverse(S); //simplify(Determinant(Jacobian([Q[1,1], Q[2,2], Q[1,2]], [tau1, tau2, rho]))); 
	 */
	a = 1.0 - SQR(x[2]);				       /* a = 1 - rho^2 */
	t = sqrt(x[0] * x[1]);				       /* t = sqrt(tau1*tau2) */
	gsl_matrix_set(Q, 0, 0, x[0] / a);
	gsl_matrix_set(Q, 1, 1, x[1] / a);
...
{
	/*
	 * return GMRFLib_SUCCESS is Covariance matrix is pos.def, otherwise return !GMRFLib_SUCCESS and adjust coefficients to it is 
	 */
	int debug = 0, fail = 0;
	double f = 0.99;

	while (1.0 - SQR(rho[0]) - SQR(rho[1]) - SQR(rho[2]) + 2.0 * rho[0] * rho[1] * rho[2] < 0.0) {
		rho[0] *= f;
		rho[1] *= f;
		rho[2] *= f;
		fail++;
	...
	if (fail && debug) {
		printf("%s: Had to adjust %d times, f = %g\n", __GMRFLib_FuncName, fail, pow(f, (double) fail));
	}

	return (fail ? !GMRFLib_SUCCESS : GMRFLib_SUCCESS);
}
...
double priorfunc_wishart3d(double *x, double *parameters)
{
	...
	fail = inla_wishart3d_adjust(&x[3]);

	gsl_matrix_set(R, 0, 0, parameters[1]);
	gsl_matrix_set(R, 1, 1, parameters[2]);
	gsl_matrix_set(R, 2, 2, parameters[3]);
	gsl_matrix_set(R, 0, 1, parameters[4]);
	gsl_matrix_set(R, 1, 0, parameters[4]);
	gsl_matrix_set(R, 0, 2, parameters[5]);
	gsl_matrix_set(R, 2, 0, parameters[5]);
	gsl_matrix_set(R, 1, 2, parameters[6]);
	gsl_matrix_set(R, 2, 1, parameters[6]);

#define COMPUTE_Q if (1) {						\
		gsl_matrix_set(Q, 0, 0, 1.0/x[0]);			\
		gsl_matrix_set(Q, 1, 1, 1.0/x[1]);			\
		gsl_matrix_set(Q, 2, 2, 1.0/x[2]);			\
		gsl_matrix_set(Q, 0, 1, x[3]/sqrt(x[0]*x[1]));		\
		gsl_matrix_set(Q, 0, 2, x[4]/sqrt(x[0]*x[2]));		\
		gsl_matrix_set(Q, 1, 2, x[5]/sqrt(x[1]*x[2]));		\
		gsl_matrix_set(Q, 1, 0, gsl_matrix_get(Q, 0, 1));	\
		gsl_matrix_set(Q, 2, 0, gsl_matrix_get(Q, 0, 2));	\
		gsl_matrix_set(Q, 2, 1, gsl_matrix_get(Q, 1, 2));	\
		if (debug) printf("Covmatrix:\n");			\
		if (debug) GMRFLib_gsl_matrix_fprintf(stdout, Q, NULL);	\
	 ...
	 */
	size_t i, n = 6;
	gsl_matrix *Qref = NULL, *J = NULL;
	double f, save;

	/*
	 * for the numerical derivatives: compute the `population' variance: Det(Sigma), and set f = (Det(Sigma))^1/6. 
	 */
	f = 1.0e-4 * pow(exp(-GMRFLib_gsl_spd_logdet(Q)), 1.0 / 6.0);	/* Yes, its a minus... */

	Qref = GMRFLib_gsl_duplicate_matrix(Q);
	...
	for (i = 0; i < n; i++) {
		...
		gsl_matrix_set(J, i, 0, (gsl_matrix_get(Q, 0, 0) - gsl_matrix_get(Qref, 0, 0)) / f);
		gsl_matrix_set(J, i, 1, (gsl_matrix_get(Q, 1, 1) - gsl_matrix_get(Qref, 1, 1)) / f);
		gsl_matrix_set(J, i, 2, (gsl_matrix_get(Q, 2, 2) - gsl_matrix_get(Qref, 2, 2)) / f);
		gsl_matrix_set(J, i, 3, (gsl_matrix_get(Q, 0, 1) - gsl_matrix_get(Qref, 0, 1)) / f);
		gsl_matrix_set(J, i, 4, (gsl_matrix_get(Q, 0, 2) - gsl_matrix_get(Qref, 0, 2)) / f);
		gsl_matrix_set(J, i, 5, (gsl_matrix_get(Q, 1, 2) - gsl_matrix_get(Qref, 1, 2)) / f);
	}
...
int inla_read_data_likelihood(inla_tp * mb, dictionary * ini, int sec)
{
	...
	if (ds->data_id == L_GAUSSIAN) {
		idiv = 3;
		a[0] = ds->data_observations.weight_gaussian = Calloc(mb->predictor_n, double);
	} else if (ds->data_id == L_LAPLACE) {
		idiv = 3;
		a[0] = ds->data_observations.weight_laplace = Calloc(mb->predictor_n, double);
	} else if (ds->data_id == L_T) {
		idiv = 3;
		a[0] = ds->data_observations.weight_t = Calloc(mb->predictor_n, double);
		...
		idiv = 6;
		a[0] = ds->data_observations.event = Calloc(mb->predictor_n, double);	/* the failure code */
		a[1] = ds->data_observations.truncation = Calloc(mb->predictor_n, double);
		a[2] = ds->data_observations.lower = Calloc(mb->predictor_n, double);
		a[3] = ds->data_observations.upper = Calloc(mb->predictor_n, double);
	} else {
		assert(0 == 1);
	}
	na = idiv - 2;
	...
	ds->data_observations.y = Calloc(mb->predictor_n, double);
	ds->data_observations.d = Calloc(mb->predictor_n, double);

	for (i = j = 0; i < n; i += idiv, j++) {
		ii = (int) x[i];
		if (!LEGAL(ii, mb->predictor_n)) {
...
int loglikelihood_laplace(double *logll, double *x, int m, int idx, double *x_vec, void *arg)
{
	...
	double y, tau, ltau, w, alpha, u, lnc, epsilon, ggamma, a;
...
int loglikelihood_gaussian(double *logll, double *x, int m, int idx, double *x_vec, void *arg)
{
	...
	if (m > 0) {
		if (m <= 3) {
			logll[0] = -0.9189385332046726 + 0.5 * (lprec - SQR((x[0] + OFFSET(idx)) - y) * prec);
			if (m > 1) {
				logll[1] = -((x[1] + OFFSET(idx)) - y) * prec;
			}
			if (m > 2) {
				logll[2] = -prec;
			}
			...
			for (i = 0; i < m; i++) {
				logll[i] = -0.9189385332046726 + 0.5 * (lprec - SQR((x[i] + OFFSET(idx)) - y) * prec);
			}
		}
	} else {
		for (i = 0; i < -m; i++) {
...
int loglikelihood_t(double *logll, double *x, int m, int idx, double *x_vec, void *arg)
{
	...
	if (m > 0) {
		...
		for (i = 0; i < -m; i++) {
			logll[i] = gsl_cdf_tdist_P((y - (x[i] + OFFSET(idx))) * fac, dof);
		}
	}
	return GMRFLib_SUCCESS;
}
...
int loglikelihood_logperiodogram(double *logll, double *x, int m, int idx, double *x_vec, void *arg)
{
	...
	if (exact) {
		...
		double y = ds->data_observations.y[idx], v;

		if (m > 0) {
			if (m <= 3) {
				v = y - (x[0] + OFFSET(idx)) + M_LOG2E;
				logll[0] = -M_LOG2E + v - 0.5 * exp(v);
				if (m > 1) {
					v = y - (x[1] + OFFSET(idx)) + M_LOG2E;
					logll[1] = -1.0 + 0.5 * exp(v);
				}
				if (m > 2) {
					v = y - (x[2] + OFFSET(idx)) + M_LOG2E;
					logll[2] = -0.5 * exp(v);
				}
			} else {
				for (i = 0; i < m; i++) {
					v = y - (x[i] + OFFSET(idx)) + M_LOG2E;
					logll[i] = -M_LOG2E + v - 0.5 * exp(v);
				}
			}
		}
	} else {
		...
		if (m > 0) {
			v = y - (x[0] + OFFSET(idx)) + M_LOG2E;
			logll[0] = -M_LOG2E + v - 0.5 * exp_taylor(v, x0, order);
			if (m > 1) {
				v = y - (x[1] + OFFSET(idx)) + M_LOG2E;
				logll[1] = -1.0 + 0.5 * dexp_taylor(v, x0, order);
			}
			if (m > 2) {
				v = y - (x[2] + OFFSET(idx)) + M_LOG2E;
				logll[2] = -0.5 * ddexp_taylor(v, x0, order);
			}
		}
	}
	return GMRFLib_SUCCESS;
	...
	if (m > 0) {
		...
		for (i = 0; i < m; i++) {
			...
			if (mu / size > cutoff) {
				 ...
				 * * the Poission limit 
				 */
				logll[i] = y * ((x[i] + OFFSET(idx)) + log(E)) - E * exp(x[i] + OFFSET(idx)) - gsl_sf_lnfact((unsigned int) y);
			}
		}
	} else {
		for (i = 0; i < -m; i++) {
...
{
	/*
	 * y ~ Binomial(n, p), where logit(p) = x. 
	 */
	int i;
	...
	int status;
	Data_section_tp *ds = (Data_section_tp *) arg;
	double y = ds->data_observations.y[idx], n = ds->data_observations.nb[idx], t1, t4;
	...
	if (m > 0) {
		...
		if (m <= 3) {
			logll[0] = res.val + y * (x[0] + OFFSET(idx)) - n * log(1.0 + exp((x[0] + OFFSET(idx))));
			/*
			 * these derivaties are spesific for the logit-link!!! 
			 */
			if (m > 1) {
				t1 = exp((x[1] + OFFSET(idx)));
				logll[1] = (y * t1 - n * t1 + y) / (0.1e1 + t1);
			}
			if (m > 2) {
				t1 = exp((x[2] + OFFSET(idx)));
				t4 = SQR(0.1e1 + t1);
				logll[2] = -n * t1 / t4;
			}
		} else {
			for (i = 0; i < m; i++) {
				logll[i] = res.val + y * (x[i] + OFFSET(idx)) - n * log(1.0 + exp((x[i] + OFFSET(idx))));
			}
		}
	} else {
		for (i = 0; i < -m; i++) {
...
int loglikelihood_exp(double *logll, double *x, int m, int idx, double *x_vec, void *arg)
{
	...
	Data_section_tp *ds = (Data_section_tp *) arg;
	int i, ievent;
	double y, event, truncation, lower, upper, gama, loggama;
	...
	if (m > 0) {
		...
		case SURV_EVENT_FAILURE:
			for (i = 0; i < m; i++) {
				loggama = x[i] + OFFSET(idx);
				gama = exp(loggama);
				logll[i] = loggama - gama * (y - truncation);
			}
			break;
...
int loglikelihood_weibull(double *logll, double *x, int m, int idx, double *x_vec, void *arg)
{
	...
	Data_section_tp *ds = (Data_section_tp *) arg;
	int i, ievent;
	double y, event, truncation, lower, upper, alpha, gama, loggama, ypow, lowerpow, upperpow, truncationpow;
	...
	if (m > 0) {
		...
		case SURV_EVENT_FAILURE:
			...
			for (i = 0; i < m; i++) {
				loggama = x[i] + OFFSET(idx);
				gama = exp(loggama);
				logll[i] = loggama + log(alpha) + (alpha - 1.0) * log(y) - gama * (ypow - truncationpow);
			}
			break;
		case SURV_EVENT_RIGHT:
...
int loglikelihood_weibull_cure(double *logll, double *x, int m, int idx, double *x_vec, void *arg)
{
	...
	Data_section_tp *ds = (Data_section_tp *) arg;
	int i, ievent;
	double y, event, truncation, lower, upper, alpha, gama, loggama, ypow, lowerpow, upperpow, truncationpow, p;
	...
	if (m > 0) {
		...
		case SURV_EVENT_FAILURE:
			...
			for (i = 0; i < m; i++) {
				loggama = x[i] + OFFSET(idx);
				gama = exp(loggama);
				logll[i] = log(1.0 - p) + loggama + log(alpha) + (alpha - 1.0) * log(y) - gama * (ypow - truncationpow);
			}
			break;
		case SURV_EVENT_RIGHT:
			lowerpow = pow(lower, alpha);
...
int loglikelihood_stochvol(double *logll, double *x, int m, int idx, double *x_vec, void *arg)
{
	...
	Data_section_tp *ds = (Data_section_tp *) arg;
	double y = ds->data_observations.y[idx], t1, t2, t4;

	if (m > 0) {
		if (m <= 3) {
			logll[0] = -0.9189385332046726 - 0.5 * (x[0] + OFFSET(idx)) - 0.5 * SQR(y) / exp((x[0] + OFFSET(idx)));
			if (m > 1) {
				t1 = y * y;
				t2 = exp((x[1] + OFFSET(idx)));
				t4 = 1.0 / t2;
				logll[1] = (t1 - t2) * t4 / 0.2e1;
			}
			if (m > 2) {
				t1 = y * y;
				t2 = exp(-(x[2] + OFFSET(idx)));
				logll[2] = -t1 * t2 / 0.2e1;
			}
		} else {
			for (i = 0; i < m; i++) {
				logll[i] = -0.9189385332046726 - 0.5 * (x[i] + OFFSET(idx)) - 0.5 * SQR(y) / exp((x[i] + OFFSET(idx)));
			}
		}
	} else {
		for (i = 0; i < -m; i++) {
			logll[i] = 1.0 - 2.0 * gsl_cdf_ugaussian_Q(ABS(y) / exp((x[i] + OFFSET(idx)) / 2.0));
		}
	}
	return GMRFLib_SUCCESS;
}
int loglikelihood_stochvol_t(double *logll, double *x, int m, int idx, double *x_vec, void *arg)
{
	...
	if (m > 0) {
		...
		lg2 = gsl_sf_lngamma((dof + 1.0) / 2.0);
		if (m <= 3) {
			f = exp((x[0] + OFFSET(idx)) / 2.0) / sd;
			obs = y / f;
			logll[0] = lg2 - lg1 - 0.5 * log(M_PI * dof) - (dof + 1.0) / 2.0 * log(1.0 + SQR(obs) / dof) - log(f);
			if (m > 1) {
				logll[1] = 0.5 * (dof + 1.0) * y2 / (exp((x[1] + OFFSET(idx))) * isd2 * dof + y2) - 0.5;
			}
			if (m > 2) {
				logll[2] = -0.5 * (dof + 1.0) * y2 * exp((x[2] + OFFSET(idx))) * isd2 * dof / (SQR(exp((x[2] + OFFSET(idx))) * isd2 * dof + y2));
			}
		} else {
			for (i = 0; i < m; i++) {
				f = exp((x[i] + OFFSET(idx)) / 2.0) / sd;
				obs = y / f;
		...
		for (i = 0; i < -m; i++) {
			logll[i] = 1.0 - 2.0 * gsl_cdf_tdist_Q(ABS(y) * sd / exp((x[i] + OFFSET(idx)) / 2.0), dof);
		}
	}
	return GMRFLib_SUCCESS;
}
int loglikelihood_stochvol_nig(double *logll, double *x, int m, int idx, double *x_vec, void *arg)
{
	...
	int i;
	Data_section_tp *ds = (Data_section_tp *) arg;
	double skew, skew2, shape, shape2, y, gam, gam2, tmp, obs, a;
	...
	if (m > 0) {
		for (i = 0; i < m; i++) {
			obs = y / exp((x[i] + OFFSET(idx)) / 2.0);
			tmp = SQR(gam * obs + skew) + shape2;
			logll[i] = a - 0.5 * log(tmp) + skew * (gam * obs + skew)
			    + gsl_sf_bessel_lnKnu(1.0, sqrt((skew2 + shape2) * tmp)) - (x[i] + OFFSET(idx)) / 2.0;
		}
	}
	return GMRFLib_SUCCESS;
}
...
{
	return inla_read_prior_generic(mb, ini, sec, prior, "PRIOR", "PARAMETERS", default_prior);
}
int inla_read_prior_group(inla_tp * mb, dictionary * ini, int sec, Prior_tp * prior, const char *default_prior)
{
	return inla_read_prior_generic(mb, ini, sec, prior, "GROUP.PRIOR", "GROUP.PARAMETERS", default_prior);
}
int inla_read_prior0(inla_tp * mb, dictionary * ini, int sec, Prior_tp * prior, const char *default_prior)
{
	return inla_read_prior_generic(mb, ini, sec, prior, "PRIOR0", "PARAMETERS0", default_prior);
}
int inla_read_prior1(inla_tp * mb, dictionary * ini, int sec, Prior_tp * prior, const char *default_prior)
{
	return inla_read_prior_generic(mb, ini, sec, prior, "PRIOR1", "PARAMETERS1", default_prior);
}
int inla_read_prior2(inla_tp * mb, dictionary * ini, int sec, Prior_tp * prior, const char *default_prior)
{
	return inla_read_prior_generic(mb, ini, sec, prior, "PRIOR2", "PARAMETERS2", default_prior);
}
int inla_read_prior3(inla_tp * mb, dictionary * ini, int sec, Prior_tp * prior, const char *default_prior)
{
	return inla_read_prior_generic(mb, ini, sec, prior, "PRIOR3", "PARAMETERS3", default_prior);
}
...
inla_tp *inla_build(const char *dict_filename, int verbose, int make_dir)
{
	...
	for (sec = 0; sec < nsec; sec++) {
		secname = GMRFLib_strdup(iniparser_getsecname(ini, sec));
		sectype = GMRFLib_strdup(strupc(iniparser_getstring(ini, inla_string_join((const char *) secname, "TYPE"), NULL)));
		if (!strcmp(sectype, "OFFSET")) {
			if (mb->verbose) {
				printf("\tsection=[%1d] name=[%s] type=[OFFSET]\n", sec, iniparser_getsecname(ini, sec));
			}
			found++;
			sec_read[sec] = 1;
			inla_parse_offset(mb, ini, sec);
		}
		Free(secname);
		Free(sectype);
	}
	 ...
	 */
	mb->loglikelihood = Calloc(mb->predictor_n, GMRFLib_logl_tp *);
	mb->loglikelihood_arg = Calloc(mb->predictor_n, void *);
	mb->d = Calloc(mb->predictor_n, double);
...
int inla_parse_predictor(inla_tp * mb, dictionary * ini, int sec)
{
	...
	char *tmp_n = GMRFLib_strdup(iniparser_getstring(ini, inla_string_join(secname, "N"), NULL));

	if (!tmp_n) {
		inla_error_missing_required_field(__GMRFLib_FuncName, secname, "n");
	} else {
		if (inla_sread_ints(&(mb->predictor_n), 1, tmp_n) == INLA_FAIL) {
			inla_error_field_is_void(__GMRFLib_FuncName, secname, "n", tmp_n);
		}
		if (mb->predictor_n <= 0) {
			inla_error_field_is_void(__GMRFLib_FuncName, secname, "n", tmp_n);
		}
...
#define SetInitial(id_, val_) mb->f_initial[mb->nf][id_] = val_
	...
	double **log_prec = NULL, **log_prec0 = NULL, **log_prec1 = NULL, **log_prec2, **phi_intern = NULL, **rho_intern = NULL, **group_rho_intern = NULL,
	    ...
	    **log_positive = NULL, **h2_intern = NULL, **sphere_oc = NULL;
	...
	case F_IID1D:
		...
		if (mb->f_prior[mb->nf][0].id != P_LOGGAMMA) { /* This is what it should be, yes! */
			char *m;
			GMRFLib_sprintf(&m, "Model=[IID1D] needs prior=[WISHART1D], you have prior=[%s]", mb->f_prior[mb->nf]->name);
			inla_error_general(m);
			exit(1);
		}
		break;
	...
	if ((mb->f_id[mb->nf] == F_MATERN2D) || (mb->f_id[mb->nf] == F_RW2D)) {
		...
		if (mb->f_id[mb->nf] == F_GENERIC0) {
			...
			Free(ptmp);
			mb->f_N[mb->nf] = mb->f_n[mb->nf] = n;
		} else if (mb->f_id[mb->nf] == F_IID2D) {
			/*
			 * IID2D-model; need length N
			 */
			ptmp = GMRFLib_strdup(iniparser_getstring(ini, inla_string_join(secname, "N"), NULL));
			if (!ptmp) {
				inla_error_missing_required_field(__GMRFLib_FuncName, secname, "N");
			...
			if (inla_divisible(n, 2) == GMRFLib_FALSE) {
				GMRFLib_sprintf(&msg, "%s: N=%1d is not divisible by 2", secname, n);
				inla_error_general(msg);
				exit(1);
			}
			if (mb->verbose) {
			...
			Free(ptmp);
			mb->f_N[mb->nf] = mb->f_n[mb->nf] = n;
		} else if (mb->f_id[mb->nf] == F_SPHERE) {
			/*
			 * SPHERE 
			 */
	...
	case F_SPHERE:
	{
		...
		sphere_dir = GMRFLib_strdup(".");
		sphere_dir = iniparser_getstring(ini, inla_string_join(secname, "SPHERE_DIR"), sphere_dir);
		sphere_dir = iniparser_getstring(ini, inla_string_join(secname, "SPHERE.DIR"), sphere_dir);
		sphere_dir = iniparser_getstring(ini, inla_string_join(secname, "SPHEREDIR"), sphere_dir);
		if (mb->verbose) {
			printf("\t\tsphere.dir = [%s]\n", sphere_dir);
		}

		sph_basis_model_tp modelT, modelK;
		int nT, nK;
		 ...
		 * 
		 */
		inla_sphere_build_model(&sphere_model_orig, (const char *) sphere_dir, &modelT, &modelK);
		mb->f_model[mb->nf] = (void *) sphere_model_orig;

		/*
		 * The _userfunc0 must be set directly after the _build_model() call. This is a bit dirty; FIXME later. 
		 */
		inla_sphere_build_model(&sphere_model, (const char *) sphere_dir, &modelT, &modelK);
		GMRFLib_ai_INLA_userfunc0 = (GMRFLib_ai_INLA_userfunc0_tp *) inla_sphere_userfunc0;
		GMRFLib_ai_INLA_userfunc1 = (GMRFLib_ai_INLA_userfunc1_tp *) inla_sphere_userfunc1;
		...
		if (modelT.order < 0 && modelK.order < 0) {
			...
			nT = sph_basis_n(&modelT);
			nK = sph_basis_n(&modelK);
			...
			for (k = 0; k < nT; k++) {
				...
				HYPER_INIT(sphere_model->Tmodel->theta[k], tmp);
			}
			for (k = 0; k < nK; k++) {
				if (k == 0) {
				...
				HYPER_INIT(sphere_model->Kmodel->theta[k], tmp);
			}
			...
			HYPER_INIT(sphere_model->oc, tmp);

			mb->f_theta[mb->nf] = Calloc(nT + nK + 1, double **);
			for (k = 0; k < nT; k++)
				mb->f_theta[mb->nf][k] = sphere_model->Tmodel->theta[k];
			for (k = 0; k < nK; k++)
				mb->f_theta[mb->nf][k + nT] = sphere_model->Kmodel->theta[k];
			mb->f_theta[mb->nf][nK + nT] = sphere_model->oc;
			...
			for (k = 0; k < nT + nK + 1; k++) {
				...
				if (!fx) {
					...
					if (k == nT + nK) {
						mb->theta[mb->ntheta] = sphere_model->oc;
					} else {
						if (k < nT) {
							mb->theta[mb->ntheta] = sphere_model->Tmodel->theta[k];
						} else {
							mb->theta[mb->ntheta] = sphere_model->Kmodel->theta[k - nT];
						}
					}
	...
	case F_3DIIDWISHARTPART0:
	{
		...
		mb->f_theta[mb->nf][2] = log_prec2;
		if (!mb->f_fixed[mb->nf][2]) {
			mb->theta = Realloc(mb->theta, mb->ntheta + 1, double **);
			mb->theta_tag = Realloc(mb->theta_tag, mb->ntheta + 1, char *);
			mb->theta_tag_userscale = Realloc(mb->theta_tag_userscale, mb->ntheta + 1, char *);
			mb->theta_dir = Realloc(mb->theta_dir, mb->ntheta + 1, char *);
			GMRFLib_sprintf(&msg, "Log-precision for %s (third component)", mb->f_tag[mb->nf]);
			mb->theta_tag[mb->ntheta] = msg;
			GMRFLib_sprintf(&msg, "Precision for %s (third component)", mb->f_tag[mb->nf]);
			mb->theta_tag_userscale[mb->ntheta] = msg;
			GMRFLib_sprintf(&msg, "%s-parameter2", mb->f_dir[mb->nf]);
			mb->theta_dir[mb->ntheta] = msg;
		...
		tmp = iniparser_getdouble(ini, inla_string_join(secname, "INITIAL3"), 0.0);
		if (!mb->f_fixed[mb->nf][3] && mb->reuse_mode) {
			tmp = mb->theta_file[mb->theta_counter_file++];
		}
		SetInitial(3, tmp);
		HYPER_INIT(rho_intern01, tmp);
		if (mb->verbose) {
		...
		if (!mb->f_fixed[mb->nf][5] && mb->reuse_mode) {
			tmp = mb->theta_file[mb->theta_counter_file++];
		}
		SetInitial(5, tmp);
		HYPER_INIT(rho_intern12, tmp);
		if (mb->verbose) {
		...
		if (!mb->f_fixed[mb->nf][3]) {
			...
			mb->theta_tag_userscale = Realloc(mb->theta_tag_userscale, mb->ntheta + 1, char *);
			mb->theta_dir = Realloc(mb->theta_dir, mb->ntheta + 1, char *);
			GMRFLib_sprintf(&msg, "Rho_intern01 for %s", mb->f_tag[mb->nf]);
			mb->theta_tag[mb->ntheta] = msg;
			GMRFLib_sprintf(&msg, "Rho01 for %s", mb->f_tag[mb->nf]);
			mb->theta_tag_userscale[mb->ntheta] = msg;
		...
		mb->f_theta[mb->nf][4] = rho_intern02;
		if (!mb->f_fixed[mb->nf][4]) {
			mb->theta = Realloc(mb->theta, mb->ntheta + 1, double **);
			mb->theta_tag = Realloc(mb->theta_tag, mb->ntheta + 1, char *);
			mb->theta_tag_userscale = Realloc(mb->theta_tag_userscale, mb->ntheta + 1, char *);
			mb->theta_dir = Realloc(mb->theta_dir, mb->ntheta + 1, char *);
			GMRFLib_sprintf(&msg, "Rho_intern02 for %s", mb->f_tag[mb->nf]);
			mb->theta_tag[mb->ntheta] = msg;
			GMRFLib_sprintf(&msg, "Rho02 for %s", mb->f_tag[mb->nf]);
			mb->theta_tag_userscale[mb->ntheta] = msg;
			GMRFLib_sprintf(&msg, "%s-parameter4", mb->f_dir[mb->nf]);
			mb->theta_dir[mb->ntheta] = msg;
		...
		mb->f_theta[mb->nf][5] = rho_intern12;
		if (!mb->f_fixed[mb->nf][3]) {
			mb->theta = Realloc(mb->theta, mb->ntheta + 1, double **);
			mb->theta_tag = Realloc(mb->theta_tag, mb->ntheta + 1, char *);
			mb->theta_tag_userscale = Realloc(mb->theta_tag_userscale, mb->ntheta + 1, char *);
			mb->theta_dir = Realloc(mb->theta_dir, mb->ntheta + 1, char *);
			GMRFLib_sprintf(&msg, "Rho_intern12 for %s", mb->f_tag[mb->nf]);
			mb->theta_tag[mb->ntheta] = msg;
			GMRFLib_sprintf(&msg, "Rho12 for %s", mb->f_tag[mb->nf]);
			mb->theta_tag_userscale[mb->ntheta] = msg;
			GMRFLib_sprintf(&msg, "%s-parameter5", mb->f_dir[mb->nf]);
			mb->theta_dir[mb->ntheta] = msg;
	...
	if (mb->f_id[mb->nf] == F_GENERIC0) {
		...
		mb->f_Qfunc[mb->nf] = sphere_model->Qfunc;
		mb->f_Qfunc_arg[mb->nf] = sphere_model->Qfunc_arg;
		mb->f_graph[mb->nf] = sphere_model->graph;

		if (0) {
			FILE *fp;
			fp = fopen("sphere.graph.dat", "w");
			GMRFLib_print_graph(fp, sphere_model->graph);
			fclose(fp);
			FIXME("write graph");
			exit(0);
		}
		...
		inla_iid2d_arg_tp *arg = NULL;

		assert(mb->f_n[mb->nf] == mb->f_N[mb->nf]);
		arg = Calloc(1, inla_iid2d_arg_tp);
		arg->n = mb->f_n[mb->nf] / 2;
		arg->N = mb->f_N[mb->nf];
		...
		assert(mb->f_N[mb->nf] == mb->f_n[mb->nf]);
		arg = Calloc(1, inla_iid3d_arg_tp);
		arg->n = mb->f_n[mb->nf] / 3;		       /* yes */
		arg->N = mb->f_N[mb->nf];
		mb->f_Qfunc_arg[mb->nf] = (void *) arg;
		mb->f_rankdef[mb->nf] = 0;
		arg->log_prec0 = log_prec0;
		...
		arg->rho_intern02 = rho_intern02;
		arg->rho_intern12 = rho_intern12;
		mb->f_Qfunc[mb->nf] = Qfunc_iid3d;
		mb->f_Qfunc_arg[mb->nf] = (void *) arg;
		inla_make_iid3d_graph(&(mb->f_graph[mb->nf]), arg);
	} else if (mb->f_id[mb->nf] == F_SEASONAL) {
...
{
	inla_off_func_arg_tp *a = (inla_off_func_arg_tp *) arg;
	double beta = exp(a->log_positive[GMRFLib_thread_id][0]);

	// printf("beta %g weight %g = %g\n", beta, a->weights[idx], beta * a->weights[idx]);
	return beta * a->weights[idx];
}
int inla_parse_offset(inla_tp * mb, dictionary * ini, int sec)
{
	/*
	 * parse section = OFFSET (work in progress...)
	 */
	int i;
	char *filename = NULL, *secname = NULL, default_tag[100], *model = NULL, *msg = NULL;
	map_table_tp *um;
	double **log_positive, tmp;

	if (mb->verbose) {
		printf("\tinla_parse_offset...\n");
	}
	secname = GMRFLib_strdup(iniparser_getsecname(ini, sec));
	if (mb->verbose) {
		printf("\t\tsection[%s]\n", secname);
	}
	mb->off_prior = Realloc(mb->off_prior, mb->noff + 1, Prior_tp *);
	mb->off_tag = Realloc(mb->off_tag, mb->noff + 1, char *);
	mb->off_dir = Realloc(mb->off_dir, mb->noff + 1, char *);
	mb->off_weights = Realloc(mb->off_weights, mb->noff + 1, double *);
	mb->off_output = Realloc(mb->off_output, mb->noff + 1, Output_tp *);
	mb->off_usermap = Realloc(mb->off_usermap, mb->noff + 1, map_table_tp *);
	mb->off_func = Realloc(mb->off_func, mb->noff + 1, inla_off_func_tp *);
	mb->off_func_arg = Realloc(mb->off_func_arg, mb->noff + 1, void *);
	mb->off_theta = Realloc(mb->off_theta, mb->noff + 1, double ***);
	mb->off_fixed = Realloc(mb->off_fixed, mb->noff + 1, int *);
	mb->off_ntheta = Realloc(mb->off_ntheta, mb->noff + 1, int);
	mb->off_modelname = Realloc(mb->off_modelname, mb->noff + 1, char *);
	mb->off_id = Realloc(mb->off_id, mb->noff + 1, int);

	HYPER_NEW(log_positive, 0.0);

	sprintf(default_tag, "default tag for off %d", (int) (10000 * GMRFLib_uniform()));


	mb->off_tag[mb->noff] = GMRFLib_strdup((secname ? secname : default_tag));
	mb->off_dir[mb->noff] = GMRFLib_strdup(iniparser_getstring(ini, inla_string_join(secname, "DIR"), inla_fnmfix(GMRFLib_strdup(mb->off_tag[mb->noff]))));
	if (mb->verbose) {
		printf("\t\tdir=[%s]\n", mb->off_dir[mb->noff]);
	}
	...
	filename = GMRFLib_strdup(iniparser_getstring(ini, inla_string_join(secname, "WEIGHTS"), NULL));
	if (!filename) {
		if (mb->verbose) {
			printf("\t\tfile for weights=[(null)]: set all weights to 1\n");
		}
		mb->off_weights[mb->noff] = Calloc(mb->predictor_n, double);

		for (i = 0; i < mb->predictor_n; i++) {
			mb->off_weights[mb->noff][i] = 1.0;
		}
	} else {
		if (mb->verbose) {
			printf("\t\tfile for weights=[%s]\n", filename);
		}
		inla_read_data_general(&(mb->off_weights[mb->noff]), NULL, NULL, filename, mb->predictor_n, 0, 1, mb->verbose, -1.0);
	}


	if (mb->off_id[mb->noff] == O_POSITIVE) {

		if (mb->off_ntheta[mb->noff]) {
			mb->off_prior[mb->noff] = Calloc(mb->off_ntheta[mb->noff], Prior_tp);
			mb->off_fixed[mb->noff] = Calloc(mb->off_ntheta[mb->noff], int);
			mb->off_theta[mb->noff] = Calloc(mb->off_ntheta[mb->noff], double **);
		} else {
			mb->off_prior[mb->noff] = NULL;
			mb->off_fixed[mb->noff] = NULL;
			mb->off_theta[mb->noff] = NULL;
		}

		mb->off_usermap[mb->noff] = um = mapfunc_find(iniparser_getstring(ini, inla_string_join(secname, "OFFSET.USERMAP"), NULL));
		if (mb->verbose && um) {
			printf("\t\toff.usermap=[%s]\n", um->name);
		}

		tmp = iniparser_getdouble(ini, inla_string_join(secname, "INITIAL"), 0.0);
		if (!mb->off_fixed[mb->noff][0] && mb->reuse_mode) {
			tmp = mb->theta_file[mb->theta_counter_file++];
		}
		if (mb->verbose) {
			printf("\t\tinitialise log_positive[%g]\n", tmp);
			printf("\t\tfixed=[%1d]\n", mb->off_fixed[mb->noff][0]);
		}
		HYPER_INIT(log_positive, tmp);
		...
		mb->off_theta[mb->noff][0] = log_positive;
		mb->off_fixed[mb->noff][0] = iniparser_getboolean(ini, inla_string_join(secname, "FIXED"), 0);

		if (!mb->off_fixed[mb->noff][0]) {
			/*
			 * add this \theta 
			 */
			mb->theta = Realloc(mb->theta, mb->ntheta + 1, double **);
			...
			mb->theta_tag_userscale = Realloc(mb->theta_tag_userscale, mb->ntheta + 1, char *);
			mb->theta_dir = Realloc(mb->theta_dir, mb->ntheta + 1, char *);
			GMRFLib_sprintf(&msg, "Log positive fixed-effect for %s", (secname ? secname : mb->off_tag[mb->noff]));
			mb->theta_tag[mb->ntheta] = msg;
			GMRFLib_sprintf(&msg, "Positive fixed-effect for %s", (secname ? secname : mb->off_tag[mb->noff]));
			mb->theta_tag_userscale[mb->ntheta] = msg;
			GMRFLib_sprintf(&msg, "%s-parameter", mb->off_dir[mb->noff]);
			mb->theta_dir[mb->ntheta] = msg;
		...
		inla_off_func_arg_tp *def;

		def = Calloc(1, inla_off_func_arg_tp);
		def->log_positive = log_positive;
		def->weights = mb->off_weights[mb->noff];
		def->n = mb->predictor_n;

		mb->off_func[mb->noff] = offset_positive_func;
		mb->off_func_arg[mb->noff] = (void *) def;
	}

	switch (mb->off_id[mb->noff]) {
	case O_POSITIVE:
		inla_read_prior(mb, ini, sec, &(mb->off_prior[mb->noff][0]), "LOGGAMMA");
		break;
	default:
		assert(0 == 1);
	}

	inla_parse_output(mb, ini, sec, &(mb->off_output[mb->noff]));

	G.all_offset = inla_all_offset;
...
double extra(double *theta, int ntheta, void *argument)
{
	...
	double val = 0.0, log_precision, log_precision0, log_precision1, log_precision2, precision0, precision1, precision2,
	    ...
	    h2_intern, phi, phi_intern, n = NAN, normc = -0.9189385332046729, dof_intern, logdet, tvec[6] = { 0, 0, 0, 0, 0, 0 }, log_positive;
...
#pragma omp threadprivate(problem)
			...
			case F_AR1:
			{
				...
				if (mb->f_cyclic[i]) {
					...
					val += mb->f_nrep[i] * (normc_g + normc * (n - mb->f_rankdef[i])
								+ (n - mb->f_rankdef[i]) / 2.0 * log_precision + ngroup * 0.5 * log(1.0 - SQR(phi)));
				}
				if (!mb->f_fixed[i][0]) {
					val += mb->f_prior[i][0].priorfunc(&log_precision, mb->f_prior[i][0].parameters);
				}
...
#pragma omp threadprivate(hold)
	...
	for (i = 0; i < mb->noff; i++) {
		...
		case O_POSITIVE:
		{
			assert(mb->off_ntheta[i] == 1);
			if (!mb->off_fixed[i][0]) {
				log_positive = theta[count];
				count++;
			} else {
				log_positive = mb->off_theta[i][0][GMRFLib_thread_id][0];
			}
			if (!mb->off_fixed[i][0]) {
				val += mb->off_prior[i][0].priorfunc(&log_positive, mb->off_prior[i][0].parameters);
			}
			break;
		}
		default:
...
int inla_INLA(inla_tp * mb)
{
	...
	memset(&(mb->d[mb->predictor_n]), 0, (N - mb->predictor_n) * sizeof(double));
	mb->loglikelihood = Realloc(mb->loglikelihood, N, GMRFLib_logl_tp *);
	memset(&(mb->loglikelihood[mb->predictor_n]), 0, (N - mb->predictor_n) * sizeof(GMRFLib_logl_tp *));
...
int inla_MCMC(inla_tp * mb_old, inla_tp * mb_new)
{
	...
	memset(&(mb_old->d[mb_old->predictor_n]), 0, (N - mb_old->predictor_n) * sizeof(double));
	mb_old->loglikelihood = Realloc(mb_old->loglikelihood, N, GMRFLib_logl_tp *);
	memset(&(mb_old->loglikelihood[mb_old->predictor_n]), 0, (N - mb_old->predictor_n) * sizeof(GMRFLib_logl_tp *));
	mb_old->loglikelihood_arg = Realloc(mb_old->loglikelihood_arg, N, void *);
	memset(&(mb_old->loglikelihood_arg[mb_old->predictor_n]), 0, (N - mb_old->predictor_n) * sizeof(void *));
	mb_new->d = Realloc(mb_new->d, N, double);
	memset(&(mb_new->d[mb_new->predictor_n]), 0, (N - mb_new->predictor_n) * sizeof(double));
	mb_new->loglikelihood = Realloc(mb_new->loglikelihood, N, GMRFLib_logl_tp *);
	memset(&(mb_new->loglikelihood[mb_old->predictor_n]), 0, (N - mb_old->predictor_n) * sizeof(GMRFLib_logl_tp *));
	mb_new->loglikelihood_arg = Realloc(mb_new->loglikelihood_arg, N, void *);
	memset(&(mb_new->loglikelihood_arg[mb_new->predictor_n]), 0, (N - mb_new->predictor_n) * sizeof(void *));
...
#pragma omp parallel for private(i)
	...
	GMRFLib_ai_INLA(&(mb_old->density), &(mb_old->gdensity), NULL, NULL, NULL, NULL, NULL, NULL, mb_old->theta, mb_old->ntheta,
			...
			inla_all_offset, (void *) mb_old, 0, NULL, NULL, NULL);
	GMRFLib_free_ai_store(ai_store);
...
#pragma omp parallel for private(i) schedule(static)
	for (i = 0; i < 3; i++) {
		if (i == 0) {
			...
			if (mb->cpo) {
				inla_output_detail_cpo(mb->dir, mb->cpo, mb->predictor_n, local_verbose);
			}
			if (mb->dic) {
