...
#define DO_BYTESWAP (!big_endian_query())	  /* write file in big-endian format */
...
int xtmpl_open(int nx, int ny, const char *title)
{
	...
	WIN(window) = XCreateSimpleWindow(WIN(dpy),
					  RootWindow(WIN(dpy),WIN(screen)),
					  0,0,WIN(dim_x),WIN(dim_y),0,
					  BlackPixel(WIN(dpy),WIN(screen)),
	...
	XSetWindowBackground(WIN(dpy), WIN(window), WhitePixel(WIN(dpy),
...
int xtmpl_draw_a_line(int x0, int y0, int x1, int y1, GC gc)
{
    if (WIN(dpy) && xtmpl_view)
    {
	XDrawLine (WIN(dpy), WIN(window), gc, x0, WIN(dim_y) -1 -y0, x1, WIN(dim_y) -1 -y1); 
	XFlush(WIN(dpy));
	return XTMPL_OK;
    }
    else
...
int xtmpl_line(int fg, int x0, int y0, int x1, int y1)
{
    ...
    */
    xtmpl_record_event((fg ? DRAW : ERASE),
...
int record_channel_q()
{
    ...
    sprintf(w, "%1d", xtmpl_window); w[1] = '\0';
...
main()
{
    ...
    xtmpl_window = 1; doit(1,0);
    xtmpl_window = 0; doit(0,1);
    xtmpl_window = 1; doit(0,1);
}
#endif
#ifdef HILDE
main()
