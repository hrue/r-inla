...
int main(void) {
  if (false) {
    ...
    *i = 1.0;
    ...
    sv[0] = s[0];
    {
      for (typename test_type::search_iterator si = st.search(sv.begin());
           !si.is_null(); ++si) {
...

    {
      ...
      std::cout << oss;
      oss.build_tree();
      for (typename OrderedSegmentSet<double>::L_search_iterator si =
               oss.L_search(sv.begin());
           !si.is_null(); ++si) {
        std::cout << si.is_null() << " " << *si << std::endl;
      }
      for (typename OrderedSegmentSet<double>::R_search_iterator si =
               oss.R_search(sv.begin());
           !si.is_null(); ++si) {
        std::cout << si.is_null() << " " << *si << std::endl;
      }
    }
