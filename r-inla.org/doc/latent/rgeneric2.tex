...
\texttt{INLA}. It will run slower as the model properties has to be
evaluated in \texttt{R} within a \texttt{C}-program.
...
which is \texttt{sourced} before the model is defined, and
\texttt{<args>} are further parameters to the model (like dimension,
prior-settings, etc).  Then the model can used as
{\small
\begin{verbatim}
    y ~ ... + f(i, model=model)
\end{verbatim}
