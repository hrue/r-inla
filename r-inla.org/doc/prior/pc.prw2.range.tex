...
\end{verbatim}
but does not need to! This is why this parameter is not pre-set or set
automatically. If $h \le 0$, then $h$ is set by \texttt{INLA}, and 
currently this mean $h=1$ (but this could change in the future).

The $\lambda$ parameter is computed from the calibration as defined
above if  set to $\lambda \le 0$. If $\lambda > 0$, then this
value of $\lambda$ is used without computing the calibration, and the
values of $r_0$ and $\alpha$ are  silently ignored.

Note that $r_0$ is in real scale, ie in the scale of \texttt{loc}, and
$r_0$ is \textbf{not} scaled by $h$.

Default values are \texttt{param = c(1, 0.5, 1, 0)}.
