...
formula <- y ~ f(id, model="rw1", constr=F) + 
               ...
               f(id2, x2, model="rw1", constr=F) + 
               f(id3, x3, model="rw1", constr=F)-1 
...
for (i in 0:3) {
    rang <- range(r[[12]][[i+1]][,4] ,
                  r[[12]][[i+1]][,6] ,
                  get(paste("b", i, sep="")))
    plot(r[[12]][[i+1]][,2], type="l", 
         ylim=rang, col="red", ylab=paste("b", i, sep=""),xlab="time")
    lines(r[[12]][[i+1]][,4] , col="blue", lty=3)
    lines(r[[12]][[i+1]][,6] , col="blue", lty=3)
...
# -------------------------------------------
par(mfrow=c(3,2))
plot(r[[23]][[1]], type="l", xlab="1/V", main="precision for y", ylab="")  # precision of v_t
  abline(v=1/V,col=2, lwd=3)
plot(r[[23]][[2]], type="l", xlab="1/W1", main="precision for b_0", ylab="")  # precision of w0_t
  abline(v=1/W0,col=2, lwd=3)
plot(r[[23]][[3]], type="l", xlab="1/W1", main="precision for b_1", ylab="")  # precision of w1_t
  abline(v=1/W1,col=2, lwd=3)
plot(r[[23]][[4]], type="l", xlab="1/W2", main="precision for b_2", ylab="")  # precision of w2_t
  abline(v=1/W2,col=2, lwd=3)
plot(r[[23]][[5]], type="l", xlab="1/W3", main="precision for b_3", ylab="")  # precision of w3_t
...
# formulating the model
# ---------------------
formula <- y ~ f(id, model="rw1",param=c(1,0.001),initial=5,constr=F) + 
               f(id1, x1, model="rw1",param=c(1,0.01),initial=5,constr=F) + 
               f(id2, x2, model="rw1",param=c(1,0.001),initial=5,constr=F) -1
...
# ----------------------------------
par(mfrow=c(2,2))
plot(r[[23]][[1]], type="l", xlab="1/W0", main="precision for w_0", ylab="")  # precision of V
abline(v=1/W0,col=2, lwd=3)

plot(r[[23]][[2]], type="l", xlab="1/W1", main="precision for w_1", ylab="")  # precision of V
abline(v=1/W1,col=2, lwd=3)

plot(r[[23]][[3]], type="l", xlab="1/W2", main="precision for w_2", ylab="")  # precision of V
abline(v=1/W2,col=2, lwd=3)
...
w.iw  <- rep(c(NA,-1), c(nd, nrow(Y)-nd))
...
formula = Y ~ f(iv, model="generic1", Cmatrix=c.mat) +
              ...
              f(ix2, w.ix2, copy="ix") + 
              f(iw, w.iw, model="generic1", Cmatrix=c.mat) -1 
...
# call to fit the model
# ---------------------
r = inla(formula, data = data.frame(iv,ix,ix2,iw,w.iw),
         family = rep("gaussian",2), 
...
#postscript("D:\\meus artigos\\INLA-DLMs\\figures/ex6_mapv_first3t.eps", horizontal=FALSE, heig=10, width=21)
par(mfrow=c(3,2), mar=c(0,0,0,0))
for (t in 1:3) {
  q <- quantile(c(v[,t], r[[12]]$iv$mean[seq(1, nd, k)+t-1]), 0:4/4)
  plot(nc, col=c4[findInterval(v[,t], q,T)])
  plot(nc, col=c4[findInterval(r[[12]]$iv$mean[seq(1, nd, k)+t-1], q,T)])
  legend("bottomleft", leglabs(round(q,3),bet="to"), fill=c4, bty="n", ncol=2)
}
#dev.off()
...
par(mfrow=c(3,2), mar=c(0,0,0,0))
for (t in 1:3) {
  q <- quantile(c(w[,t], r[[12]]$iw$mean[seq(1, nd, k)+t-1]), 0:4/4)
  plot(nc, col=c4[findInterval(w[,t], q, T)])
  plot(nc, col=c4[findInterval(r[[12]]$iw$mean[seq(1, nd, k)+t-1], q, T)])
  legend("bottomleft", leglabs(format(q,dig=2),bet="to"), fill=c4, bty="n", ncol=2)
}

### map of w in others three times
par(mfrow=c(3,2), mar=c(0,0,0,0))
for (t in c(10,15,20)) {
  q <- quantile(c(w[r=,t], r[[12]]$iw$mean[seq(1, nd, k)+t-1]), 0:4/4)
  plot(nc, col=c4[findInterval(w[,t], q, T)])
  plot(nc, col=c4[findInterval(r[[12]]$iw$mean[seq(1, nd, k)+t-1], q, T)])
  legend("bottomleft", leglabs(format(q,dig=2),bet="to"), fill=c4, bty="n", ncol=2)
}
...
postscript("D:\\meus artigos\\INLA-DLMs\\figures/ex6b_mapy_2715-c.eps", 
...
postscript("D:\\meus artigos\\INLA-DLMs\\figures/ex6b_mapx1_2715-c.eps", 
  horizontal=FALSE, width=17, height=5)
par(mfrow=c(3,2), mar=c(0,0,0,0))
ind1 = c(2,7,15)
for (t in ind1) {
...
formula7 = Y ~ f(iw1, model="generic1", Cmatrix=c.mat, param=c(4,0.13333, 0,0.001), initial=c(log(50),0.2)) +
               ...
               f(iw2, ww2, model="generic1", Cmatrix=c.mat, param=c(4,0.08, 0,0.001), initial=c(log(100),0.2)) +
               f(iw3, ww3, model="generic1", Cmatrix=c.mat, param=c(4,0.08, 0,0.001), initial=c(log(100),0.2)) -1
...
# --------------------------------------------
par(mfrow=c(2,2))
plot(r[[23]][[1]], type="l", xlab="", main=expression(tau[1]), ylab="")  # precision of w1_t
  abline(v=tau[1],col=2, lwd=3)
plot(r[[23]][[3]], type="l", xlab="", main=expression(tau[2]), ylab="")  # precision of w2_t
  abline(v=tau[2],col=2, lwd=3)
plot(r[[23]][[5]], type="l", xlab="", main=expression(tau[3]), ylab="")
...
# ----------------------------------------------
par(mfrow=c(2,2))
plot(r[[23]][[2]], type="l", xlab="", main=expression(phi[1]), ylab="")  
abline(v=phi[1],col=2, lwd=3)
plot(r[[23]][[4]], type="l", xlab="", main=expression(phi[2]), ylab="")  
abline(v=phi[2],col=2, lwd=3)
plot(r[[23]][[6]], type="l", xlab="", main=expression(phi[3]), ylab="")  
...
plot.ts(r[[12]]$l$mean,ylab="Slope",axes=FALSE,col=4,lwd=2)
...
lines(1:534,12*atan2(r[[12]]$rr$mean,r[[12]]$q$mean)/(2*pi),type='l',ylim=c(2.7,5.3),ylab='Peak',xlab='',lwd=2,axes=FALSE,col=2)
abline(h=3,lty=3)
abline(h=4,lty=3)
abline(h=5,lty=3)
...
lines(1:534,exp(2*sqrt((r[[12]]$q$mean)^2 + (r[[12]]$rr$mean)^2)),type='l',ylab='PT-ratio',xlab='Years',ylim=c(0,12),lwd=2,axes=FALSE,col=2)
abline(h=0,lty=3)
abline(h=5,lty=3)
abline(h=10,lty=3)
axis(2)
