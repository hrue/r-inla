...
#          control.inla=list(strategy="laplace", int.strategy="grid"),
          control.family = list(list(), list(initial=log(2000), fixed=T)),
...
# ------------------------------------------
par(mfrow=c(2,1))
plot(r[[22]][[1]], type="l", xlab="1/V", main="precision for Gaussian observations", ylab="")  # precision of V
  abline(v=1/V,col=2, lwd=5)
plot(r[[22]][[2]], type="l", xlab="1/W", main="precision for w_t", ylab="")  # precision of W
...
i = 1:n
ww = rep(1,n)
formula1 = y ~ ww + f(i, ww, model="rw1") -1 
...
# graph for states (x)
rang <- range(r[[12]][[1]][1:n, 4:6]+r$summary.fixed[1], x)
plot(r[[12]][[1]][1:n,2]+r$summary.fixed[1], type="l", 
 ylim=rang, col="red", xlim=c(1,n),ylab="X_t",xlab="time")
lines(r[[12]][[1]][1:n,4]+r$summary.fixed[1], col="blue", lty=3)
lines(r[[12]][[1]][1:n,6]+r$summary.fixed[1], col="blue", lty=3)
lines(x[1:n])
legend("topright", legend=c("simulated X_t","posterior mean","95% CI"), col=c("black", "red","blue"),lty=c(1,1,2),bty="n")
#title("a toy example")
...
# ------------------------------------------
par(mfrow=c(2,2))
plot(r[[22]][[1]], type="l", xlab="1/V", main="precision for observations", ylab="")  # precision of V
  abline(v=1/V1,col=2, lwd=3)
plot(r[[22]][[2]], type="l", xlab="1/W1", main="precision for w_1t", ylab="")  # precision of W
  abline(v=1/W1,col=2, lwd=3)
plot(r[[22]][[3]], type="l", xlab="1/W2", main="precision for w_2t", ylab="")  # precision of W
...
# ---------------------------------------
i=1:n
formula1 = y3 ~ cosw + f(i, cosw, model="rw1") -1
...
# graph for states (x)
rang <- range(r[[12]][[1]][1:n, 4:6]+r$summary.fixed[1], a)
plot(r[[12]][[1]][1:n,2]+r$summary.fixed[1], type="l", 
 ylim=rang, col="red", xlim=c(1,n),ylab="X_t",xlab="time")
lines(r[[12]][[1]][1:n,4]+r$summary.fixed[1], col="blue", lty=3)
lines(r[[12]][[1]][1:n,6]+r$summary.fixed[1], col="blue", lty=3)
...
# ------------------------------------------
par(mfrow=c(2,2))
plot(r[[22]][[1]], type="l", xlab="1/V", main="precision for y", ylab="")  # precision of V
  abline(v=1/V,col=2, lwd=3)
plot(r[[22]][[2]], type="l", xlab="1/W1", main="precision for x_1", ylab="")  # precision of W
  abline(v=1/W1,col=2, lwd=3)
...
plot(r[[22]][[1]], type="l", xlab="1/V", main="precision for y", ylab="")  # precision of V
  abline(v=1/V,col=2, lwd=3)
plot(r[[22]][[2]], type="l", xlab="1/W1", main="precision for a_t", ylab="")  # precision of W
  abline(v=1/W1,col=2, lwd=3)
plot(r[[22]][[3]], type="l", xlab="1/W2", main="precision for b_t", ylab="")  # precision of W
  abline(v=1/W2,col=2, lwd=3)


# an alternative way to fit the same model using a "seasonal" model
#i=1:n
#formula2 = y ~ f(i, model="seasonal", season.length=12) -1
#r = inla(formula2, data = data.frame(i,y),
#         control.predictor=list(compute=TRUE, cdf=c(.025, .975)))
...
## Simulating a dynamic regression model with three covariates 
# ------------------------------------------------------------
set.seed(1)
n <- 500
x1 <- runif(n)
x2 <- runif(n)  ##rnorm(n)##rexp(n, 1)*0.5
x3 <- runif(n)
b0_0 = 0
b1_0 = 0
b2_0 = 0
...
w1  = rnorm(n,0,sqrt(W1))
w2  = rnorm(n,0,sqrt(W2))
w3  = rnorm(n,0,sqrt(W3))
y = b0 = b1 = b2 = b3 = rep(0,n)
b0[1] = b0_0 + w0[1]
b1[1] = b1_0 + w1[1]
b2[1] = b2_0 + w2[1]
b3[1] = b3_0 + w3[1]
...
y <- b0 + b1*x1 + b2*x2 + b3*x3 + v


# Plotting the dynamic regression model
# -------------------------------------
par(mfrow=c(1,1), mar=c(3,3,1,.5), mgp=c(2,1,0))
plot(y,type="l",xlab="time",ylab="", ylim=range(y,b1,b2,b3))
lines(b0,col=2)
lines(b1,col=3)
lines(b2,col=4)
lines(b3,col=5)
legend("topleft", c("y","b0", "b1","b2","b3"), col=1:5, lty=1)


# defining indices for rw1 coefficients
# -------------------------------------
id <- id1 <- id2 <- id3 <- 1:length(y)


# formulating the model
# ---------------------
formula <- y ~ f(id, model="rw1") + 
               x1 + f(id1, x1, model="rw1") + 
               x2 + f(id2, x2, model="rw1") + 
               x3 + f(id3, x3, model="rw1") 
...
# call to fit the model
# ---------------------
r = inla(formula, data = data.frame(x1, x2, x3, id, id1, id2, id3,y), 
         control.predictor=list(compute=TRUE, cdf=c(.025, .975)))
...
# graph for y
par(mfrow=c(1,1))
rang <- range(r[[17]][, 3:5], y)
plot(r[[17]][1:n,1], type="l", ylim=rang, col="red", 
     xlim=c(1,n),ylab="z",xlab="time")
lines(r[[17]][,3], col="blue", lty=3)
lines(r[[17]][,5], col="blue", lty=3)
lines(y)
legend("topleft", c("simulated","posterior mean","95% CI"),
...
for (i in 0:3) {
    rang <- range(r[[12]][[i+1]][,4] + r[[2]][i+1,3],
                  r[[12]][[i+1]][,6] + r[[2]][i+1,5],
                  get(paste("b", i, sep="")))
    plot(r[[12]][[i+1]][,2] + r[[2]][i+1,1], type="l", 
         ylim=rang, col="red", ylab=paste("b", i, sep=""),xlab="time")
    lines(r[[12]][[i+1]][,4] + r[[2]][i+1,3], col="blue", lty=3)
    lines(r[[12]][[i+1]][,6] + r[[2]][i+1,5], col="blue", lty=3)
    lines(get(paste("b", i, sep="")))
    legend("topright", c("simulated","posterior mean","95% CI"),
...
# summary of hyperparameter estimates
# -----------------------------------
r$summary.hyperpar


# posterior densities for precision estimates
# -------------------------------------------
par(mfrow=c(3,2))
plot(r[[22]][[1]], type="l", xlab="1/V", main="precision for y", ylab="")  # precision of v_t
  abline(v=1/V,col=2, lwd=3)
plot(r[[22]][[2]], type="l", xlab="1/W1", main="precision for b_0", ylab="")  # precision of w0_t
  abline(v=1/W0,col=2, lwd=3)
plot(r[[22]][[3]], type="l", xlab="1/W1", main="precision for b_1", ylab="")  # precision of w1_t
  abline(v=1/W1,col=2, lwd=3)
plot(r[[22]][[4]], type="l", xlab="1/W2", main="precision for b_2", ylab="")  # precision of w2_t
  abline(v=1/W2,col=2, lwd=3)
plot(r[[22]][[5]], type="l", xlab="1/W3", main="precision for b_3", ylab="")  # precision of w3_t
  abline(v=1/W3,col=2, lwd=3)
...
## Simulating a dynamic Poisson model (dynamic intercept without covariate)
## ------------------------------------------------------------------------
set.seed(123456)
...
## -------------------------------------
par(mfrow=c(1,1))
plot.ts(y,xlab="time",ylab="",ylim=range(y))
lines(exp(theta),col=2)
legend("topleft",legend=c("y","theta"),
       col=c("black", "red"),lty=1,bty="n")
title(paste("W1=",W1, sep=""))
...
## indices for the INLA library
## ----------------------------
k       = c(1:n, 2:n)                    # \theta_{1,t}
l       = c(rep(NA,n), 1:m)              # \theta_{1,t-1}
weight2 = c(rep(NA,n), rep(-1,m))        # weights for \theta_{1,t-1}
u       = c(rep(NA,n), 2:n)              # w_{1,t}

## formulating the model
## ---------------------
formula = Y ~ f(k, model="iid", initial=-10, fixed=T) +
              f(l, weight2, copy="k") + 
              f(u, model ="iid") -1
...
# call to fit the model
# ---------------------
r = inla(formula, data = data.frame(k,l,weight2,u),
...
## graph for states (theta_t)
rang <- range(r[[12]][[1]][1:n,4:6], theta[1:n])
plot(r[[12]][[1]][1:n,2], type="l", 
     ylim=rang, col="red", xlim=c(1,n),ylab="theta",xlab="time")
lines(r[[12]][[1]][1:n,4], col="blue", lty=3)
lines(r[[12]][[1]][1:n,6], col="blue", lty=3)
lines(theta[1:n])
legend("topright", legend=c("observed","posterior mean","95% CI"), col=c("black", "red","blue"),lty=c(1,1,2),bty="n")
...
## Simulating a dynamic Poisson regression
...
# formulating the model
# ---------------------
formula <- y ~ f(id, model="rw1",param=c(1,0.001),initial=5) + 
               x1 + f(id1, x1, model="rw1",param=c(1,0.01),initial=5) + 
               x2 + f(id2, x2, model="rw1",param=c(1,0.001),initial=5) 
...
rang <- range(r[[12]][[1]][1:n,4:6]+r$summary.fixed[1,1], b0[1:n])
plot(r[[12]][[1]][1:n,2]+r$summary.fixed[1,1], type="l", 
     ylim=rang, col="red", xlim=c(1,n),ylab="b0",xlab="time")
lines(r[[12]][[1]][1:n,4]+r$summary.fixed[1,1], col="blue", lty=3)
lines(r[[12]][[1]][1:n,6]+r$summary.fixed[1,1], col="blue", lty=3)
lines(b0[1:n])
legend("topleft", legend=c("simulated","posterior mean","95% CI"), col=c("black", "red","blue"),lty=c(1,1,2),bty="n")
##title("a toy example")

## graph for states (b1_t)
rang <- range(r[[12]][[2]][1:n,4:6]+r$summary.fixed[2,1], b1[1:n])
plot(r[[12]][[2]][1:n,2]+r$summary.fixed[2,1], type="l", 
     ylim=rang, col="red", xlim=c(1,n),ylab="b1",xlab="time")
lines(r[[12]][[2]][1:n,4]+r$summary.fixed[2,1], col="blue", lty=3)
lines(r[[12]][[2]][1:n,6]+r$summary.fixed[2,1], col="blue", lty=3)
lines(b1[1:n])
legend("bottomleft", legend=c("simulated","posterior mean","95% CI"), col=c("black", "red","blue"),lty=c(1,1,2),bty="n")
##title("a toy example")

## graph for states (b2_t)
rang <- range(r[[12]][[3]][1:n,4:6]+r$summary.fixed[3,1], b2[1:n])
plot(r[[12]][[3]][1:n,2]+r$summary.fixed[3,1], type="l", 
     ylim=rang, col="red", xlim=c(1,n),ylab="b2",xlab="time")
lines(r[[12]][[3]][1:n,4]+r$summary.fixed[3,1], col="blue", lty=3)
lines(r[[12]][[3]][1:n,6]+r$summary.fixed[3,1], col="blue", lty=3)
lines(b2[1:n])
legend("topleft", legend=c("simulated","posterior mean","95% CI"), col=c("black", "red","blue"),lty=c(1,1,2),bty="n")
##title("a toy example")
...
# ----------------------------------
par(mfrow=c(2,2))
plot(r[[22]][[1]], type="l", xlab="1/W0", main="precision for w_0", ylab="")  # precision of V
abline(v=1/W0,col=2, lwd=3)

plot(r[[22]][[2]], type="l", xlab="1/W1", main="precision for w_1", ylab="")  # precision of V
abline(v=1/W1,col=2, lwd=3)

plot(r[[22]][[3]], type="l", xlab="1/W2", main="precision for w_2", ylab="")  # precision of V
abline(v=1/W2,col=2, lwd=3)
...
#  y_{i,t} = x_{i,t} + v_{i,t},           i=1,...,n,  t=1,...,k
# System equation:
#  x_{i,t} = x_{i,t-1} + w_{i,t},         i=1,...,n,  t=2,...,k
#
# We can re-write the system equation model as
#  0 = x_{i,t} - x_{i,t-1} - w_{i,t},     i=1,...,n,  t=2,...,k
# where 
#  v_{,t} \sim N(0, \Sigma_1)
#  w_{,t} \sim N(0, \Sigma_2)
#
# we use \Sigma_j^{-1} = D_j(I - \phi_j C), j = {1, 2}
# where
...
# Loading North Carolina's map (it has 100 areas)
require(spdep)
...
v <- t(myrmvnorm(k, rep(0,n), solve(tau[1]*Q1)))
w <- t(myrmvnorm(k, rep(0,n), solve(tau[2]*Q2)))
...
### ----------------------------
ix    <- c(1:nd, (1:nd)[-c(1,((1:(n-1))*k)+1)])  ## x_{,t}
ix2   <- c(rep(NA,nd), (1:nd)[-((1:n)*k)])       ## x_{,t-1}
w.ix2 <- c(rep(NA,nd), rep(-1,nd-n))             ## weights for x_{,t-1}
iv    <- c(1:nd, rep(NA,nd-n))                   ## v_{,t} 
iw    <- c(rep(NA,nd), ix[-(1:nd)])              ## w_{,t}


## formulating the model
## ---------------------
# with default prior for \tau and \phi
formula = Y ~ f(iv, model="generic1", Cmatrix=c.mat) +
              f(ix, model="iid", initial=-10, fixed=T) +
              f(ix2, w.ix2, copy="ix") + 
              f(iw, model="generic1", Cmatrix=c.mat) -1 

# with informative prior for \tau and default prior for \phi
#formula1 = Y ~ f(iv, model="generic1", Cmatrix=c.mat, param=c(1,1/30, 0,0.001)) +
#              f(ix, model="iid", initial=-10, fixed=T) +
#              f(ix2, w.ix2, copy="ix") + 
#              f(iw, model="generic1", Cmatrix=c.mat, param=c(1,1/50, 0,0.001)) -1 

# with vague prior for \tau and default prior for \phi
#formula2 = Y ~ f(iv, model="generic1", Cmatrix=c.mat, param=c(4,0.13333, 0,0.001)) +
#              f(ix, model="iid", initial=-10, fixed=T) +
#              f(ix2, w.ix2, copy="ix") + 
#              f(iw, model="generic1", Cmatrix=c.mat, param=c(4,0.08, 0,0.001)) -1 
...
# call to fit the model
# ---------------------
r = inla(formula, data = data.frame(iv,ix,ix2,iw),
         family = rep("gaussian",2), 
         control.family = list(list(initial=10, fixed=T),list(initial=10, fixed=T)),
         control.predictor=list(compute=TRUE, cdf=c(.025, .975)))

#r1 = inla(formula1, data = data.frame(iv,ix,ix2,iw),
#         family = rep("gaussian",2), 
#         control.family = list(list(initial=10, fixed=T),list(initial=10, fixed=T)),
#         control.predictor=list(compute=TRUE, cdf=c(.025, .975)))

#r2 = inla(formula2, data = data.frame(iv,ix,ix2,iw),
#         family = rep("gaussian",2), 
#         control.family = list(list(initial=10, fixed=T),list(initial=10, fixed=T)),
#         control.predictor=list(compute=TRUE, cdf=c(.025, .975)))
...
plot(as.vector(t(v)), r[[12]]$iv$mean)
plot(as.vector(t(w)), r[[12]]$iw$mean)

plot.ts(x[1,])
lines(r[[12]]$ix$mean[1:k], col=2)
...
# --------------------------------------------
par(mfrow=c(2,2))
#rang1 <- range(r[[22]][[1]][,2],r1[[22]][[1]][,2],r2[[22]][[1]][,2])
plot(r[[22]][[1]], type="l", xlab="1/V", main=expression(tau[v]), ylab="") #,ylim=rang1)  # precision of V
#  lines(r1[[22]][[1]],lty=3)
#  lines(r2[[22]][[1]],lty=2)
abline(v=tau[1],col=2, lwd=3)
#legend("topright", legend=c("with vague prior","with informative prior","with default prior"), col=c("black", "black", "black"),lty=c(3,1,2),bty="n",cex=0.5)
#rang2 <- range(r[[22]][[2]][,2],r1[[22]][[2]][,2],r2[[22]][[2]][,2])
plot(r[[22]][[2]], type="l", xlab=expression(phi[v]), main=expression(phi[v]), ylab="") #,ylim=rang2)  
#  lines(r1[[22]][[2]],lty=3)
#  lines(r2[[22]][[2]],lty=2)
abline(v=phi[1],col=2, lwd=3)
#rang3 <- range(r[[22]][[3]][,2],r1[[22]][[3]][,2],r2[[22]][[3]][,2])
plot(r[[22]][[3]], type="l", xlab="1/W", main=expression(tau[w]), ylab="") #,ylim=rang3)  # precision of V
#  lines(r1[[22]][[3]],lty=3)
#  lines(r2[[22]][[3]],lty=2)
abline(v=tau[2],col=2, lwd=3)
#rang4 <- range(r[[22]][[4]][,2],r1[[22]][[4]][,2],r2[[22]][[4]][,2])
plot(r[[22]][[4]], type="l", xlab=expression(phi[w]), main=expression(phi[w]), ylab="") #,ylim=rang4)  
#  lines(r1[[22]][[4]],lty=3)
#  lines(r2[[22]][[4]],lty=2)
abline(v=phi[2],col=2, lwd=3)

### map of v in first three times
par(mfrow=c(3,2), mar=c(0,0,0,0))
for (t in 1:3) {
  plot(nc, col=ifelse(v[,t]<0,"white", "gray"))
  plot(nc, col=ifelse(r[[12]]$iv$mean[seq(1, nd, k)+t-1]<0,"white", "gray"))
}

### map of w in first three times
par(mfrow=c(3,2), mar=c(0,0,0,0))
for (t in 1:3) {
  plot(nc, col=ifelse(w[,t]<0,"white", "gray"))
  plot(nc, col=ifelse(r[[12]]$iw$mean[seq(1, nd, k)+t-1]>0, "white", "gray"))
}

### map of w in others three times
par(mfrow=c(3,2), mar=c(0,0,0,0))
for (t in c(10,15,20)) {
  plot(nc, col=ifelse(w[,t]<0,"white", "gray"))
  plot(nc, col=ifelse(r[[12]]$iw$mean[seq(1, nd, k)+t-1]>0, "white", "gray"))
...
ind1 = c(2,7,15)
for (t in ind1) {
  plot(nc, col=ifelse(y[,t]<0,"white", "gray"))
#title(t, "areas")
  plot(nc, col=ifelse(r[[17]][seq(1, nd, k)+t-1,1]<0,"white", "gray"))
#title("areas")
}
...
ind1 = c(2,7,15)
for (t in ind1) {
  plot(nc, col=ifelse(x[,t]<0,"white", "gray"))
  plot(nc, col=ifelse(r[[12]]$ix$mean[seq(1, nd, k)+t-1]<0,"white", "gray"))
}
...
#plot(nc, col=ifelse(w1[1,]>0,"gray","transparent"))
#plot(nc, col=ifelse(w2[1,]>0,"gray","transparent"))
#plot(nc, col=ifelse(w3[1,]>0,"gray","transparent"))
...
rang=range(yy[10,],yy[nc.nb[[10]][1],],yy[nc.nb[[10]][2],],yy[nc.nb[[10]][3],])
plot.ts(yy[10,],ylim=rang)
lines.ts(yy[nc.nb[[10]][1],], col=2, ylim=rang)
lines.ts(yy[nc.nb[[10]][2],], col=3, ylim=rang)
lines.ts(yy[nc.nb[[10]][3],], col=4, ylim=rang)
...
## formulating the model
## ---------------------
# with informative prior for precision parameters and initial \phi=0.2
formula7 = Y ~ f(iw1, model="generic1", Cmatrix=c.mat, param=c(4,0.13333, 0,0.001), initial=c(log(50),0.2)) +
               f(ix1, model="iid", initial=-10, fixed=T) +
               f(ix1b, wx1b, copy="ix1") +
               f(ix2, model="iid", initial=-10, fixed=T) +
               f(ix2b, wx2b, copy="ix2") +
               f(iw2, model="generic1", Cmatrix=c.mat, param=c(4,0.08, 0,0.001), initial=c(log(100),0.2)) +
               f(iw3, model="generic1", Cmatrix=c.mat, param=c(4,0.08, 0,0.001), initial=c(log(100),0.2)) -1

# with vague prior for precision parameters and initial \phi=0.2
#formula12 = Y ~ f(iw1, model="generic1", Cmatrix=c.mat, param=c(0.01,0.0003333, 0,0.001), initial=c(log(50),0.2)) +
#              f(ix1, model="iid", initial=-10, fixed=T) +
#              f(ix1b, wx1b, copy="ix1") +
#              f(ix2, model="iid", initial=-10, fixed=T) +
#              f(ix2b, wx2b, copy="ix2") +
#              f(iw2, model="generic1", Cmatrix=c.mat, param=c(0.01,0.0002, 0,0.001), initial=c(log(100),0.2)) +
#              f(iw3, model="generic1", Cmatrix=c.mat, param=c(0.01,0.0002, 0,0.001), initial=c(log(100),0.2)) -1
...
## call to fit the model
## ---------------------
r7 = inla(formula7, data = data.frame(ix1,ix1b,wx1b,ix2,ix2b,wx2b,iw1,iw2,iw3),
          family = rep("gaussian",3),
...
#r12 = inla(formula12, data = data.frame(ix1,ix1b,wx1b,ix2,ix2b,wx2b,iw1,iw2,iw3),
#         family = rep("gaussian",3),
#         control.inla = list(h=0.1), 
#         control.family = list(list(initial=10, fixed=T),
#         list(initial=10, fixed=T), list(initial=10, fixed=T)),
...
plot(as.vector(t(x1)), r[[12]]$ix1$mean)
plot(as.vector(t(x2[,-1])), r[[12]]$ix2$mean)
...
for (i in c(area, nc.nb[[area]])) {
  id <- 1:k + (i-1)*k
  rang <- range(r[[17]][id,3:5], yy[i,])
  plot.ts(r[[17]][id,1], ylim=rang, col="red", ylab="X1_t",xlab="time")
  lines(r[[17]][id,3], col="blue", lty=3)
  lines(r[[17]][id,5], col="blue", lty=3)
  lines(yy[i,])
  legend("bottomleft", legend=c("simulated y","posterior mean","95% CI"), col=c("black", "red","blue"),lty=c(1,1,2),bty="n")
}
...
for (i in c(area, nc.nb[[area]])) {
  id <- 1:k + (i-1)*k
  rang <- range(r[[12]]$ix1[id, 4:6], x1[i,])
  plot.ts(r[[12]]$ix1[id,2], ylim=rang, col="red", ylab="x1_t",xlab="time")
  lines(r[[12]]$ix1[id,4], col="blue", lty=3)
  lines(r[[12]]$ix1[id,6], col="blue", lty=3)
  lines(x1[i,])
...
par(mfrow=c(2,2), mar=c(3,3,2,1), mgp=c(2,1,0))
area=20
for (i in c(10, nc.nb[[10]])) {
  id <- 1:k + (i-1)*k
  rang <- range(r[[12]]$ix2[id, 4:6], x2[i,])
  plot.ts(r[[12]]$ix2[id,2], ylim=rang, col="red", ylab="x2_t",xlab="time")
  lines(r[[12]]$ix2[id,4], col="blue", lty=3)
  lines(r[[12]]$ix2[id,6], col="blue", lty=3)
  lines(x2[i,])
...
# --------------------------------------------
par(mfrow=c(2,2))
plot(r[[22]][[1]], type="l", xlab="", main=expression(tau[1]), ylab="")  # precision of w1_t
  abline(v=tau[1],col=2, lwd=3)
plot(r[[22]][[3]], type="l", xlab="", main=expression(tau[2]), ylab="")  # precision of w2_t
  abline(v=tau[2],col=2, lwd=3)
plot(r[[22]][[5]], type="l", xlab="", main=expression(tau[3]), ylab="")
  abline(v=tau[3],col=2, lwd=3)


# posterior densities for correlation parameters
# ----------------------------------------------
par(mfrow=c(2,2))
plot(r[[22]][[2]], type="l", xlab="", main=expression(phi[1]), ylab="")  
abline(v=phi[1],col=2, lwd=3)
plot(r[[22]][[4]], type="l", xlab="", main=expression(phi[2]), ylab="")  
abline(v=phi[2],col=2, lwd=3)
plot(r[[22]][[6]], type="l", xlab="", main=expression(phi[3]), ylab="")  
abline(v=phi[3],col=2, lwd=3)


### map of y_t at times 2, 7 and 15
par(mfrow=c(3,2), mar=c(0,0,0,0))
ind1 = c(2,7,15)
for (t in ind1) {
  plot(nc, col=ifelse(yy[,t]<0,"white", "gray"))
  plot(nc, col=ifelse(r[[17]][seq(1, nd, k)+t-1,1]<0,"white", "gray"))
}

### map of x_1 at times 2, 7 and 15
par(mfrow=c(3,2), mar=c(0,0,0,0))
ind1 = c(2,7,15)
for (t in ind1) {
  plot(nc, col=ifelse(x1[,t]<0,"white", "gray"))
  plot(nc, col=ifelse(r[[12]]$ix1$mean[seq(1, nd, k)+t-1]<0,"white", "gray"))
}

### maps of x_2 at times 2, 7 and 15
par(mfrow=c(3,2), mar=c(0,0,0,0))
ind1 = c(2,7,15)
for (t in ind1) {
  plot(nc, col=ifelse(x2[,t]<0,"white", "gray"))
  plot(nc, col=ifelse(r[[12]]$ix2$mean[seq(1, nd, k)+t-1]<0,"white", "gray"))
}
...
formula = Y ~ f(q, model="seasonal", season.length=4,param=c(1,0.0001),initial=4) +
              weight2 + f(l, weight2, model="rw1",param=c(1,0.0001),initial=4) +
...
plot.ts(r[[12]]$l$mean+r[[2]][1],ylab="Slope",axes=FALSE,col=4,lwd=2)
#lines(r[[12]]$l[,4],col=4)
#lines(r[[12]]$l[,6],col=4)
...
plot(r[[22]][[2]], type="l", xlab="1/W1", main="precision for w_1t", ylab="")  # precision of W
plot(r[[22]][[3]], type="l", xlab="1/W2", main="precision for w_2t", ylab="")  # precision of W
plot(r[[22]][[4]], type="l", xlab="1/W3", main="precision for w_3t", ylab="")  # precision of W
...
ww = rep(1,n)               # weights for T_t


# formulating the model
# ---------------------
formula = y ~ belt + ww + f(i, ww, model="rw1",param=c(1,0.0005)) +  
...
# call to fit the model
# ---------------------
r = inla(formula, data = data.frame(belt,ww,i,j,y),
         family = "poisson",
         control.predictor=list(compute=TRUE))
...
lines(1:192, exp(vandrivers$seatbelt*r[[2]][1] + r[[12]][[1]][,2]+r[[2]][2]),col="blue")
lines(1:192, exp(vandrivers$seatbelt*r[[2]][1] + r[[12]][[1]][,4]+r[[2]][2]),lty=3,col="blue")
lines(1:192, exp(vandrivers$seatbelt*r[[2]][1] + r[[12]][[1]][,6]+r[[2]][2]),lty=3,col="blue")
...
# -----------------------------------------
par(mfrow=c(2,1))
plot(r[[22]][[1]], type="l", xlab="1/V", main="precision for w_t", ylab="")  # precision for w_t
...
# formulating the model
# ---------------------
formula = Y ~ cosine + f(q, cosine, model="rw1",param=c(1,0.01),initial=4) +
              sine + f(rr, sine, model="rw1",param=c(1,0.01),initial=4) + 
              weight2 + f(l, weight2, model="rw1",param=c(1,0.2),initial=4) +
              f(i, model="iid", initial=-10, fixed=TRUE) +
              f(j, weight1, copy="i") + 
              f(w1, model ="iid") -1
...
lines(r[[12]]$l$mean+r[[2]][3,1], col=2)

plot(1:534, m3.fit$m[,3], type="l")
lines(r[[12]]$q$mean+r[[2]][1,1], col=2)

plot(1:534, m3.fit$m[,4], type="l")
lines(r[[12]]$rr$mean+r[[2]][2,1], col=2)
...
lines(1:534,exp(r[[12]]$i$mean),type="l",col=2,lwd=2)
lines(1:534,exp(m3.fit$m[,1]),type='l',col=4,lwd=2)
axis(2)
box()

plot(1:534,12*atan2(m3.fit$m[,4],m3.fit$m[,3])/(2*pi),type='l',ylim=c(2.7,5.3),ylab='Peak',xlab='',lwd=2,axes=FALSE,col=4)
lines(1:534,12*atan2(r[[12]]$rr$mean+r[[2]][2,1],r[[12]]$q$mean+r[[2]][1,1])/(2*pi),type='l',ylim=c(2.7,5.3),ylab='Peak',xlab='',lwd=2,axes=FALSE,col=2)
abline(h=3,lty=3)
abline(h=4,lty=3)
abline(h=5,lty=3)
...
plot(1:534,exp(2*sqrt(m3.fit$m[,3]^2 + m3.fit$m[,4]^2)),type='l',ylab='PT-ratio',xlab='Years',ylim=c(0,12),lwd=2,axes=FALSE,col=4)
lines(1:534,exp(2*sqrt((r[[12]]$q$mean+r[[2]][1,1])^2 + (r[[12]]$rr$mean+r[[2]][2,1])^2)),type='l',ylab='PT-ratio',xlab='Years',ylim=c(0,12),lwd=2,axes=FALSE,col=2)
abline(h=0,lty=3)
abline(h=5,lty=3)
abline(h=10,lty=3)
axis(2)
...
# formulating the model
# ---------------------
formula <- SHARE ~ PROM + f(id1, PROM, model="rw1", param=c(1,0.001))  +
                   PRICE + f(id2, PRICE, model="rw1", param=c(1,0.01)) + 
                   CPROM + f(id3, CPROM, model="rw1", param=c(1,0.001)) 
...
# summary for fixed effect and hyperparameters
r.rw1$summary.fixed
...
par(mfrow=c(3,1),mar=c(0,5.1,0,2.1),oma=c(6,0,5,0))
ylm <- range(r.rw1[[12]][[1]][,4:8] + r.rw1$summary.fixed[2,1]) #,bats$PROM)
plot.ts(r.rw1[[12]][[1]]$mean + r.rw1$summary.fixed[2,1],type='l',
        ylab='PROM',xlab='',axes=FALSE, col=2,ylim=ylm)
lines.ts(r.rw1[[12]][[1]][,4] + r.rw1$summary.fixed[2,1], col=4, lty=3)
lines.ts(r.rw1[[12]][[1]][,8] + r.rw1$summary.fixed[2,1], col=4, lty=3)
#lines.ts(bats$PROM)
axis(2)
box()

ylm <- range(r.rw1[[12]][[2]][,4:8] + r.rw1$summary.fixed[3,1]) #,bats$PRICE)
plot.ts(r.rw1[[12]][[2]]$mean + r.rw1$summary.fixed[3,1],type='l',
        ylab='PRICE',xlab='',axes=FALSE, col=2,ylim=ylm)
lines.ts(r.rw1[[12]][[2]][,4] + r.rw1$summary.fixed[3,1], col=4, lty=3)
lines.ts(r.rw1[[12]][[2]][,8] + r.rw1$summary.fixed[3,1], col=4, lty=3)
#lines.ts(bats$PRICE)
axis(2)
box()

ylm <- range(r.rw1[[12]][[3]][,4:8] + r.rw1$summary.fixed[4,1]) #,bats$CPROM)
plot.ts(r.rw1[[12]][[3]]$mean + r.rw1$summary.fixed[4,1],type='l',
        ylab='CPROM',axes=FALSE, col=2,ylim=ylm,xlab="Time")
lines.ts(r.rw1[[12]][[3]][,4] + r.rw1$summary.fixed[4,1], col=4, lty=3)
lines.ts(r.rw1[[12]][[3]][,8] + r.rw1$summary.fixed[4,1], col=4, lty=3)
#lines.ts(bats$CPROM)
axis(2)
...
# formulating the model
# ---------------------
formula <- SHAREf ~ PROMf + f(id1f, PROMf, model="rw1", param=c(1,0.001))  +
                   PRICEf + f(id2f, PRICEf, model="rw1", param=c(1,0.01)) + 
                   CPROMf + f(id3f, CPROMf, model="rw1", param=c(1,0.001)) 
