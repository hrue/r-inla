...
The model we would like to use is specified by setting the item
...
quantity. For example precision parameters are always modelled on the
log scale. Posterior marginals and summary estimates are, however,
...
\begin{itemize}
	...
	set a sum to zero. By default the sum to 0 constraint is
	...
	{\tt A} and a
	vector {\tt e}, which defines the extra constraint {\tt Ax = e}; for
	example {\tt extraconstr = list(A = A, e=e)}. Note that this
