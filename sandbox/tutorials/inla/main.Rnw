...
Table~\ref{tab:likelihoodModels} gives an overview of all implemented likelihood models. In the following we will shortly present the main definition and its properties.
...
\bottomrule
\end{tabular}
\caption{List of latent Gaussian field implemented in the R inla package.}\label{tab:likelihoodModels}
\end{center}
\end{table}

\subsection{Independent random noise}
...
\subsection{Random walk of order 1}
\subsection*{Parametrization}
...
The prior is defined on $\mathbf{\theta}$ which is defined as
%\begin{displaymath}
    $\theta =\log \tau$
%\end{displaymath}
...
\subsection{Random walk of order 2}
...
\end{eqnarray*}
where $\mathbf{Q}=\tau\mathbf{R}$ and $\mathbf{R}$ is the structure
matrix reflecting the neighbourhood structure of the model. The RW2 is a intrinsic with rank deficiency 2.
...
\subsection{Random walk of order 2 on a lattice}  
\subsection{Continuous random walk of order 2}
\subsection{Model for seasonal variation}
\subsection{Model for spatial effect}
\subsection{Model for spatial effect (proper version)}
\subsection{Model for weighted spatial effects}
\subsection{Model for spatial effect + random effect}
\subsection{Autoregressive model of order 1}
\subsection{Autoregressive model of order p}
\subsection{The Ornstein-Uhlenbeck process}
\subsection{User defined structure matrix, type 0}
\subsection{User defined structure matrix, type 1}
\subsection{User defined structure matrix, type 2}
\subsection{Model for correlated effects with Wishart prior}
\subsection{Gaussian field with Matern covariance function}
\subsection{Classical measurement error model}
\subsection{Berkson measurement error model}
