...
This chapter discusses the definition of random effects models in INLA.
Going back to Equation~\ref{eq:model_linPred}, we will describe how
to define general Gaussian random effects models of the form
\begin{equation*}
  \mm{w} f(\mm{c}; \mm{\theta}),
\end{equation*}
...
The {\tt f()}-function takes as first argument the covariate that should
be modeled. To define an independent and identically distributed random
effect for each observation. This is simply an index $c=1,2,\ldots,n$,
with $n$ denoting the total number of observations, leading to:
\begin{verbatim}
	f(c, model="iid")
\end{verbatim}
A smooth effect of a covariate $\mm{c}=(c_1, \ldots, c_n)^\top$, assuming
covariate values $c_i$ for observation $i$, can be obtained by
\begin{verbatim}
	f(c, model="rw2")
\end{verbatim}
where we specify a random walk of order 2 for covariate $\mm{c}$.
...
for which we want estimate an effect,  the optional field
...
The hyperparameter specification and default values of the model (4) can be asked for within {\tt R} using the function {\tt inla.model.properties(<identifier>, "latent")}.
