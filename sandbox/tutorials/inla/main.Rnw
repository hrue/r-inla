...
A random effects model is specified using the {\tt f()}-function:
\begin{verbatim}
	f(<covariate>, [<weights>], model=<identifier>, hyper, ...)
\end{verbatim}
The first argument is always the name of the covariate, and the
second argument might be a vector of known weights (one for each
covariate value). The entry {\tt model} specifies which latent model
should be used (for available model see Table~\ref{tab:latentModels}).
The entry {\tt hyper} is used to specify details of the hyperparameters.
...
\end{center}
\end{table}
Specific documentation for a model can be found on \url{http://www.r-inla.org/models/latent-models}. Within {\tt R} model documentation can be requested using the function {\tt inla.doc(<identifier>)}, e.g.~{\tt inla.doc("rw1")},
% , for example
