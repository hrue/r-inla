...
static void
taucs_gemm_NCm1p1_small(int m, int n, int k, double * A, int lda, double * B, int ldb, double * C, int ldc)
{
	int j, i, l;
	double *Cj;
	double *Ail;
...
taucs_cilk_gemm(char *transa, char *transb,
		...
		double * A, int *plda,
		double * B, int *pldb, taucs_real_datatype * beta, double * C, int *pldc)
{
	int m = *pm;
	int n = *pn;
	int k = *pk;
	...
	if (n <= TAUCS_THRESHOLD_GEMM_BLAS && k <= TAUCS_THRESHOLD_GEMM_BLAS) {
		 ...
		 */
		taucs_gemm(transa, transb, pm, pn, pk, alpha, A, plda, B, pldb, beta, C, pldc, F_ONE, F_ONE);
		return;
	}
	...
	if (k >= n && k >= m) {
		 ...
		 */
		spawn taucs_cilk_gemm(transa, transb, pm, pn, &khalf1, alpha, A, plda, B, pldb, beta, C, pldc, F_ONE, F_ONE);

		sync;
		spawn taucs_cilk_gemm(transa, transb,
				      pm, pn, &khalf2, alpha, A + khalf1 * lda, plda, B + khalf1 * ldb, pldb, beta, C, pldc, F_ONE, F_ONE);
		sync;
		return;
	}
	...
	if (n >= k && n >= m) {
		...
		spawn taucs_cilk_gemm(transa, transb, pm, &nhalf1, pk, alpha, A, plda, B, pldb, beta, C, pldc, F_ONE, F_ONE);

		spawn taucs_cilk_gemm(transa, transb,
				      pm, &nhalf2, pk, alpha, A, plda, B + nhalf1, pldb, beta, C + nhalf1 * ldc, pldc, F_ONE, F_ONE);
		sync;
		return;
	}
	...
	if (1 /* m >= k && m >= n */ ) {		       /* the condition must be true */
		...
		spawn taucs_cilk_gemm(transa, transb, &mhalf1, pn, pk, alpha, A, plda, B, pldb, beta, C, pldc, F_ONE, F_ONE);

		spawn taucs_cilk_gemm(transa, transb, &mhalf2, pn, pk, alpha, A + mhalf1, plda, B, pldb, beta, C + mhalf1, pldc, F_ONE, F_ONE);
...
static void taucs_herk_LNm1p1_small(int n, int k, double * A, int lda, double * C, int ldc)
{
	int j, i, l;
	double *Cj;
	double *Ail;
...
taucs_cilk_herk(char *uplo, char *trans,
		int *pn, int *pk,
		taucs_real_datatype * alpha,
		double * A, int *plda, taucs_real_datatype * beta, double * C, int *pldc)
{
	int n = *pn;
	int k = *pk;
	...
	if (n <= TAUCS_THRESHOLD_HERK_BLAS && k <= TAUCS_THRESHOLD_HERK_BLAS) {
		 ...
		 */
		taucs_herk(uplo, trans, pn, pk, alpha, A, plda, beta, C, pldc, F_ONE, F_ONE);
		return;
	}
	...
	if (k > n) {
		 ...
		 */
		spawn taucs_cilk_herk(uplo, trans, pn, &khalf1, alpha, A, plda, beta, C, pldc, F_ONE, F_ONE);

		sync;
		spawn taucs_cilk_herk(uplo, trans, pn, &khalf2, alpha, A + khalf1 * lda, plda, beta, C, pldc, F_ONE, F_ONE);
		 ...
		 */
		spawn taucs_cilk_herk(uplo, trans, &nhalf1, pk, alpha, A, plda, beta, C, pldc, F_ONE, F_ONE);

		spawn taucs_cilk_gemm("N", "C",
				      &nhalf2, &nhalf1, pk,
				      &taucs_minusone_const, A + nhalf1, plda, A, plda, &taucs_one_const, C + nhalf1, pldc, F_ONE, F_ONE);

		spawn taucs_cilk_herk(uplo, trans, &nhalf2, pk, alpha, A + nhalf1, plda, beta, C + nhalf1 * ldc + nhalf1, pldc, F_ONE, F_ONE);
...
static void taucs_trsm_RLCNp1_small(int m, int n, double * A, int lda, double * B, int ldb)
{
	int j, i, k;
	double *Bi;
	double *Bik;
...
taucs_cilk_trsm(char *side, char *uplo, char *transa, char *diag,
		int *pm, int *pn, double * alpha, double * A, int *plda, double * B, int *pldb)
{
	int n = *pn;
	int m = *pm;
	...
	if (m <= TAUCS_THRESHOLD_TRSM_BLAS && n <= TAUCS_THRESHOLD_TRSM_BLAS) {
		 ...
		 */
		taucs_trsm(side, uplo, transa, diag, pm, pn, alpha, A, plda, B, pldb, F_ONE, F_ONE, F_ONE, F_ONE);
		return;
	}
	...
	if (m >= n) {
		 ...
		 */
		spawn taucs_cilk_trsm(side, uplo, transa, diag, &mhalf1, pn, alpha, A, plda, B, pldb, F_ONE, F_ONE, F_ONE, F_ONE);
		spawn taucs_cilk_trsm(side, uplo, transa, diag, &mhalf2, pn, alpha, A, plda, B + mhalf1, pldb, F_ONE, F_ONE, F_ONE, F_ONE);
		 ...
		 */
		spawn taucs_cilk_trsm(side, uplo, transa, diag, pm, &nhalf1, alpha, A, plda, B, pldb, F_ONE, F_ONE, F_ONE, F_ONE);

		sync;
		spawn taucs_cilk_gemm("N", "C",
				      pm, &nhalf2, &nhalf1,
				      &taucs_minusone_const, B, pldb, A + nhalf1, plda, &taucs_one_const, B + nhalf1 * ldb, pldb, F_ONE, F_ONE);
		sync;
		spawn taucs_cilk_trsm(side, uplo, transa, diag,
				      pm, &nhalf2, alpha, A + nhalf1 + nhalf1 * lda, plda, B + nhalf1 * ldb, pldb, F_ONE, F_ONE, F_ONE, F_ONE);
		sync;
		return;
	}
}
...
*/
static int taucs_potrf_lower_small(int n, double * A, int lda)
{
	int j, i, k;
	double *Aj;
	double *Ajk;
...
cilk static void taucs_cilk_potrf(char *uplo, int *pn, double * A, int *plda, int *pinfo)
{
	int n = *pn;
	int nhalf1, nhalf2;
	...
	if (n <= TAUCS_THRESHOLD_POTRF_BLAS) {
		 ...
		 */
		taucs_potrf(uplo, pn, A, plda, pinfo, F_ONE);
		return;
	}
	...
	spawn taucs_cilk_potrf(uplo, &nhalf1, A, plda, pinfo, F_ONE);
	...
	spawn taucs_cilk_trsm("R",
			      ...
			      "C",
			      "N",
			      &nhalf2, &nhalf1, &taucs_one_const, A, plda, A + nhalf1, plda, F_ONE, F_ONE, F_ONE, F_ONE);
	sync;
	/*
	 * taucs_trsm ("Right", "Lower", "Conjugate", "No unit diagonal", &nhalf2,&nhalf1, &taucs_one_const, A, plda,
	 * A+nhalf1,plda); 
	 */
	...
	spawn taucs_cilk_herk("L",
			      ...
			      A + nhalf1, plda, &taucs_one_real_const, A + nhalf1 + (nhalf1 * *plda), plda, F_ONE, F_ONE);
	sync;

	spawn taucs_cilk_potrf(uplo, &nhalf2, A + nhalf1 + (nhalf1 * *plda), plda, pinfo, F_ONE);
...
static void
multifrontal_supernodal_front_extend_add_ORIG(supernodal_frontal_matrix * parent_mtr, supernodal_frontal_matrix * my_mtr, int *bitmap)
{
		...
		for (i = j; i < my_mtr->up_size; i++) {
			...
			if (parent_j < parent_mtr->sn_size) {
				if (parent_i < parent_mtr->sn_size) {
					(parent_mtr->SFM_F1)[(parent_mtr->sn_size) * parent_j + parent_i] = (parent_mtr->SFM_F1)[(parent_mtr->sn_size) * parent_j + parent_i] + v;
				} else {
					(parent_mtr->SFM_F2)[(parent_mtr->up_size) * parent_j + (parent_i - parent_mtr->sn_size)] = (parent_mtr->SFM_F2)[(parent_mtr->up_size) * parent_j + (parent_i - parent_mtr->sn_size)] + v;
				}
			} else {
				(parent_mtr->SFM_U)[(parent_mtr->up_size) * (parent_j - parent_mtr->sn_size) + (parent_i - parent_mtr->sn_size)] = (parent_mtr->SFM_U)[(parent_mtr->up_size) * (parent_j - parent_mtr->sn_size) + (parent_i - parent_mtr->sn_size)] + v;
			}
		}
	}
}
	...
	if (0) {
		for (j = 0; j < my_mtr->up_size; j++) {
			...
			for (i = j; i < my_mtr->up_size; i++) {
				...
				if (parent_j < sn_size) {
					if (parent_i < sn_size) {
						F1[sn_size * parent_j + parent_i] += v;
					} else {
						int off_i = parent_i - sn_size;
						F2[up_size * parent_j + off_i] += v;
					}
				} else {
					int off_i = parent_i - sn_size;
					int off_j = parent_j - sn_size;
					U[up_size * off_j + off_i] += v;
				}
			}
		}
	} else {
    ...
    )
{
	...
	for (c_sn = sn_first_child[sn]; c_sn != -1; c_sn = sn_next_child[c_sn]) {
		c_znz[i] = recursive_amalgamate_supernodes(c_sn, n_sn, sn_size, sn_up_size, sn_rowind, sn_first_child, sn_next_child, rowind,	/* temporary 
																		 */
							   column_to_sn_map, map, do_order, ipostorder);
		assert(c_znz[i].zeros + c_znz[i].nonzeros == (double) (((sn_up_size[c_sn] - sn_size[c_sn]) * sn_size[c_sn])
...

	{
		...
		/*
		 * for (ip=0; ip<new_sn_up_size; ip++) printf("row %d zcount = %.0f\n",rowind[ip],zcount[rowind[ip]]); 
		 */
	...
	sn_rowind[sn] = (int *) taucs_realloc(sn_rowind[sn], new_sn_up_size * sizeof(int));
	for (ip = 0; ip < new_sn_up_size; ip++)
	...
	/*
	 * printf("supernode %d out of %d (done)\n",sn,*n_sn); printf("returning, merging\n"); 
	 */
	return merged_znz;
}
										    ...
										    supernodal_factor_matrix * snL, int *fail)
{
	...
	inlet void extend_add_inlet(supernodal_frontal_matrix * child_matrix) {
		...
		supernodal_frontal_free(child_matrix);
		/*
		 * The following approach is not working correctly because nothing is guaranteed about different procedure
		 * instances atomcity:
		 * 
...
#ifdef TAUCS_CILK
		extend_add_inlet(spawn recursive_multifrontal_supernodal_factor_llt(child, FALSE, bitmaps, A, snL, fail));
#else
								     ...
								     supernodal_factor_matrix * snL, int *fail)
{
...
#if 0
	context = Cilk_init(&argc, argv);
	always_null = EXPORT(recursive_multifrontal_supernodal_factor_llt) (context, n_sn, TRUE, maps, A, snL, fail);
	Cilk_terminate(context);
#else
	always_null = spawn recursive_multifrontal_supernodal_factor_llt(n_sn, TRUE, maps, A, snL, fail);
	sync;
#endif
	...
	/*
	 * always_null = spawn recursive_multifrontal_supernodal_factor_llt((L->n_sn), TRUE, map, A,L,&fail); 
	 */
}
...
cilk void *taucs_dtl(ccs_factor_llt_mf_maxdepth) (taucs_ccs_matrix * A, int max_depth) {
	...
	fail = taucs_ccs_symbolic_elimination(A, L, TRUE /* sort, to avoid complex conjuation */ ,
					      max_depth);
#else
...
recursive_leftlooking_supernodal_update(int J, int K,
					int bitmap[],
					double * dense_update_matrix, taucs_ccs_matrix * A, supernodal_factor_matrix * L)
{
	int i, j, ir;
	int child;
	...
	/*
	 * for(i=0;i<sn_size_father;i++) { bitmap[L->sn_struct[J][i]]=i+1; }
	 * 
	 * for(i=sn_size_father;i<sn_up_size_father;i++) bitmap[L->sn_struct[J][i]] = i - sn_size_father + 1; 
	 */
	...
	if (exist_upd) {
		...
		/*
		 * for(i=0;i<sn_up_size_father;i++) bitmap[L->sn_struct[J][i]]=0; 
		 */
	...
	/*
	 * else for(i=0;i<sn_up_size_father;i++) bitmap[L->sn_struct[J][i]]=0; 
	 */
						       ...
						       int *bitmap, int *indmap, taucs_ccs_matrix * A, supernodal_factor_matrix * L)
{
	...
	if (!is_root) {
		...
		if (L->sn_size[sn]) {
			(L->sn_blocks)[sn] = (double *) taucs_calloc(((L->sn_size)[sn]) * ((L->sn_size)[sn]),
									     sizeof(double));
			if (!((L->sn_blocks)[sn]))
	...
	for (child = first_child[sn]; child != -1; child = next_child[child]) {
		...
		if (!is_root) {
			if (!dense_update_matrix) {
				dense_update_matrix =
				    (double *) taucs_calloc((L->sn_up_size)[sn] * (L->sn_size)[sn], sizeof(double));
				if (!dense_update_matrix)
...
int taucs_dtl(supernodal_solve_llt) (void *vL, void *vx, void *vb) {
	...
	y = taucs_malloc((L->n) * sizeof(double));
	t = taucs_malloc((L->n) * sizeof(double));
	if (!y || !t) {
		taucs_free(y);
		taucs_free(t);
...
int taucs_ccs_symbolic_elimination(taucs_ccs_matrix * A, void *vL, int do_order, int max_depth)
{
	...
	if (0) {
		...
		cc1 = (int *) taucs_calloc((A->n), sizeof(int));
		cc2 = (int *) taucs_calloc((A->n), sizeof(int));
		rc1 = (int *) taucs_calloc((A->n), sizeof(int));
		rc2 = (int *) taucs_calloc((A->n), sizeof(int));
		p1 = (int *) taucs_calloc((A->n), sizeof(int));
...

	{
		...
		for (sn = 0; sn < (L->n_sn); sn++) {
			...
			for (i = 0, colnnz = (L->sn_up_size)[sn]; i < (L->sn_size)[sn]; i++, colnnz--) {
				/*
				 * There was a bug here. I did not count muliply-adds in the update part of the computation as 2
				 * flops but one. 
				 */
				/*
	...
	L->sn_blocks_ld = taucs_calloc((L->n_sn), sizeof(int));
	L->sn_blocks = taucs_calloc((L->n_sn), sizeof(double *));	/* so we can free before allocation */

	L->up_blocks_ld = taucs_calloc((L->n_sn), sizeof(int));
	L->up_blocks = taucs_calloc((L->n_sn), sizeof(double *));
