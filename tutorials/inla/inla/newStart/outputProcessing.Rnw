...
cat("\\begin{figure}[h]\n")
cat("\\label{output:fig1}\n")
for(fig in figs) {
    cat("  \\subfigure[]{\\rotatebox{0}{")
    cat("\\includegraphics[height=0.45\\linewidth]{", fig, "}}}\n", sep="")
}
cat("\\end{figure}\n")
@ 
%%
...
fixed effects, the statistics are stored in a matrix
...
data.frames\footnote{HR: WHY don't we use data.frames for all summaries?}
<<>>=
...
\section{Transforming posterior marginal distributions}
Write about
<<eval=FALSE>>=
inla.tmarginal
@

\section{Deriving posterior quantitites}
Write about
<<eval=FALSE>>=
inla.dmarginal
inla.pmarginal
...
\section{Sampling from the posterior marginal and posterior distribution}
Write about
<<eval=FALSE>>=
inla.rmarginal
