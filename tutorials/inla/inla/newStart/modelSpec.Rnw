...
# Specify the model and run inla
formula = y ~ 1 + x + f(t , model = "ar1" )
result = inla(formula, data = list(x = x, y = y, t = t), family ="gaussian")
@

Using the \tv{summary()} function we can expect the parameter estimates. Of note
in the summary only estimates for the fixed effects here $\beta_0$ and
$\beta_1$ as well as the three hyperparameters are given. Section~TODO disusses
