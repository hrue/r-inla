...
Getting the marginals distributions 
...
\begin{figure}\centering
<<vstval,echo=FALSE,fig=TRUE,eps=FALSE,width=5.5,height=5>>=
<<stvalplot>>
@
...
as we have in some other examples in Chapters \ref{ch:lowst} and \ref{ch:stpp}. 
...
To sample from the model, we define a space-time separabel covariance function, 
...
<<sample>>=
kappa.s <- 3; kappa.t <- 5; s2 <- 1/2; taue <- 3
y <- chol(stcov(loc, time, kappa.s, kappa.t, s2))%*%rnorm(n) + 
    rnorm(n, 0, sqrt(1/taue))
@ 
...
We build the mesh based in the domain and points
<<cmesh>>=
(mesh.s <- inla.mesh.2d(loc, max.edge=1.5, cutoff=0.25, offset=1.5))$n
@ 
We use the spatial mesh built in Chapter~\ref{ch:mesh}  
%<<prmesh1>>=
%load('prmesh1.RData') 
%plot(mesh.s, asp=1); points(loc, pch=19, cex=0.5)
%@ 
and define the SPDE model object 
<<cspde>>=
...
<<apred>>=
A <- inla.spde.make.A(mesh=mesh.s, loc=loc, 
...
Getting the marginals distributions for random field parameters in the user scale:
<<userrfc>>=
rf <- inla.spde2.result(res, 'i', spde, do.transf=TRUE) 
@ 

These distributions are showed in Figure~\ref{fig:stcpost}, 
as well also the marginal ditribution for the intercept, error precision and the temporal correlation, 
...
abline(v=taue, col=2)
plot(res$marginals.hyper[[4]], type='l', 
     xlab=expression(beta), ylab='Density')
abline(v=exp(-kappa.t*diff(mesh.t$loc[1:2])), col=2)
for (j in 15:17) {
    plot(rf[[j]][[1]], type='l', ylab='Density', 
         xlab=substring(names(rf)[j],11))
    abline(v=c(kappa.s,s2,sqrt(8)/kappa.s)[j-14], col=2)
}
@ 
...
\caption{Marginal posterior distribution for the intercept, 
  likelihood precision and the parameters in the spatio-temporal process.}
\label{fig:stcpost}
\end{figure}
