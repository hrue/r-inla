...
We need an additional carefull 
in the data management. 
Because we want to estimate the effect 
of the covariate with the copy strategy, 
we need that the index on the both 
random field math the random field
<<dat>>=
...
form <- Y ~  0 + f(m, model=spde) + 
  a.y + f(x, model=spde) + f(c.y, copy='m', fixed=FALSE, 
               hyper=list(theta=list(param=c(-3,25), initial=1))) 
@ 
