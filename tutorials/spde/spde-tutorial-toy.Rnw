...
data(SPDEtoy)
@ 
This dataset is a three columns \code{data.frame} 
with the coordinates in the first two clomuns and the 
response in the third column. 
<<strdata>>=
str(SPDEtoy)
@ 
...
y|\beta_0,x,\sigma_e^2 \sim N(\beta_0 + \bA x, \sigma_e^2) \\
\bx \sim GF(0, \Sigma)
\end{array}\end{equation}
where $\beta_0$ is the intercept, 
$\bA$ is the projection matrix and 
$x$ is a spatial Gaussian random field. 

The first step to work with the SPDE approach is to build a mesh. 
We will use as a mesh a collection of triangles, 
formed by $m$ vertices and the edges connecting them. 
This mesh has to include the spatial domain of interest. 
More details on the mesh building is given in 
Section~\ref{sec:mesh}. 
We will use the fifth mesh built in Section~\ref{sec:mesh}. 
<<loadmeshes,echo=F>>=
for (i in 1:6)
  load(paste('mesh', i, '.RData', sep=''))
@ 

The SPDE model in the original parametrization can be built 
...
However, we will choose the parametrization 
considering the marginal variance and 
the practical range, $\sqrt{8\nu}/\kappa$, instead. 
For details on this parametrization please have a 
look in \cite{lindgren:2012}. 
Additionally, wen defining the SPDE model 
we do need to set priors for both the parameters.  
The smoothness parameter $\nu$ is fixed by 
fixing $\alpha=\nu+d/2 \in [1,2]$. 
The \texttt{inla.spde2.pcmatern()} 
considers this parametrization and set the PC-prior 
derived in \cite{fuglstadetal:2017}. 
Our domain is the $[0,1]\times [0,1]$ square 
and we set the prior median to $0.3$. 
For the marginal standard deviation we set 
the probability of it been bigger than 1 to 1\%. 
...
The second step is to build the projector matrix. 
This will project the random field modeled at the 
mesh nodes, see section~\ref{sec:secres}. 
...
The \texttt{inla.stack()} function is usefull to organize 
the data, covariates, indexes and projector matrices. 
This is very useful to work with SPDE models.
It allows one to control how effects are 
linked to the linear predictor. 
...
In the toy example we have a linear predictor that can 
be written as 
\[
\eta^{*} = \b1\beta_0 + \bA\bx
\]
where the first part of the right side is 
a representation for the intercept 
and the last part is a representation for the spatial effect. 
That is, each one is represented as a product of a projector 
