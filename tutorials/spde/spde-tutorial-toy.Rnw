\section{The SPDE approach with INLA: a toy example}
...
<<readdata>>=
str(dat1 <- read.csv2("data/toydat.csv")); attach(dat1)
@ 
...
require(geoR)
lk <- likfit(as.geodata(dat1), ini=c(5,.1), nugget=.1, kappa=1)
lk.est <- c(lk$beta, lk$nugget, lk$cov.pars)
lk.est[4] <- 1/lk.est[4]
@ 

\subsection{The triangulation}\label{sec:mesh}
...
Firstly, we defines the domain and \code{attach} the data
<<domain>>= 
pl01 <- matrix(c(0,1,1,0,0, 0,0,1,1,0), ncol=2)
@ 
and create some triangulations with code bellow:
<<mesh5ab>>=
m1 <- inla.mesh.create.helper(cbind(s1, s2)[1:5, 1:2], max.edge=c(1,1))
m2 <- inla.mesh.create.helper(cbind(s1, s2)[1:5, 1:2], max.edge=c(.3,1))
m3 <- inla.mesh.create.helper(, pl01, max.edge=c(1,1))
m4 <- inla.mesh.create.helper(cbind(s1, s2)[1:5, 1:2], pl01, max.edge=c(.3,1))
m5 <- inla.mesh.create.helper(cbind(s1, s2)[1:5, 1:2], pl01, 
...
for (i in 1:6) { 
  plot(dat1[1:5,1:2], xlim=c(-.4,1.4), asp=1, pch=19, axes=FALSE, col=2)
  plot(get(paste('m', i, sep='')), add=TRUE) 
  points(dat1[1:5, 1:2], pch=19, col=2); lines(pl01, col=4)
}
@ 
...
<<mesh1>>=
mesh1 <- inla.mesh.create.helper(cbind(s1, s2), max.edge=c(0.1, 0.2))
...
mesh6 <- inla.mesh.create.helper(cbind(s1, s2), max.edge=c(0.04, 0.1))
...
for (i in 1:6) {
  ...
  points(dat1[,1:2], pch=19, col=2, cex=dat1[,3]/10)
}
@ 
\setkeys{Gin}{width=0.99\textwidth}
\begin{figure}\centering
...
\subsubsection{Simple estimation with mesh with points together}
...
res1 <- inla(y ~ f(i, model=spde1), 
             ...
             data=data.frame(y=y, i=1:nrow(dat1)))
@ 
...
\subsubsection{When locations are not vertices of triangulation}
\label{sec:ptsnomesh}
...
<<proj2>>=
dim(A2 <- inla.spde.make.A(mesh2, loc=as.matrix(dat1[,1:2])))
@ 
...
res2 <- inla(resp ~ 0 +  m + f(i, model=spde2), 
             data=list(resp=y, i=1:mesh2$n, m=rep(1,mesh2$n)), 
...
\subsubsection{The stack functionality}\label{sec:stack}
...
on this mesh 
<<a3>>=
A3 <- inla.spde.make.A(mesh3, loc=as.matrix(dat1[,1:2]))
@ 
...
<<stackdata>>=
st3.dat <- inla.stack(data=list(resp=dat1$y), A=list(A3), 
...
## 4
A4 <- inla.spde.make.A(mesh4, loc=as.matrix(dat1[,1:2]))
ind4 <- inla.spde.make.index(name='i', n.mesh=mesh4$n)
st4.dat <- inla.stack(data=list(resp=dat1$y), A=list(A4, 1), 
                        effects=list(ind4, m=rep(1,nrow(dat1))))
...
### 5
A5 <- inla.spde.make.A(mesh5, loc=as.matrix(dat1[,1:2]))
ind5 <- inla.spde.make.index(name='i', n.mesh=mesh5$n)
st5.dat <- inla.stack(data=list(resp=dat1$y), A=list(A5, 1), 
                        effects=list(ind5, m=rep(1,nrow(dat1))))
...
### 6
##A6 <- inla.spde.make.A(mesh6, loc=as.matrix(dat1[,1:2]))
##ind6 <- inla.spde.make.index(name='i', n.mesh=mesh6$n)
##st6.dat <- inla.stack(data=list(resp=dat1$y), A=list(A6, 1), 
  ##                      effects=list(ind6, m=rep(1,nrow(dat1))))
...
\subsection{Comparing different triangulations}\label{sec:meshcompare}
