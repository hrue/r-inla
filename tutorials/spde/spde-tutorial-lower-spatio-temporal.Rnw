\chapter{Lowering resolution of a spatio-temporal model}\label{ch:lowst}


\SweaveOpts{prefix.string=figs/lowst}
...
\section{Data temporal aggregation}
...
\section{Lowering temporal model resolution}

This approach is better expained in\cite{blangiardoC:2015}. 
The main idea is to place some knots over the time window
and define a model on such knots. 
Them project the model into the data time points as we
do using the Finite Element Method in the SPDE approach. 
...
<<lowstres,echo=F,fig=T,eps=F,width=10,height=8.7>>=
<<plt>>
@
