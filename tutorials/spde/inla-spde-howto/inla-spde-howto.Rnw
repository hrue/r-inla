...
A little warning about the triangle on the mesh. 
Is good to have they aproximately isosceles. 
Also, we have to look at the range of the process 
and compare it with the edges lengths 
of the inner mesh triangles. 
We need the lengths of the edges to be 
less than the range of the process. 
If it is too small, there might 
not be any spatial effect. 
...
plot(rf$marginals.var[[1]], ty='l', 
     xlab=expression(sigma[rf]^2), yla='Density')
abline(v=s2rf, col=2) ## add the true value
