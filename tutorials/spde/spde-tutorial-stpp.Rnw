\chapter{Spatio temporal log-Cox process}\label{ch:stpp}

\SweaveOpts{prefix.string=figs/stpp} 
...
In this chapter we show how to fit a 
spatio temporal log-Cox point process model. 
In Section~\ref{sec:burkitt} we have an example. 
In Section~\ref{sec:lstpp} we show an approach to deal with large data sets.

\input{spde-tutorial-burkitt}

\section{Large data set}\label{sec:lstpp} 
...
<<stfit,echo=F,fig=T,eps=F,width=10,height=4>>=
<<stplot>>
@ 
...
\subsection{Spacetime aggregation}
...
<<ppinsp>>=
area <- over(SpatialPoints(cbind(xyt$x, xyt$y)), polys)
@ 
...
table(time <- findInterval(xyt$t, t.breaks))
@ 
...
The projector matrix, SPDE model object and the spacetime index set definition:
<<spde>>=
...
\setkeys{Gin}{width=0.99\textwidth}
\begin{figure}[h]\centering
<<lstsresf,echo=F,fig=T,eps=F,width=5,height=5>>=
<<lstsresp>>
@ 
