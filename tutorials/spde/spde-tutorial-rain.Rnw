...
<<stackpr>>=
stk.dat <- inla.stack(data=list(y=PRprec$precMean), A=list(A,1,1,1), 
                      effects=list(i=1:spde$n.spde, Intercept=rep(1, nrow(A)), 
...
<<stkprd>>=
stk.prd <- inla.stack(data=list(y=NA), A=list(Aprd,1,1), 
                      effects=list(i=1:spde$n.spde, 
                        Intercept=rep(1,nrow(Aprd)), 
