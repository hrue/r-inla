...
We have daily rainfall data on first 90 days of the year 
2011 on 612 locations, including stations within the 
Paran\'a state and around its border. 

We read the data and the border of Paran\'a state with
<<start,results=hide>>=
...
and more 90 columns, one for each day of January to march, 
with daily accumulated precipitation. 
...
<<headdat>>=
prec[ii <- c(which(is.na(prec$A))[which.min(prec$S[is.na(prec$A)])], 
...
<<distseacalc>>=
prec$"seaDist" <- nndistF(as.matrix(pr.bord[1034:1078,]), 
                          as.matrix(prec[,1:2]))
@ 
...
\Sexpr{format(nrow(pr.bord))} 
coordinates, that form the segments. 
The half of this segments are separated by less than 
\Sexpr{format(median(sqrt(rowSums((pr.bord[-1,]-pr.bord[-nrow(pr.bord),])^2))))}
 Kilometer and we have segments up to
\Sexpr{format(max(sqrt(rowSums((pr.bord[-1,]-pr.bord[-nrow(pr.bord),])^2))))}
Kilometers. 
<<distbord>>=
...
\subsection{Testing the significance of spatial effect}
...
at north east part of the Paran\'a state.
