...
\[
log(\mu_i) = \alpha + x_i + \sum_{k=1}^m I_{ki} f_k(\tilde{F}_i^{(k)}) \\
\]
were 
$\alpha$ is the intercept, 
$x_i$ is the spatial random effect 
and $\tilde{F}_i^{(k)}$ is the mean of the sea distance(or longitude) 
in the class $k$ out of $m$ classes chosen 
considering the \texttt{inla.group()} function, 
$I_{ki}$ indicates if the covariate for the observation $i$ belongs to class $k$. 
...
plot(r.sead$summary.random[[1]][,1:2], type='l', 
     xlab='Distance to sea (Km)', ylab='Effect'); abline(h=0, lty=3) 
...
The approach in this section does not always makes sense!
However, in some SPECIFIC cases it makes sense to interpolate the linear predictor. 
This cases happens when whe only have the spatial effect and 
enviromental covariates which are smooth over space. 
The advantage is that is computationally cheap than to compute the full posterior marginals like in the previous section.
...
dsmesh <- apply(spDists(PRborder[1034:1078,], 
                        prmesh2$loc[,1:2], longlat=TRUE), 2, min)
