...
we can use the \pkg{gpclib} package instead, 
but the we do need to set a permission 
<<uionsp,results=hide,eval=F>>=
gpclibPermit()
@ 
...
\setkeys{Gin}{width=0.7\textwidth}
\begin{figure}\centering
<<plotlake,echo=F,results=hide,fig=TRUE,eps=FALSE,width=7,height=5>>=
<<hexample>>
@ 
...
\setkeys{Gin}{width=0.7\textwidth}
\begin{figure}\centering
<<plothex,echo=F,results=hide,fig=TRUE,eps=FALSE,width=7,height=5>>=
<<holeexample>>
@ 
