...
\SweaveOpts{prefix.string=figs/survival} 
...
\section{Parametric survival model}\label{sec:parsurv} 
...
if (sum(ls()=='nwEngland')==0)
    load('~/inla-project/source/inla/rinla/data/Leuk.rda')
@ 
...
the 'weibull' likelihood. Hovewer, other parametric 
...
<<wsmap,echo=F,fig=T,eps=F,width=7,height=5.3>>= 
<<wscode>>
@ 
...
\section{Cox proportional hazard survival model}\label{sec:coxph} 
