...
a permission before the work with this library 
<<uionsp>>=
gpclibPermit()
...
\section{Mesh with holes and physical boundaries}
...
In the Figure~\ref{fig:lakehole} we have two neighbour regions, 
...
plot(sp) ### to visualize it
text(c(13, 17, 23), c(3, 12, 3), LETTERS[1:3], cex=3)
@ 

\setkeys{Gin}{width=0.99\textwidth}
\begin{figure}\centering
<<plotlake,echo=F,results=hide,fig=TRUE,eps=FALSE,width=5.5,height=3.5>>=
<<hexample>>
@ 
...
\setkeys{Gin}{width=0.99\textwidth}
\begin{figure}\centering
<<plothex,echo=F,results=hide,fig=TRUE,eps=FALSE,width=5.5,height=5.5>>=
<<holeexample>>
@ 
