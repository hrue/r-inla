...
In this Chapter we present the model proposed 
in~\cite{ingebrigtsenLS:2013}. 
To introduce variables in the covariance matrix 
...
This is a kind of non stationarity 
that allows variation in the variance and range 
over the domain. 
