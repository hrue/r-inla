\chapter{Explanatory variables in the covariance}
...
In this Chapter we present an example of the model proposed 
in~\cite{ingebrigtsenLS:2013}. 
This model consider that the two parameters in the SPDE 
can depends on a set of explanatory variables/covariates. 

First, we have to remember the definition for the precision 
...
The way it is done is to have a regression like model for 
...
the local precision depends in one of the coordinates. 
...
<<vvarns,echo=F,fig=T,eps=F,width=10,heigh=5.5>>=
<<varns>>
@ 
...
\section{Simulation at the mesh nodes}

Both precision matrix defined in previous 
...
However, in this section we would like to show the 
...
\section{Estimation with data simulated at the mesh nodes} 
...
\section{Estimation with locations not at the mesh nodes}
...
at the mesh vertices to this locations. 
