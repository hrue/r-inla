...
\section{The data and the model}

The data is called by
<<readdata>>=
data(PRprec) 
PRprec[1:3, 1:10] 
@ 
...
<<summ>>=
sapply(PRprec[,4:9], summary) 
apply(PRprec[,4:9]>0, 2, summary) 
@ 
...
par(mar=c(0,0,0,0)) 
plot(PRprec[,1:2], cex=0.3 + PRprec[,jday]/20, asp=1, col=z+1, axes=FALSE) 
...
<<rain>>
@ 
\caption{Histogram of positive precipitation 
...
<<A>>=
A <- inla.spde.make.A(prmesh2, loc=as.matrix(PRprec[,1:2])) 
@ 

Taking into account that we need to model two 
responses together, the response is a two column matrix. 
The data stack for the rain occurence is 
<<stack>>=
stk.z <- inla.stack(tag='est.z', 
                    data=list(y=cbind(z, NA)), ### first column 
                    A=list(A, 1), 
                    effects=list( 
...
<<xq,eval=F>>=
ordx <- order(res$summary.random$i.z$mean) 
par(mar=c(3,3,0.5,0.5), mgp=c(1.5,0.5,0), las=1) 
plot(res$summary.random$i.z$mean[ordx], type='l', ylab='x', 
     ylim=range(res$summary.random$i.z[, 4:6])) 
for (i in c(4,6)) lines(res$summary.random$i.z[ordx,i], lty=2) 
abline(h=0, lty=3) 
@ 
\begin{figure}\centering 
...
<<resfield>>=
res.f <- inla.spde2.result(res, 'i.z', spde, do.transf=TRUE) 
@ 
...
<<projpred>>=
xmean <- inla.mesh.project(projgrid, res$summary.random$i.z$mean) 
xsd <- inla.mesh.project(projgrid, res$summary.random$i.z$sd) 
@ 
