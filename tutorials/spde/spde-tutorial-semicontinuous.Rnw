...
one for each response and $2\times n$ rows. 
We put the first response on first column 
at first $n$ rows and the second response 
on last $n$ rows of second column.
<<Y>>=
n <- nrow(PRprec)
Y <- matrix(NA, 2*n, 2)
Y[1:n, 1] <- z
Y[n + 1:n, 2] <- y
@ 
later we associate, on the model formulae, 
one likelihood for each column.
...
<<>>=
(mesh <- inla.mesh.create.helper(, PRborder, cutoff=.15,
                                 max.edge=c(.15, .3), offset=c(.2,.3)))$n
@ 
...
@ 
The intercepts here are estimated in same way 
when we have covariate. In this case, the 
covariate is assigned $0$ (zero) at positions 
corresponding to the response unaffected by it. 
So, we have list of effects with length four, 
the index set of random effect on each response 
and the intercept on each response. 
A different way is used on Chapter~\ref{sec:prprec}. 
