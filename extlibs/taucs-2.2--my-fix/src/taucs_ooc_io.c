...
taucs_io_handle *taucs_io_create_multifile(char *basename)
{
	...
	nbytes = write(f, TAUCS_FILE_SIGNATURE, strlen(TAUCS_FILE_SIGNATURE));
	...
	h->nreads = h->nwrites = h->bytes_read = h->bytes_written = h->read_time = h->write_time = 0.0;
...
int taucs_io_append(taucs_io_handle * f, int index, int m, int n, int flags, void *data)
{
...
int taucs_io_write(taucs_io_handle * f, int index, int m, int n, int flags, void *data)
{
	int this_size;
	off_t this_offset;
	ssize_t nbytes;
...
int taucs_io_read(taucs_io_handle * f, int index, int m, int n, int flags, void *data)
{
	int this_size = 0;				       /* warning */
	off_t this_offset;
	ssize_t nbytes;
...
int taucs_io_close(taucs_io_handle * f)
{
	...
	if (f->type == IO_TYPE_SINGLEFILE) {
		 ...
		 */
		/*
		 * last_size = matrices[f->nmatrices-1].m * matrices[f->nmatrices-1].n * ((matrices[f->nmatrices-1].flags &
		 * TAUCS_INT) ? sizeof(int) : sizeof(double)); offset = matrices[f->nmatrices-1].offset + last_size;
		 */
	...
	if (f->type == IO_TYPE_MULTIFILE) {
		...
		for (i = 0; i < f->nmatrices; i++) {
			...
			if (curr_file_offset + (double) sizeof(int) < IO_FILE_RESTRICTION * 1024.0 * 1024.0) {
				...
				h->f[h->last_created_file] = file_id;
				nbytes =
				    write(h->f[h->last_created_file], (char *) &(matrices[i].m) + first_size,
					  sizeof(int) - first_size);
			...
			if (curr_file_offset + (double) sizeof(int) < IO_FILE_RESTRICTION * 1024.0 * 1024.0) {
				...
				h->f[h->last_created_file] = file_id;
				nbytes =
				    write(h->f[h->last_created_file], (char *) &(matrices[i].n) + first_size,
					  sizeof(int) - first_size);
				if (nbytes != sizeof(int) - first_size) {
					taucs_printf("taucs_close: Error writing data .\n");
					return -1;
			...
			if (curr_file_offset + (double) sizeof(int) < IO_FILE_RESTRICTION * 1024.0 * 1024.0) {
				...
				h->f[h->last_created_file] = file_id;
				nbytes =
				    write(h->f[h->last_created_file], (char *) &(matrices[i].flags) + first_size,
					  sizeof(int) - first_size);
				if (nbytes != sizeof(int) - first_size) {
					taucs_printf("taucs_close: Error writing data .\n");
					return -1;
			...
			if (curr_file_offset + (double) sizeof(double) < IO_FILE_RESTRICTION * 1024.0 * 1024.0) {
				...
				h->f[h->last_created_file] = file_id;
				nbytes =
				    write(h->f[h->last_created_file], (char *) &(matrices[i].offset) + first_size,
					  sizeof(double) - first_size);
...
taucs_io_handle *taucs_io_open_singlefile(char *filename)
{
	...
	hs->matrices = (taucs_io_matrix_singlefile *) taucs_malloc((h->nmatrices) * sizeof(taucs_io_matrix_singlefile));
...
taucs_io_handle *taucs_io_open_multifile(char *basename)
{
	...
	hs->matrices = (taucs_io_matrix_multifile *) taucs_malloc((h->nmatrices) * sizeof(taucs_io_matrix_multifile));
	...
	for (i = 0; i < h->nmatrices; i++) {
		...
		if ((int) (curr_file_offset + sizeof(int)) < IO_FILE_RESTRICTION * 1024 * 1024) {
			...
			hs->f[start_file_index] = file_id;
			nbytes =
			    read(hs->f[start_file_index], (char *) &hs->matrices[i].flags + first_size, sizeof(int) - first_size);
			if (nbytes != sizeof(int) - first_size) {
				taucs_printf("taucs_open: Error in open data .\n");
				return NULL;
			}
		...
		if ((int) (curr_file_offset + sizeof(double)) < IO_FILE_RESTRICTION * 1024 * 1024) {
			...
			hs->f[start_file_index] = file_id;
			nbytes =
			    read(hs->f[start_file_index], (char *) &hs->matrices[i].offset + first_size,
				 sizeof(double) - first_size);
			if (nbytes != sizeof(double) - first_size) {
				taucs_printf("taucs_open: Error in open data .\n");
				return NULL;
