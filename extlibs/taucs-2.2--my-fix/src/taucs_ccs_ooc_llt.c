...
static int radix_sort(unsigned int *x, int n)
{
	int i;
	unsigned int mask;
...
recursive_symbolic_elimination(int j,
			       ...
			       int map[],
			       int do_order,
			       int ipostorder[], double given_mem, void (*sn_struct_handler) (), void *sn_struct_handler_arg)
{
	int i, ip, c, c_sn;
	int in_previous_sn;
	...
	for (c = first_child[j]; c != -1; c = next_child[c]) {
		recursive_symbolic_elimination(c, A, first_child, next_child, n_sn, sn_size, sn_up_size, sn_rowind, sn_first_child, sn_next_child, rowind,	/* temporary 
																				 */
					       column_to_sn_map,
					       map, do_order, ipostorder, given_mem, sn_struct_handler, sn_struct_handler_arg);
	}
	...
	if (j < A->n) {
		...
		for (c = first_child[j]; c != -1; c = next_child[c]) {
			...
			if (sn_struct_handler)
				(*sn_struct_handler) (sn_struct_handler_arg, c_sn, sn_up_size[c_sn], &(sn_rowind[c_sn]));
		}
	...
	if (j < A->n) {
		...
		if (do_order) {
			 ...
			 */
			/*
			 * radix_sort(sn_rowind [*n_sn],nnz); qsort(sn_rowind [*n_sn],nnz,sizeof(int),compare_ints); 
			 */
...
taucs_ccs_ooc_symbolic_elimination(taucs_ccs_matrix * A,
				   ...
				   double given_mem, void (*sn_struct_handler) (), void *sn_struct_handler_arg)
{
	supernodal_factor_matrix *L = (supernodal_factor_matrix *) vL;
	int *first_child;
	int *next_child;
	...
	/*
	 * { int i; printf("c2sn: "); for (i=0; i<A->n; i++) printf("%d ",column_to_sn_map[i]); printf("\n"); } 
	 */
					...
					taucs_datatype * dense_update_matrix, taucs_ccs_matrix * A, supernodal_factor_matrix * L)
{
	...
	for (i = sn_size_child; i < sn_up_size_child; i++)
		...
		if (bitmap[L->sn_struct[K][i]]
			...
			/*
			 * for(j=0;j<sn_size_child;j++) for(ir=i;ir<sn_up_size_child;ir++) if( L->sn_struct[K][ir] <=
			 * L->sn_struct[J][sn_size_father-1]){ L->sn_blocks[J][
			 * (bitmap[L->sn_struct[K][i]]-1)*(L->sn_blocks_ld[J])+(bitmap[L->sn_struct[K][ir]]-1)] -=
			 * L->up_blocks[K][j*(L->up_blocks_ld[K])+ir-sn_size_child]*
			 * L->up_blocks[K][j*L->up_blocks_ld[K]+i-sn_size_child]; taucs_printf("sn_block: L[%d,%d] =
			 * %lf\n",(bitmap[L->sn_struct[K][ir]]-1),(bitmap[L->sn_struct[K][i]]-1),L->sn_blocks[J][
			 * (bitmap[L->sn_struct[K][i]]-1)*(L->sn_blocks_ld[J])+(bitmap[L->sn_struct[K][ir]]-1)]);} else{
			 * L->up_blocks[J][ (bitmap[L->sn_struct[K][i]]-1)*(L->up_blocks_ld[J])+(bitmap[L->sn_struct[K][ir]]-1)] -= 
			 * L->up_blocks[K][j*L->up_blocks_ld[K]+ir-sn_size_child]*
			 * L->up_blocks[K][j*L->up_blocks_ld[K]+i-sn_size_child]; taucs_printf("up_block: L[%d,%d] =
			 * %lf\n",(bitmap[L->sn_struct[K][ir]]-1),(bitmap[L->sn_struct[K][i]]-1),L->up_blocks[J][
			 * (bitmap[L->sn_struct[K][i]]-1)*(L->up_blocks_ld[J])+(bitmap[L->sn_struct[K][ir]]-1)]); }
			 */
		}
	...
	if (exist_upd) {
		...
		/*
		 * The GEMM code computes on the upper triangle of the trapezoidal matrix, which is junk. 
		 */
		/*
		 * taucs_gemm ("No Conjugate", "Conjugate", &M,&N,&PK, &taucs_one_const,
		 * &(L->up_blocks[K][first_row-sn_size_child]),&LDA, &(L->up_blocks[K][first_row-sn_size_child]),&LDB,
		 * &taucs_zero_const, dense_update_matrix,&LDC); 
		 */
		...
		taucs_herk("Lower",
			   ...
			   &(L->up_blocks[K][first_row - sn_size_child]), &LDA, &taucs_zero_real_const, dense_update_matrix, &LDC);
		...
		if (M - N > 0) {
			...
			taucs_gemm("No Conjugate",
				   ...
				   &taucs_zero_const, dense_update_matrix + N, &LDC);
		}
		/*
		...
		/*
		 * for(j=0;j<row_count;j++) for(ir=0;ir<sn_up_size_father;ir++) taucs_printf("dense[%d,%d] =
		...
		for (j = 0; j < row_count; j++)
			for (ir = row_count; ir < M; ir++) {
				 ...
				 * -= dense_update_matrix[j*LDC+ir]; 
				 */
				L->up_blocks[J][(bitmap[L->sn_struct[K][first_row + j]] - 1) * (L->sn_up_size[J] - L->sn_size[J]) +
						(bitmap[L->sn_struct[K][ir + first_row]] - 1)] =
				    taucs_sub(L->
					      up_blocks[J][(bitmap[L->sn_struct[K][first_row + j]] - 1) * (L->sn_up_size[J] -
													   L->sn_size[J]) +
							   (bitmap[L->sn_struct[K][ir + first_row]] - 1)],
					      dense_update_matrix[j * LDC + ir]);
			}
...
static int leftlooking_supernodal_front_factor(int sn, int *indmap, taucs_ccs_matrix * A, supernodal_factor_matrix * L)
{
	int ip, jp;
	int *ind;
	taucs_datatype *re;
	 ...
	 */
	if (sn_size)
		taucs_potrf("LOWER", &sn_size, (L->sn_blocks)[sn], &sn_size, &INFO);
	...
	if (up_size && sn_size)
		taucs_trsm("Right",
			   ...
			   "Conjugate",
			   "No unit diagonal",
			   &up_size, &sn_size, &taucs_one_const, (L->sn_blocks)[sn], &sn_size, (L->up_blocks)[sn], &up_size);
...
static int recursive_leftlooking_supernodal_factor_llt(int sn, /* this supernode */
						       ...
						       int *map, taucs_ccs_matrix * A, supernodal_factor_matrix * L)
{
	int child;
	int *first_child = L->first_child;
	int *next_child = L->next_child;
	...
	/*
	 * if (!is_root) sn_size = L->sn_size[sn]; else sn_size = -1;
	 * 
	 * if (!is_root) { (L->sn_blocks )[sn] = (double*)taucs_calloc(((L->sn_size)[sn])*((L->sn_size)[sn]),sizeof(double));
	 * 
	...
	for (child = first_child[sn]; child != -1; child = next_child[child]) {
		if (recursive_leftlooking_supernodal_factor_llt(child, FALSE, map, A, L)) {
			/*
			 * failure 
			 */
			return -1;
	...
	if (!is_root) {
		...
		if (!dense_update_matrix)
			dense_update_matrix = (taucs_datatype *) taucs_calloc((L->sn_up_size)[sn] * (L->sn_size)[sn],
									      sizeof(taucs_datatype));
		for (child = first_child[sn]; child != -1; child = next_child[child])
			recursive_leftlooking_supernodal_update(sn, child, map, dense_update_matrix, A, L);
...
void *taucs_ccs_factor_llt_ll(taucs_ccs_matrix * A)
{
	...
	taucs_ccs_ooc_symbolic_elimination(A, L, TRUE /* sort row indices */ ,
					   FALSE /* don't return col_tosn_map */ ,
					   1.0 / 0.0, NULL, NULL /* sn_struct handler */ );
	...
	if (recursive_leftlooking_supernodal_factor_llt((L->n_sn), TRUE, map, A, L)) {
...
static
void ooc_sn_struct_handler(void *argument, int sn, int sn_up_size, int *sn_struct_ptr[])
{
	taucs_io_handle *handle = (taucs_io_handle *) argument;

	taucs_io_append(handle, IO_BASE + sn,		       /* matrix written in postorder */
			1, sn_up_size, TAUCS_INT, *sn_struct_ptr);
...
recursive_leftlooking_supernodal_update_ooc(int J, int K,
					    ...
					    taucs_io_handle * handle, taucs_ccs_matrix * A, supernodal_factor_matrix * L)
{
	int i, j, ir;
	int child;
	int *first_child = L->first_child;
	...
	if (exist_upd) {
		(L->up_blocks)[K] = (taucs_datatype *) taucs_calloc(((L->sn_up_size)[K] - (L->sn_size)[K])
								    * ((L->sn_size)[K]), sizeof(taucs_datatype));
		taucs_io_read(handle, IO_BASE + L->n_sn + 2 * K + 1,
			      (L->sn_up_size)[K] - (L->sn_size)[K], (L->sn_size)[K], TAUCS_CORE_DATATYPE, (L->up_blocks)[K]);
		...
		for (j = 0; j < row_count; j++)
			for (ir = row_count; ir < M; ir++) {
				 ...
				 * -= dense_update_matrix[j*LDC+ir]; 
				 */
				L->up_blocks[J][(bitmap[L->sn_struct[K][first_row + j]] - 1) * ((L->sn_up_size)[J] -
												(L->sn_size)[J]) +
						(bitmap[L->sn_struct[K][ir + first_row]] - 1)] =
				    taucs_sub(L->
					      up_blocks[J][(bitmap[L->sn_struct[K][first_row + j]] - 1) * ((L->sn_up_size)[J] -
		...
		for (child = first_child[K]; child != -1; child = next_child[child]) {
			recursive_leftlooking_supernodal_update_ooc(J, child, bitmap, dense_update_matrix, handle, A, L);
		}
	} else {
		for (i = 0; i < sn_up_size_father; i++)
			bitmap[L->sn_struct[J][i]] = 0;
...
static int recursive_append_L(int sn,			       /* this supernode */
			      int is_root,		       /* is v the root? */
			      taucs_io_handle * handle, supernodal_factor_matrix * L)
{
	int child;
	...
	if (!is_root) {
		taucs_io_append(handle, IO_BASE + L->n_sn + 2 * sn,
				L->sn_size[sn], L->sn_size[sn], TAUCS_CORE_DATATYPE, L->sn_blocks[sn]);

		taucs_io_append(handle, IO_BASE + L->n_sn + 2 * sn + 1,
				L->sn_up_size[sn] - L->sn_size[sn], L->sn_size[sn], TAUCS_CORE_DATATYPE, L->up_blocks[sn]);
		taucs_free((L->sn_blocks)[sn]);
		taucs_free((L->up_blocks)[sn]);
		taucs_free((L->sn_struct)[sn]);
		(L->sn_blocks)[sn] = NULL;
...
static int recursive_read_L(int sn,			       /* this supernode */
			    int is_root,		       /* is v the root? */
			    taucs_io_handle * handle, supernodal_factor_matrix * L)
{
	int child;
...
static int recursive_read_L_cols(int sn,		       /* this supernode */
				 int is_root,		       /* is v the root? */
				 taucs_io_handle * handle, supernodal_factor_matrix * L)
{
	int child;
...
static int recursive_leftlooking_supernodal_factor_llt_ooc(int sn,	/* this supernode */
							   int is_root,	/* is v the root? */
							   int *map,
							   int *sn_in_core,
							   taucs_io_handle * handle,
							   taucs_ccs_matrix * A, supernodal_factor_matrix * L)
{
	int child;
	...
	for (child = first_child[sn]; child != -1; child = next_child[child]) {
		...
		} else if (recursive_leftlooking_supernodal_factor_llt_ooc(child, FALSE, map, sn_in_core, handle, A, L)) {
			/*
			 * failure 
			 */
			return -1;
	...
	if (!is_root) {
		...
		if (L->sn_up_size[sn] - L->sn_size[sn] > 0)
			(L->up_blocks)[sn] =
			    (taucs_datatype *) taucs_calloc(((L->sn_up_size)[sn] - (L->sn_size)[sn]) * ((L->sn_size)[sn]),
							    sizeof(taucs_datatype));
		...
		for (child = first_child[sn]; child != -1; child = next_child[child]) {
			recursive_leftlooking_supernodal_update_ooc(sn, child, map, dense_update_matrix, handle, A, L);
		}
...
static double recursive_compute_supernodes_ll_in_core(int sn,  /* this supernode */
						      int is_root,	/* is v the root? */
						      double avail_mem, int *sn_in_core, supernodal_factor_matrix * L)
{
	int child;
	...
	/*
	 * if (first_child[sn] == -1) total_mem =
	 * (double)(L->sn_size)[sn]*(double)(L->sn_up_size)[sn]*sizeof(taucs_datatype)+(double)(L->sn_up_size)[sn]*sizeof(int);
	 * else total_mem = children_mem +
	 * 2*(double)(L->sn_size)[sn]*(double)(L->sn_up_size)[sn]*sizeof(taucs_datatype)+(double)(L->sn_up_size)[sn]*sizeof(int);
	 * 
	 * if (total_mem <= avail_mem || first_child[sn] == -1) sn_in_core[sn] = 1; else sn_in_core[sn] = 0; return total_mem; 
	 */
	total_mem =
	    children_mem + (double) (L->sn_size)[sn] * (double) (L->sn_up_size)[sn] * sizeof(taucs_datatype) +
	    (double) (L->sn_up_size)[sn] * sizeof(int);

	if ((total_mem + (double) (L->sn_size)[sn] * (double) (L->sn_up_size)[sn] * sizeof(taucs_datatype)) <= avail_mem
	    || first_child[sn] == -1) {
		sn_in_core[sn] = 1;
		return total_mem;
	} else {
...
static int recursive_compute_supernodes_in_core_old(int sn,    /* this supernode */
						    int is_root,	/* is v the root? */
						    int avail_mem, int *sn_in_core, supernodal_factor_matrix * L)
{
	int child;
	...
	for (child = first_child[sn]; child != -1; child = next_child[child]) {
		child_mem = recursive_compute_supernodes_in_core_old(child, FALSE, curr_avail_mem, sn_in_core, L);
...
static void recursive_compute_supernodes_ipostorder(int sn,    /* this supernode */
						    int is_root,	/* is v the root? */
						    int *current_index_ptr, supernodal_factor_matrix * L)
{
	int child;
	int *first_child = L->first_child;
	int *next_child = L->next_child;
	...
	for (child = first_child[sn]; child != -1; child = next_child[child]) {
		recursive_compute_supernodes_ipostorder(child, FALSE, current_index_ptr, L);
...
/* no panelization */
#if 0
void *taucs_ccs_factor_llt_ll_ooc(taucs_ccs_matrix * A, double memory)
{
	supernodal_factor_matrix *L;
	int i, j, ip, jp;
	int sn, p;
	 ...
	 */
	memory_overhead = 4.0 * (double) ((A->n) * sizeof(int)) +	/* integer vectors in L */
	...
	if (memory - memory_overhead < 2.0 * (double) ((A->n) * sizeof(taucs_datatype)) + 2.0 * (double) ((A->n) * sizeof(int))) {
	...
	taucs_ccs_ooc_symbolic_elimination(A, L, TRUE /* sort row indices */ ,
					   ...
					   (memory - memory_overhead) / 3.0, ooc_sn_struct_handler, handle);

	/*
	 * we now compute an exact memory overhead bound using n_sn 
	 */
	memory_overhead = 4.0 * (double) ((L->n_sn) * sizeof(int)) +	/* integer vectors in L */
	...
	wtime = taucs_wtime();
	ctime = taucs_ctime();
	if (recursive_compute_supernodes_ll_in_core(L->n_sn, TRUE, (memory - memory_overhead) / 3.0, sn_in_core, L) < 0.0) {
		ooc_supernodal_factor_free(L);
		taucs_free(map);
		return NULL;
	}
	...
	if (recursive_leftlooking_supernodal_factor_llt_ooc((L->n_sn), TRUE, map, sn_in_core, handle, A, L)) {
		ooc_supernodal_factor_free(L);
		taucs_free(map);
		return NULL;
	}
...
static void recursive_supernodal_solve_l_ooc(int sn,	       /* this supernode */
					     int is_root,      /* is v the root? */
					     taucs_io_handle * handle,
					     int n_sn,
					     ...
					     taucs_datatype x[], taucs_datatype b[], taucs_datatype t[])
{
	...
	/*
	 * double done = 1.0; double dzero = 0.0; 
	 */
	...
	for (child = first_child[sn]; child != -1; child = next_child[child]) {
		recursive_supernodal_solve_l_ooc(child,
						 FALSE,
						 handle, n_sn, first_child, next_child, sn_struct, sn_sizes, sn_up_sizes, x, b, t);
	}
	...
	if (!is_root) {
		...
		taucs_io_read(handle, IO_BASE + n_sn + 2 * sn, sn_size, sn_size, TAUCS_CORE_DATATYPE, sn_block);

		if (up_size > 0 && sn_size > 0) {
			up_block = (taucs_datatype *) taucs_calloc(up_size * sn_size, sizeof(taucs_datatype));
			taucs_io_read(handle, IO_BASE + n_sn + 2 * sn + 1, up_size, sn_size, TAUCS_CORE_DATATYPE, up_block);
		}
		...
		if (flops > BLAS_FLOPS_CUTOFF) {
			...
			taucs_trsm("Left",
				   "Lower",
				   "No Conjugate",
				   "No unit diagonal", &sn_size, &ione, &taucs_one_const, sn_block, &sn_size, xdense, &sn_size);

			if (up_size > 0 && sn_size > 0)
				taucs_gemm("No Conjugate", "No Conjugate",
					   &up_size, &ione, &sn_size,
					   &taucs_one_const,
					   up_block, &up_size, xdense, &sn_size, &taucs_zero_const, bdense, &up_size);
			...
			for (jp = 0; jp < sn_size; jp++) {
				...
				for (ip = jp + 1; ip < sn_size; ip++) {
					 ...
					 */
					xdense[ip] = taucs_sub(xdense[i], taucs_mul(xdense[jp], sn_block[sn_size * jp + ip]));
				}
			}
			...
			for (jp = 0; jp < sn_size; jp++) {
				for (ip = 0; ip < up_size; ip++) {
					 ...
					 */
					bdense[ip] = taucs_add(bdense[ip], taucs_mul(xdense[jp], up_block[up_size * jp + ip]));
				}
			}
			...
			for (jp = 0; jp < sn_size; jp++) {
				...
				for (ip = jp + 1; ip < sn_size; ip++) {
					 ...
					 */
					b[i] = taucs_sub(b[i], taucs_mul(x[j], sn_block[sn_size * jp + ip]));
				}
				...
				for (ip = 0; ip < up_size; ip++) {
					 ...
					 */
					b[i] = taucs_sub(b[i], taucs_mul(x[j], up_block[up_size * jp + ip]));
				}
			}
		}
...
static void recursive_supernodal_solve_lt_ooc(int sn,	       /* this supernode */
					      int is_root,     /* is v the root? */
					      taucs_io_handle * handle,
					      int n_sn,
					      int *first_child, int *next_child,
		...
		if (flops > BLAS_FLOPS_CUTOFF) {
			...
			if (up_size > 0 && sn_size > 0)
				taucs_gemm("Conjugate", "No Conjugate",
					   ...
					   up_block, &up_size, xdense, &up_size, &taucs_one_const, bdense, &sn_size);

			taucs_trsm("Left",
				   "Lower",
				   "Conjugate",
				   "No unit diagonal", &sn_size, &ione, &taucs_one_const, sn_block, &sn_size, bdense, &sn_size);
			...
			for (ip = sn_size - 1; ip >= 0; ip--) {
				for (jp = 0; jp < up_size; jp++) {
					 ...
					 */
					bdense[ip] = taucs_sub(bdense[ip], taucs_mul(xdense[jp], up_block[up_size * ip + jp]));
				}
			}
			...
			for (ip = sn_size - 1; ip >= 0; ip--) {
				for (jp = sn_size - 1; jp > ip; jp--) {
					 ...
					 */
					bdense[ip] = taucs_sub(bdense[ip], taucs_mul(bdense[jp], sn_block[sn_size * ip + jp]));
				}
				/*
			...
			for (ip = sn_size - 1; ip >= 0; ip--) {
				...
				for (jp = 0; jp < up_size; jp++) {
					 ...
					 */
					b[i] = taucs_sub(b[i], taucs_mul(x[j], up_block[up_size * ip + jp]));
				}
				...
				for (jp = sn_size - 1; jp > ip; jp--) {
					 ...
					 */
					b[i] = taucs_sub(b[i], taucs_mul(x[j], sn_block[sn_size * ip + jp]));
				}
				/*
...
int taucs_dtl(ooc_solve_llt) (void *vL, void *vx, void *vb) {
	 ...
	 */
	/*
	 * if(!MULTIFILE) handle = taucs_io_open_singlefile(filename); else handle = taucs_io_open_multifile(filename); 
	 */
	...
	/*
	 * for (i=0; i<L->n_sn; i++) { L->sn_struct[i] = (int*)taucs_malloc(L->sn_up_size[i]*sizeof(int));
	...
	recursive_supernodal_solve_l_ooc(L->n_sn, TRUE,	       /* this is the root */
					 handle,
					 L->n_sn, L->first_child, L->next_child, L->sn_struct, L->sn_size, L->sn_up_size, y, x, t);

	recursive_supernodal_solve_lt_ooc(L->n_sn, TRUE,       /* this is the root */
					  handle,
					  L->n_sn, L->first_child, L->next_child, L->sn_struct, L->sn_size, L->sn_up_size, x, y, t);
...
static double recursive_smart_panelize_ooc_supernodes(int sn,  /* this supernode */
						      int is_root,	/* is v the root? */
						      double global_mem,
						      int *curr_panel,
						      int *sn_in_core, int *sn_to_panel_map, supernodal_factor_matrix * L)
{
	int child;
	...
	for (child = first_child[sn]; child != -1; child = next_child[child]) {
		if (!sn_in_core[child]) {
			curr_child_mem = recursive_smart_panelize_ooc_supernodes(child,
										 FALSE,
										 global_mem,
										 curr_panel, sn_in_core, sn_to_panel_map, L);
	...
	if (!is_root) {
		this_sn_mem = (double) (L->sn_size)[sn] * (double) (L->sn_up_size)[sn] * (double) sizeof(taucs_datatype)
...
static double recursive_dumb_panelize_ooc_supernodes(int sn,   /* this supernode */
						     int is_root,	/* is v the root? */
						     int *curr_panel,
						     int *sn_in_core, int *sn_to_panel_map, supernodal_factor_matrix * L)
{
	...
	for (child = first_child[sn]; child != -1; child = next_child[child]) {
		if (!sn_in_core[child])
			(void) recursive_dumb_panelize_ooc_supernodes(child, FALSE, curr_panel, sn_in_core, sn_to_panel_map, L);
	}
...
static double recursive_panelize_ooc_supernodes(int sn,	       /* this supernode */
						int is_root,   /* is v the root? */
						double global_mem,
						double avail_mem,
						int *curr_panel,
						int *sn_in_core, int *sn_to_panel_map, supernodal_factor_matrix * L)
{
	...
	if (!is_root) {
		this_sn_mem =
		    (double) (L->sn_size)[sn] * (double) (L->sn_up_size)[sn] * sizeof(taucs_datatype) +
		    (double) (L->sn_up_size)[sn] * sizeof(int);
						  ...
						  taucs_io_handle * handle, taucs_ccs_matrix * A, supernodal_factor_matrix * L)
{
	...
	if (L->sn_up_size[K] - L->sn_size[K] > 0) {
		...
		for (i = sn_size_child; i < sn_up_size_child; i++) {
			/*
			 * We want to update only supernodes great than J with using of sort of indices 
			 */
			 ...
			 */
			if (L->col_to_sn_map[L->sn_struct[K][i]] != curr_updated_sn || i == sn_up_size_child - 1)
				if (exist_upd) {
					if (!(L->up_blocks)[K]) {
						(L->up_blocks)[K] =
						    (taucs_datatype *) taucs_calloc(((L->sn_up_size)[K] - (L->sn_size)[K])
										    * ((L->sn_size)[K]), sizeof(taucs_datatype));
						taucs_io_read(handle, IO_BASE + L->n_sn + 2 * K + 1,
							      (L->sn_up_size)[K] - (L->sn_size)[K],
							      (L->sn_size)[K], TAUCS_CORE_DATATYPE, (L->up_blocks)[K]);
					}
					LDA = LDB = (L->sn_up_size)[K] - (L->sn_size)[K];
					 ...
					 * &(L->up_blocks[K][first_row-sn_size_child]),&LDB, &taucs_zero_const,
					...
					taucs_herk("Lower",
						   ...
						   &taucs_one_real_const,
						   &(L->up_blocks[K][first_row - sn_size_child]), &LDA,
						   &taucs_zero_real_const, dense_update_matrix, &LDC);
					...
					if (L->sn_up_size[curr_updated_sn] - L->sn_size[curr_updated_sn] > 0)
						if (!(L->up_blocks)[curr_updated_sn])
							(L->up_blocks)[curr_updated_sn] =
							    (taucs_datatype *)
							    taucs_calloc(((L->sn_up_size)[curr_updated_sn] -
									  (L->sn_size)[curr_updated_sn]) *
									 ((L->sn_size)[curr_updated_sn]), sizeof(taucs_datatype));

					if (!L->sn_struct[curr_updated_sn]) {
						L->sn_struct[curr_updated_sn] =
						    (int *) taucs_malloc((L->sn_up_size)[curr_updated_sn] * sizeof(int));
						taucs_io_read(handle, IO_BASE + curr_updated_sn, 1,
							      (L->sn_up_size)[curr_updated_sn], TAUCS_INT,
							      L->sn_struct[curr_updated_sn]);
					}
					...
					for (j = 0; j < row_count; j++)
						for (ir = j; ir < row_count; ir++) {
							 ...
							 * -= dense_update_matrix[j*LDC+ir];
							 */
							L->sn_blocks[curr_updated_sn][(bitmap[L->sn_struct[K][first_row + j]] -
										       1) * L->sn_size[curr_updated_sn] +
										      (bitmap[L->sn_struct[K][first_row + ir]] -
										       1)] =
							    taucs_sub(L->
								      sn_blocks[curr_updated_sn][(bitmap
												  [L->sn_struct[K][first_row + j]] -
												  1) * L->sn_size[curr_updated_sn] +
												 (bitmap
												  [L->
												   sn_struct[K][first_row + ir]] -
												  1)],
								      dense_update_matrix[j * LDC + ir]);
							 ...
							 * to find overflows 
							 */
							assert((double) (bitmap[L->sn_struct[K][first_row + j]] - 1) *
							       (double) L->sn_size[curr_updated_sn] < 2048.0 * 1024.0 * 1024.0);
						}
					for (j = 0; j < row_count; j++)
						for (ir = row_count; ir < M; ir++) {
							/*
							...
							L->up_blocks[curr_updated_sn][(bitmap[L->sn_struct[K][first_row + j]] -
										       1) * (L->sn_up_size[curr_updated_sn] -
											     L->sn_size[curr_updated_sn]) +
										      (bitmap[L->sn_struct[K][ir + first_row]] -
										       1)] =
							    taucs_sub(L->
								      up_blocks[curr_updated_sn][(bitmap
												  [L->sn_struct[K][first_row + j]] -
												  1) *
												 (L->sn_up_size[curr_updated_sn] -
												  L->sn_size[curr_updated_sn]) +
												 (bitmap
												  [L->
												   sn_struct[K][ir + first_row]] -
												  1)],
								      dense_update_matrix[j * LDC + ir]);
							 ...
							 */
							assert((double) (bitmap[L->sn_struct[K][first_row + j]] - 1) *
							       (double) (L->sn_up_size[curr_updated_sn] -
									 L->sn_size[curr_updated_sn]) < 2048.0 * 1024.0 * 1024.0);
						}
				...
				if (sn_to_panel_map[L->col_to_sn_map[L->sn_struct[K][i]]] == updated_panel) {
					...
					if (i == sn_up_size_child - 1)
						if (exist_upd) {
							...
							/*
							 * The GEMM code computes on the upper triangle of the trapezoidal matrix,
							 * which is junk. 
							 */
							/*
							 * taucs_gemm ("No Conjugate", "Conjugate", &M,&N,&PK, &taucs_one_const,
							...
							if (!L->sn_struct[curr_updated_sn]) {
								L->sn_struct[curr_updated_sn] =
								    (int *) taucs_malloc((L->sn_up_size)[curr_updated_sn] *
											 sizeof(int));
								taucs_io_read(handle, IO_BASE + curr_updated_sn, 1,
									      (L->sn_up_size)[curr_updated_sn], TAUCS_INT,
									      L->sn_struct[curr_updated_sn]);
							...
							for (ii = L->sn_size[curr_updated_sn]; ii < L->sn_up_size[curr_updated_sn];
							     ii++) {
								bitmap[L->sn_struct[curr_updated_sn][ii]] =
								    ii - L->sn_size[curr_updated_sn] + 1;
							}
							assert((double) row_count * (double) LDC < 2048.0 * 1024.0 * 1024.0);
							for (j = 0; j < row_count; j++)
								for (ir = j; ir < row_count; ir++) {
									 ...
									 * -= dense_update_matrix[j*LDC+ir];
									 */
									L->sn_blocks[curr_updated_sn][(bitmap
												       [L->
													sn_struct[K][first_row +
														     j]] -
												       1) *
												      L->sn_size[curr_updated_sn] +
												      (bitmap
									 ...
									 */
									assert((double) (bitmap[L->sn_struct[K][first_row + j]] - 1)
									       * (double) L->sn_size[curr_updated_sn] <
							...
							for (j = 0; j < row_count; j++)
								for (ir = row_count; ir < M; ir++) {
									...
									L->up_blocks[curr_updated_sn][(bitmap
												       [L->
													sn_struct[K][first_row +
														     j]] -
												       1) *
												      (L->
												       sn_up_size[curr_updated_sn] -
												       L->
												       sn_size[curr_updated_sn]) +
												      (bitmap
												       [L->
													sn_struct[K][ir +
														     first_row]] -
	...
	if (first_row && sn_to_panel_map[J] != sn_to_panel_map[K]) {
		...
		for (child = first_child[K]; child != -1; child = next_child[child]) {
			recursive_leftlooking_supernodal_update_panel_ooc(J, child,
									  bitmap,
									  sn_to_panel_map, dense_update_matrix, handle, A, L);
		}
	}
...
static int
 recursive_leftlooking_supernodal_factor_panel_llt_ooc(int sn, /* this supernode */
						       int father_sn, int is_root,	/* is sn the root? */
						       int *map,
						       int *sn_in_core,
						       int *sn_to_panel_map,
						       int *panel_max_size,
	...
	for (child = first_child[sn]; child != -1; child = next_child[child]) {
		if (sn_in_core[child]) {
		    ...
		    if (recursive_leftlooking_supernodal_factor_panel_llt_ooc(child,
									      ...
									      map,
									      sn_in_core,
									      sn_to_panel_map, panel_max_size, handle, A, L)) {
			/*
			 * failure 
			 */
			return -1;
	...
	if (!is_root) {
		...
		if (!dense_update_matrix)
			dense_update_matrix =
			    (taucs_datatype *) taucs_calloc(panel_max_size[sn_to_panel_map[sn]], sizeof(taucs_datatype));
		...
		for (child = first_child[sn]; child != -1; child = next_child[child]) {
			if (sn_to_panel_map[sn] != sn_to_panel_map[child])
				recursive_leftlooking_supernodal_update_panel_ooc(sn, child,
										  map,
										  sn_to_panel_map,
										  dense_update_matrix, handle, A, L);
		}
		...
		if (sn_to_panel_map[sn] == sn_to_panel_map[father_sn])
			recursive_leftlooking_supernodal_update_panel_ooc(father_sn, sn,
									  map, sn_to_panel_map, dense_update_matrix, handle, A, L);
		taucs_free(dense_update_matrix);
		taucs_free((L->sn_blocks)[sn]);
		taucs_free((L->up_blocks)[sn]);
		taucs_free((L->sn_struct)[sn]);
...
int taucs_dtl(ooc_factor_llt) (taucs_ccs_matrix * A, taucs_io_handle * handle, double memory) {
	supernodal_factor_matrix *L;
	int i;
	int *map;
	int *sn_in_core;
	...
	/*
	 * int j,ip,jp; int sn,p; int current_index=0; 
	 */
	double memory_overhead;
	...
	L = multifrontal_supernodal_create();
	/*
	 * if(!MULTIFILE) handle = taucs_io_create_singlefile("/tmp/taucs.L"); else handle =
	 * taucs_io_create_multifile("/tmp/taucs.L"); 
	 */
	taucs_io_append(handle, 5, 1, 1, TAUCS_INT, &(A->n));
	...
	/*
	 * if(recursive_panelize_ooc_supernodes(L->n_sn, TRUE, (memory - memory_overhead)/3.0, (memory - memory_overhead)/3.0,
	 * &n_pn, sn_in_core, sn_to_panel_map, L)<0.0){ ooc_supernodal_factor_free(L); taucs_free(sn_in_core);
	 * taucs_free(sn_to_panel_map); taucs_free(map); return -1; }
	 */
	...
	if (recursive_smart_panelize_ooc_supernodes(L->n_sn,
						    ...
						    &n_pn, sn_in_core, sn_to_panel_map, L) < 0.0) {
		ooc_supernodal_factor_free(L);
		taucs_free(sn_in_core);
		taucs_free(sn_to_panel_map);
		taucs_free(map);
	...
	/*
	 * for(i=0;i<L->n_sn;i++){ taucs_printf("sn_in_core[%d] = %d\n",i,sn_in_core[i]); taucs_printf("sn_to_panel_map[%d] =
	 * %d\n",i,sn_to_panel_map[i]); }
	 */
	...
	/*
	 * for(i=0;i<n_pn;i++) taucs_printf(" panel_max_size[%d] = %d\n",i, panel_max_size[i]);
	 */
	...
	if (recursive_leftlooking_supernodal_factor_panel_llt_ooc(L->n_sn,
								  L->n_sn,
								  TRUE,
								  map, sn_in_core, sn_to_panel_map, panel_max_size, handle, A, L)) {
		ooc_supernodal_factor_free(L);
		taucs_free(map);
		return -1;
	}
...
int taucs_dtl(ooc_factor_llt_panelchoice) (taucs_ccs_matrix * A, taucs_io_handle * handle, double memory, int panelization_method) {
	supernodal_factor_matrix *L;
	int i;
	int *map;
	int *sn_in_core;
	...
	if (panelization_method == 2) {
		...
		if (recursive_dumb_panelize_ooc_supernodes(L->n_sn, TRUE, &n_pn, sn_in_core, sn_to_panel_map, L) < 0.0) {
			ooc_supernodal_factor_free(L);
			taucs_free(sn_in_core);
			taucs_free(sn_to_panel_map);
			taucs_free(map);
...
int taucs_ooc_factor_llt_panelchoice(taucs_ccs_matrix * A, taucs_io_handle * L, double memory, int panelchoice)
{
#ifdef TAUCS_CONFIG_DREAL
	if (A->flags & TAUCS_DOUBLE)
			...
			void *x, void *b)
{
	...
	taucs_io_read((taucs_io_handle *) L, 6, 1, 1, TAUCS_INT, &flags);
