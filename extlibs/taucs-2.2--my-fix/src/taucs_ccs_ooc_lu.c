...
{
	taucs_io_append(LU, 0 + HEADER + j * 4, nnz, 1, TAUCS_INT, ind);
	taucs_io_append(LU, 1 + HEADER + j * 4, nnz, 1, TAUCS_CORE_DATATYPE, re);
	return 0;
}
...
{
	taucs_io_append(LU, 2 + HEADER + j * 4, nnz, 1, TAUCS_INT, ind);
	taucs_io_append(LU, 3 + HEADER + j * 4, nnz, 1, TAUCS_CORE_DATATYPE, re);
	return 0;
}
...
{
	taucs_io_read(LU, 0 + HEADER + j * 4, nnz, 1, TAUCS_INT, ind);
	taucs_io_read(LU, 1 + HEADER + j * 4, nnz, 1, TAUCS_CORE_DATATYPE, re);
	return 0;
}
...
{
	taucs_io_read(LU, 2 + HEADER + j * 4, nnz, 1, TAUCS_INT, ind);
	taucs_io_read(LU, 3 + HEADER + j * 4, nnz, 1, TAUCS_CORE_DATATYPE, re);
	return 0;
}
...
static void heap_insert(int *heap, int *heapsize, int i, int j, int k)
{
	...
	child = (*heapsize - 1);
	parent = child / 2;
	while (child > 0 && ((heap[3 * parent + 1] > j) || ((heap[3 * parent + 1] == j) && (heap[3 * parent + 2] > k)))) {
...
static void skel_init(char *basename)
{
	...
	/*
	 * adjust remaining memory. We allocate 2 io buffers for the sort phase, plus an array of file pointers that is guarantreed 
	 * to be smaller. We then free them and allocate another io buffer for the stack phase, but we do not free the skel buffer
	 * first, so 3 io buffers is a conservative estimate. 
	 */
...
static void skel_sort_outofcore(int *postorder, int ncols, int *inv_postorder)
{
	...
	if (natural == 0) {
		...
		for (f = 0; f < skel_outfiles; f++) {
			...
			qsort(skel_buffer, io_count, 2 * sizeof(int), &skel_compare);
	...
	/*
	 * each element in the skel_buffer is 2 ints, but each element in the heap is 3 ints 
	 */
	...
	/*
	 * inbuf = taucs_calloc(iobufsize,2*sizeof(int)); outbuf = taucs_calloc(iobufsize,2*sizeof(int)); infiles =
	 * taucs_calloc(skel_outfiles,sizeof(int)); 
	 */
	while (skel_outfiles > 1) {
		...
		for (runstart = 0; runstart < skel_infiles; runstart += maxopenruns) {
			...
			sprintf(fname, "%s.%c.%d", skel_basename, skel_outphase, runstart / maxopenruns);
			...
			for (openruns = 0; openruns < maxopenruns && runstart + openruns < skel_infiles; openruns++) {
			...
			while (heapsize > 0) {
				...
				if (end_of_run) {
					 ...
					 * skel_buffer[3*ii+0], skel_buffer[3*ii+1], skel_buffer[3*ii+2]); printf("\n"); } 
					 */
					taucs_printf("oocsp_colanalyze: Inserting %d elements from input run into heap\n",
						     io_count);
	...
	/*
	 * taucs_free(infiles); taucs_free(inbuf); taucs_free(outbuf); 
	 */
}
...
static int skel_get_next(int j)
{
	...
	if (skel_next >= skel_buffer_ptr) {
		if (skel_outfiles > 0) {
			...
			io_size = read(skel_outfile, skel_buffer, skel_buffer_size * 2 * sizeof(int));
			if (io_size == -1)
...
static void skel_get_postordercol(int *found, int flag, int j, int *nnz, int *rowind)
{
	int row;
...
static void stack_init(char *basename, int *colptr, int *colstack, int ncols)
{
	int j;
	...
	/*
	 * stack_buffer_size = 1048576; stack_buffer = mxCalloc(stack_buffer_size,sizeof(int)); 
	 */
...
static void stack_push(int *colptr, int *colstack, int i, int j)
{
	...
	/*
	 * stack_size++; if (stack_size > stack_max_size) stack_max_size = stack_size; 
	 */
}

static void stack_pop(int *colptr, int *colstack, int *found, int flag, int j, int *nnz, int *rowind)
{
	int row;
	int i;
...
void oocsp_colanalyze(taucs_ccs_matrix * matrix,
		      char *basename, int *colperm, int **ptrparent, int **ptrpostorder, int **ptrlcolcount, int **ptrucolcount)
{
	int i, j, ip, p, jp;
	...
	*ptrpostorder = postorder;
...
#else
		/*
		 * nnz = clen[j]; oocsp_readcol_structure(matrix,j,tmp_col); rowind = tmp_col; 
		 */
	...
	skel_sort(postorder, *ncols, found /* temporary */ );
	...
	for (jp = 0; jp < (*ncols); jp++) {
		...
		stack_pop(stack_colptr, stack_colstk, found, jp, j, &nnz, tmp_col);
		rowind = tmp_col;
		for (ip = 0; ip < nnz; ip++) {
			i = rowind[ip];
		...
		skel_get_postordercol(found, jp, jp,	       /* use postorder column index */
				      &nnz, tmp_col);
		rowind = tmp_col;
		for (ip = 0; ip < nnz; ip++) {
			i = rowind[ip];
	...
	/*
	 * mxDestroyArray(tmp1_array); mxDestroyArray(tmp2_array); mxDestroyArray(tmp3_array); mxDestroyArray(tmp4_array); 
	 */
	...
	/*
	 * for (j=0; j < (*ncols); j++) { postorder[j]++; if (parent[j] == (*ncols)) parent[j] = 0; else parent[j]++; } 
	 */
	/*
	 * fix up by Vladi 
	 */
...
static
void oocsp_panelize_simple(int nrows,			       /* input */
			   int ncols,			       /* input */
			   int *postorder,		       /* input */
    ...
    )
{
	...
	/*
	 * memory usage in numfact, width > 0, snodes: 8*nrows*I + 2*nrows*D + 2*nrows*C + 3*nrows*P + 2*ncols*I + 1*w*nrows*I +
	 * 1*w*nrows*D + 1*w*I + 5*w*maxc *I + 2*w*maxc *D + 1*s*I + 1*s*maxc *I + 2*s*maxc *D
	 * 
	 * we also account for the memory required for the in-core representation of A, L, U, which is
	 * 2*ncols*sizeof(char)+3*ncols*sizeof(int) 
	 */

	*maxsn = 8 - 4;					       /* 8 is the minimum value we use here */
	do {
		*maxsn += 4;
		memuse = 8.0 * nrows * sizeof(int) + 2.0 * nrows * sizeof(taucs_datatype)
		...
		memuse += 3 * (3.0 * ncols * sizeof(int) + 2.0 * ncols * sizeof(char));

		width_multiplier = 1.0 * nrows * sizeof(int) + 1.0 * nrows * sizeof(taucs_datatype) + 1.0 * sizeof(int)
...
static void num_heap_heapify(int *heap, int *heapsize, int *ipivots, int p)
{
	int r, l, smallest;
	int temp;
...
static void gather(int a_nnz, taucs_datatype * a_re, int *a_ind, taucs_datatype * spa, char *spamap)
{
	int i, ip;
...
static void scatter(int a_nnz, taucs_datatype * a_re, int *a_ind, taucs_datatype * spa, char *spamap)
{
	int i, ip;
...
void spcol_spa_update(int pivotindex,
		      ...
		      int l_nnz,
		      int *panelcols,
		      int panelcols_n, int nrows, int **a_inrowlist, taucs_datatype * spa, char *spamap, int **a_ind, int *a_nnz)
{
	int i, ip, j, q;
	taucs_datatype v;
	...
	for (j = 0; j < panelcols_n; j++) {
		q = panelcols[j];
		if (taucs_re(spa[q * nrows + pivotindex]) == 0.0 && taucs_im(spa[q * nrows + pivotindex]) == 0.0)
			continue;
		for (ip = 0; ip < l_nnz; ip++) {
			 ...
			 */
			spa[q * nrows + i] = taucs_sub(spa[q * nrows + i], taucs_mul(spa[q * nrows + pivotindex], v));
		}
	}
	return;
}
...
spcol_spcol_update(int pivotindex,
		   ...
		   int *l_ind,
		   int l_nnz,
		   int panelcol, int *a_inrowlist, taucs_datatype * spa, char *spamap, int *a_ind, int *a_nnz, int lu_colcount)
{
	int i, ip;
	taucs_datatype pv;
	...
	/*
	 * if ((*a_nnz) > lu_colcount) { taucs_printf("oocsp_numfact: prediction=%d, size now=%d\n",lu_colcount,*a_nnz);
...
static void spcol_panel_update(int pivotindex,
			       ...
			       int **a_inrowlist, taucs_datatype * spa, char *spamap, int **a_ind, int *a_nnz, int nrows)
{
	int i, ii, ip, q, j;
	taucs_datatype x;
...
void oocsp_numfact(taucs_ccs_matrix * A, int *colperm,
		   ...
		   int *panels,
		   int *schedstart,
		   int *schedend, int *fetchnext, int *ejectnext, int *lcolcount, int *ucolcount, int spawidth, int maxsn)
{
	...
	/*
	 * char fname[256]; int file; int mode; mode_t perm; omer
	 */
	...
	/*
	 * taucs_datatype* update_tmp; int* update_vec; int update_vec_next; 
	 */
	 ...
	 * char* snode_map;
	 *//*
	 * sivan changed to int to support m2 
	 */
	int *snode_map;
	int *snode_pivrows;
	int *snode_index;
	...
	/*
	 * lu_re = (taucs_datatype*)taucs_malloc(maxsn * nrows * sizeof(taucs_datatype) ); lu_ind = (int*) taucs_malloc(maxsn *
	 * nrows * sizeof(int) ); 
	 */
	...
	/*
	 * lu_re = (taucs_datatype*)taucs_malloc(nrows*sizeof(taucs_datatype)); lu_ind = (int*)taucs_malloc(nrows*sizeof(int)); 
	 */
#endif
	...
	/*
	 * update_vec = (int*)taucs_malloc(ncols*sizeof(int)); update_tmp =
	 * (taucs_datatype*)taucs_malloc(ncols*sizeof(taucs_datatype)); 
	 */
	...
	for (step = 1; step <= nsteps; step++) {
		...
		while (fn < ncols && (schedstart[fetchnext[fn]]) == step) {
			j = fetchnext[fn];

			/*
			 * if (A->clen[j] > lcolcount[j]+ucolcount[j]) { taucs_printf("oocsp_numfact: Column %d in matrix larger
			 * than L+U estimate\n",j); taucs_printf("oocsp_numfact: Size %d >
			 * %d+%d\n",A->clen[j],lcolcount[j],ucolcount[j]); taucs_printf("oocsp_numfact: Aborting\n"); }
			 * 
			...
			panel_id[p] = j;
			if (lcolcount[j] < 1 || lcolcount[j] > nrows || ucolcount[j] < 1 || ucolcount[j] > nrows) {
			...
			if (!panel_inrowlist[p] || !panel_ind[p] || !panel_re[p]) {
			...
			/*
			 * time_tmp = taucs_wtime(); oocsp_readcol(A,j,panel_ind[p],panel_re[p]); time_read += (taucs_wtime() -
			 * time_tmp); bytes_read += (double) (panel_nnz[p] * (sizeof(taucs_datatype)+sizeof(int))); 
			 */
...
#ifdef SPA_ONEARRAY
					panel_spa + (p * nrows), panel_spamap + (p * nrows)
#else
					panel_spa[p], panel_spamap[p]
#endif
				    );
			}

			for (i = 0; i < panel_nnz[p]; i++) {
				panel_inrowlist[p][i] = rowlists_insert(panel_ind[p][i], p);
		...
		while ((i = num_heap_extractmin(heap, &heapsize, ipivots)) != -1) {
			...
			/*
			 * if (lindices[i]) continue;
			 *//*
			 * if (!uindices[i]) continue;
			 */
			 ...
			 * oocsp_readcol(L,ks,lu_ind+((ks-k)*nrows),lu_re+((ks-k)*nrows)); bytes_read += (double) ((L.clen[ks]) *
			 * (sizeof(taucs_datatype)+sizeof(int))); #ifdef DETAILED_TIMING col_read += 1.0; #endif }
...
#endif
				if (spawidth <= 0) {
					scatter(panel_nnz[q], panel_re[q], panel_ind[q], spa, spamap);
					...
					gather(panel_nnz[q], panel_re[q], panel_ind[q], spa, spamap);
				} else {
					spcol_spcol_update(i, lu_re, lu_ind, Lclen[k], q, panel_inrowlist[q],
#ifdef SPA_ONEARRAY
							   panel_spa + (q * nrows), panel_spamap + (q * nrows),
#else
							   panel_spa[q], panel_spamap[q],
#endif
							   panel_ind[q], &(panel_nnz[q]),
							   lcolcount[panel_id[q]] + ucolcount[panel_id[q]]);
				}
			...
			/*
			 * update_vec_next = 0; for(qp = rowlists_head[i]; qp != -1; qp = rowlists_next[qp]) { q =
			 * rowlists_colind[qp]; col_ooc_updates += 1.0; update_vec[update_vec_next] = q; update_vec_next++; }
			 * 
			 * #ifdef DETAILED_TIMING time_tmp = taucs_wtime(); #endif
			 * 
			 * #ifdef SPA_ONEARRAY spcol_panel_update(i, lu_re,lu_ind,Lclen[k], update_vec,update_vec_next,update_tmp,
			 * panel_inrowlist, panel_spa, panel_spamap, panel_ind,panel_nnz, nrows); #endif
			 * 
			 * #ifdef DETAILED_TIMING time_colcol += (taucs_wtime() - time_tmp); #endif 
			 */
		}
#else							       /* with SNODES */
		/*
			...
			for (tmp = 0; tmp < 1; tmp++) {
...
#ifdef DETAILED_TIMING
							flops +=
							    2.0 * ((ks - jj) * (srows_n - (jj - k)) - 0.5 * (ks - jj) * (ks - jj));
				...
				for (jj = k; jj < ks; jj++) {
					...
					for (iip = 0; iip < srows_n; iip++) {
						if (m2[srows[iip]] != -1) {
							S[(jj - k) * srows_n + iip] =
							    lu_re[(jj - k) * maxcolcount + m2[srows[iip]]];
...
#define OLD_1
...

			{
						...
						if (!flag) {
							for (iip = iip_block; iip < loop_bound; iip++) {
								if (panel_spamap[jj * nrows + srows[iip]] == 0) {
									...
									panel_spa[jj * nrows + srows[iip]] = taucs_zero;
									panel_ind[jj][panel_nnz[jj]] = srows[iip];
									panel_inrowlist[jj][panel_nnz[jj]] =
									    rowlists_insert(srows[iip], jj);
									(panel_nnz[jj])++;
								}
							}
						}
				...
				/*
				 * flops += (2.0 * spa_n * srows_n * (ks-k) - 2.0); 
				 *//*
				 * over estimate; sivan. 
				 */
				/*
				 ...
				 */
#ifdef DETAILED_TIMING
				flops_dense += (2.0 * spa_n * srows_n * (ks - k) - 1.0 * spa_n * (ks - k) * (ks - k));
#endif							       /* DETAILED_TIMING */
...
#ifdef USE_BLAS
				{
					...
					if (m > BLAS_THRESHOLD && n > BLAS_THRESHOLD) {
						taucs_trsm("Left",
							   "Lower",
							   "No transpose",
							   "Unit", &m, &n, &taucs_one_const, S, &srows_n, P, &srows_n);
					} else {
						/*
						 * TRSM 
						 */
						for (jj = 0; jj < spa_n; jj++) {
							for (kk = k; kk < ks; kk++) {
								for (ii = kk - k + 1; ii < (ks - k); ii++) {
									/*
									 * P[jj*srows_n + ii] -= (P[jj*srows_n + (kk-k)] *
									 * S[(kk-k)*srows_n + ii]);
									 */
									P[jj * srows_n + ii] =
									    taucs_sub(P[jj * srows_n + ii],
										      taucs_mul(P[jj * srows_n + (kk - k)],
												S[(kk - k) * srows_n + ii]));
								}
							}
						}
					...
					if (M > BLAS_THRESHOLD && N > BLAS_THRESHOLD && K > BLAS_THRESHOLD) {
						taucs_gemm("No transpose",
							   ...
							   S + m, &srows_n, P, &srows_n, &taucs_one_const, P + m, &srows_n);
					} else {
						/*
						 * GEMM 
						 */
				...
				for (jj = 0; jj < spa_n; jj++) {
					for (kk = k; kk < ks; kk++) {
						for (ii = kk - k + 1; ii < (ks - k); ii++) {
							P[jj * srows_n + ii] -=
							    (P[jj * srows_n + (kk - k)] * S[(kk - k) * srows_n + ii]);
						}
					}
				}
			...
			if (!dense_flag) {		       /* we didn't do it using the blas since m,n, or k were too small */
				...
				for (jj = k; jj < ks; jj++) {
					...
					for (qp = rowlists_head[ii]; qp != -1; qp = rowlists_next[qp]) {
						q = rowlists_colind[qp];
						/*
						 * printf("supernode update: col %d pivotrow %d updates col
						 * %d\n",jj,ii,panel_id[q]);
						 */
						...
						if (spawidth <= 0) {
							...
							spcol_spcol_update(ii,
									   lu_re + ((jj - k) * maxcolcount),
									   lu_ind + ((jj - k) * maxcolcount), Lclen[jj], q,
									   panel_inrowlist[q],
#ifdef SPA_ONEARRAY
									   panel_spa + (q * nrows), panel_spamap + (q * nrows),
#else
		...
		while (en < ncols && (schedend[ejectnext[en]]) == step) {
			p--;
			j = ejectnext[en];
...
#endif
			/*
			 * oocsp_appendcol(U,j,nrows - 1 - unext, lu_ind + (unext+1), lu_re + (unext+1)); 
			 */
			 ...
			 */
			/*
			 * oocsp_appendcol(U,j,maxcolcount - 1 - unext, lu_ind + (unext+1), lu_re + (unext+1)); 
			 */
			assert(Uclen[j] == 0);
			Uclen[j] = maxcolcount - 1 - unext;
			Uappendcol(LU, j, maxcolcount - 1 - unext, lu_ind + (unext + 1), lu_re + (unext + 1));
#ifdef DETAILED_TIMING
...
#ifdef DETAILED_TIMING
				time_append += (taucs_wtime() - time_tmp);
				bytes_appended += (double) (snode_nnz * (sizeof(taucs_datatype) + sizeof(int)));
#endif
...
#ifdef DETAILED_TIMING
				time_append += (taucs_wtime() - time_tmp);
				bytes_appended += (double) (lnext * (sizeof(taucs_datatype) + sizeof(int)));
				time_tmp = taucs_wtime();
#endif
			...
			if (spawidth <= 0) {
				...
				/*
				 * for(qp = 0; qp < spa_n; qp++) { q = spa_updcols[qp]; spcol_spcol_update(pivotindex,
				 * lu_re,lu_ind,lnext, q, panel_inrowlist[q], panel_spa + (q*nrows), panel_spamap + (q*nrows),
				 * panel_ind[q],&(panel_nnz[q]), 0); } 
				 */

				spcol_spa_update(pivotindex,
						 lu_re, lu_ind, lnext,
						 spa_updcols, spa_n, nrows,
						 panel_inrowlist, panel_spa, panel_spamap, panel_ind, panel_nnz);
			}
#ifdef DETAILED_TIMING
			time_colcol += (taucs_wtime() - time_tmp);
...
*/
	...
	/*
	 * taucs_free(pivots); taucs_free(ipivots); 
	 */
...
static
int oocsp_factor(taucs_ccs_matrix * A_in, taucs_io_handle * LU, int *colperm)
{
	/*
	 * int i,j; omer
	 */
	...
	oocsp_colanalyze(A_in, taucs_io_get_basename(LU), colperm, &etree, &postorder, &l_colcounts, &u_colcounts);

	taucs_printf("taucs_ooc_lu: calling panelize\n");
	oocsp_panelize_simple(A_in->m, A_in->n,
			      postorder,
			      l_colcounts, u_colcounts, etree,
			      &spawidth, &maxsn, &panels, &schedstart, &schedend, &fetchnext, &ejectnext);
	...
	oocsp_numfact(A_in, colperm,
		       ...
		       * L,U,
		       */
		      LU, panels, schedstart, schedend, fetchnext, ejectnext, l_colcounts, u_colcounts, spawidth, maxsn);
...
static int oocsp_solve(taucs_io_handle * LU, taucs_datatype * X, taucs_datatype * B)
{
	int i, ip, j, n;
	taucs_datatype *Y;
	taucs_datatype Aij;
	...
	/*
	 * taucs_printf("colperm = ["); for(i=0; i<n; i++) taucs_printf("%d, ",colperm[i]); taucs_printf("\b\b]\n");
	 * 
	 * taucs_printf("ipivots = ["); for(i=0; i<n; i++) taucs_printf("%d, ",ipivots[i]); taucs_printf("\b\b]\n"); 
	 */
	...
	/*
	 * for (i=0; i<n; i++) PB[i] = B[ ipivots[i] ]; 
	 */
	 ...
	 * X[ colperm[i] ] = Y[ i ];
	 * 
	 * for (i=0; i<n; i++) Y[i] = X[i]; for (i=0; i<n; i++) X[ ipivots[i] ] = Y[ i ];
	 * 
	 */

	/*
	 * for (i=0; i<n; i++) if (colperm[i] == 0) j=i; printf("rowperm[0]=%d irowperm[0]=%d colperm[0]=%d
	 * icp[0]=%d\n",ipivots[0], irowperm[0], colperm[0],j); for (i=0; i<n; i++) if (colperm[i] == 1) j=i; printf("rowperm[1]=%d 
	 * irowperm[1]=%d colperm[1]=%d icp[1]=%d\n",ipivots[1], irowperm[1], colperm[1],j); 
	 */
...
int taucs_ooc_factor_lu(taucs_ccs_matrix * A, int *colperm, taucs_io_handle * LU, double memory)
{
...
int taucs_ooc_solve_lu(taucs_io_handle * LU, void *x, void *b)
{
	...
	printf("taucs_ooc_solve_lu: starting, DZSC=%d%d%d%d\n",
	       ...
	       (flags & TAUCS_DCOMPLEX) != 0, (flags & TAUCS_SINGLE) != 0, (flags & TAUCS_SCOMPLEX) != 0);
