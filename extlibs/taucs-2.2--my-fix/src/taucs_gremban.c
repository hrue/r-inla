...
int taucs_sg_preconditioner_solve(void *vP, double *Z, double *R)
{
	...
	taucs_ccs_solve_llt(P->L, P->Ztilde, P->Rtilde);
...
int partition(int *quicksort_array_nodes_1, int *quicksort_array_nodes_2, double *quicksort_array_values, int p, int r)
{
	...
	while (1) {
		do
			j--;
		while ((quicksort_array_nodes_1[j] > x1)
		       || ((quicksort_array_nodes_1[j] == x1) && (quicksort_array_nodes_2[j] > x2)));
		do
			i++;
		while ((quicksort_array_nodes_1[i] < x1)
		       || ((quicksort_array_nodes_1[i] == x1) && (quicksort_array_nodes_2[i] < x2)));
...
int quicksort_and_shrink(int *quicksort_array_nodes_1, int *quicksort_array_nodes_2, double *quicksort_array_values,
			 int quicksort_index)
{
...
int create_recursive_preconditioner(graph * out,	       /* output: graph of the preconditioner. */
				    int curr_vertex, int *next_unused_vertex, int *curr_entry, Metis_struct * father, int *vertex_perm,	/* translates 
																	 * the 
																	 * vertex 
																	 * numbers 
																	 * in 
				    ...
				    double diag, double *diagonal,	/* diagonal values of mtxA */
				    instruction * inst,	       /* array of instructions : how the preconditioner is built at each
							        * level */
				    int max_inst,	       /* number of levels of instructions */
				    int curr_inst,	       /* current level of instructions */
				    taucs_ccs_matrix * taucs_ccs_mtxA,	/* NOTICE - taucs_ccs_mtxA should not contain the diagonal
									 * values, and it should contains each entry twice !!! */
				    char *ordering, int *p1)
{
	...
	if (nparts == n)
		...
		int *visited;				       /* helps determine how many parts the graph was actually divided
							        * into */
		int actual_nparts = 0;
		...
		if (nparts < 8)
			METIS_PartGraphRecursive(&n, father->colptr, father->rowind,
						 NULL, father->values, &wgtflag, &numflag, &nparts, options, &edgecut, part);
		else
	...
	if (inst[curr_inst].type == 0) {		       /* Gremban */
		for (i = 0; i < n; i++) {
			...
			for (j = taucs_ccs_mtxA->colptr[t1]; j < taucs_ccs_mtxA->colptr[t1 + 1]; j++)
				if ((inv_perm[taucs_ccs_mtxA->rowind[j]] == -1) || (part[inv_perm[taucs_ccs_mtxA->rowind[j]]] != p))
					weights[p] += taucs_ccs_mtxA->taucs_values[j];	/* weights[p] is the sum of all weights of
											 * all edges between part p and the rest
											 * the graph */
		}
		if (curr_inst < max_inst) {
			for (i = 0; i < nparts; i++)
		...
		for (i = 0; i < n; i++) {
			...
			for (j = taucs_ccs_mtxA->colptr[t1]; j < taucs_ccs_mtxA->colptr[t1 + 1]; j++) {
				if (inv_perm[taucs_ccs_mtxA->rowind[j]] == -1)
					weights[p] += taucs_ccs_mtxA->taucs_values[j];	/* weights[p] is the sum of all weights of
											 * all edges between part p and vertices
											 * outside current subgraph */
		...
		if (quicksort_index) {
				...
				for (j = taucs_ccs_mtxA->colptr[t1]; j < taucs_ccs_mtxA->colptr[t1 + 1]; j++) {
					if (inv_perm[taucs_ccs_mtxA->rowind[j]] != -1)
						if (part[inv_perm[taucs_ccs_mtxA->rowind[j]]] > p)
							/*
							 * since each entry appears twice in taucs_ccs_mtxA, we need not update
							 * pairs when part[...]<p 
							 */
						{
			...
			quicksort_index =
			    quicksort_and_shrink(quicksort_array_nodes_1, quicksort_array_nodes_2, quicksort_array_values,
						 quicksort_index);
				...
				for (i = 0; i < vaidya_ccs->n; i++)
					for (j = vaidya_ccs->colptr[i]; j < vaidya_ccs->colptr[i + 1]; j++)
						if (i != (vaidya_ccs->rowind[j])) {
							out->edges[(*curr_entry)].i = local_next_unused_vertex + i;
							out->edges[(*curr_entry)].j =
							    local_next_unused_vertex + vaidya_ccs->rowind[j];
	...
	vertices_in_subgraphs = (int *) taucs_malloc(n * sizeof(int));	/* for each vertex i, tmp[i] is the vertex's position in
									 * the subgraph */
	...
	if (curr_inst < max_inst) {
		for (i = 0; i < nparts; i++) {
			if (tmp1[i] > 1) {		       /* if the subgraph contains more than one vertex */
				success =
				    create_recursive_preconditioner(out, local_next_unused_vertex + i, next_unused_vertex,
								    curr_entry, sons[i], vertex_perms[i], inv_perm, diags[i],
								    diagonal, inst, max_inst, curr_inst + 1, taucs_ccs_mtxA,
								    ordering, p1);
				if (success == 0) {
					taucs_free(diags);
					taucs_free(weights);
...
void *taucs_sg_preconditioner_create(taucs_ccs_matrix * A, int **perm, int **invperm, char *ordering, char *gremban_command)
{
#ifdef NOMETIS
	return NULL;
#else
