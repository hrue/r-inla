		...
		taucs_datatype * B, int *pldb, taucs_real_datatype * beta, taucs_datatype * C, int *pldc)
{
	...
	if (n <= TAUCS_THRESHOLD_GEMM_BLAS && k <= TAUCS_THRESHOLD_GEMM_BLAS) {
		 ...
		 */
		taucs_gemm(transa, transb, pm, pn, pk, alpha, A, plda, B, pldb, beta, C, pldc);
		return;
	}
	...
	if (k >= n && k >= m) {
		 ...
		 */
		spawn taucs_cilk_gemm(transa, transb, pm, pn, &khalf1, alpha, A, plda, B, pldb, beta, C, pldc);

		sync;
		spawn taucs_cilk_gemm(transa, transb,
				      pm, pn, &khalf2, alpha, A + khalf1 * lda, plda, B + khalf1 * ldb, pldb, beta, C, pldc);
		sync;
		return;
	}
	...
	if (n >= k && n >= m) {
		...
		spawn taucs_cilk_gemm(transa, transb, pm, &nhalf1, pk, alpha, A, plda, B, pldb, beta, C, pldc);

		spawn taucs_cilk_gemm(transa, transb,
				      pm, &nhalf2, pk, alpha, A, plda, B + nhalf1, pldb, beta, C + nhalf1 * ldc, pldc);
		sync;
		return;
	}
	...
	if (1 /* m >= k && m >= n */ ) {		       /* the condition must be true */
		...
		spawn taucs_cilk_gemm(transa, transb, &mhalf1, pn, pk, alpha, A, plda, B, pldb, beta, C, pldc);

		spawn taucs_cilk_gemm(transa, transb, &mhalf2, pn, pk, alpha, A + mhalf1, plda, B, pldb, beta, C + mhalf1, pldc);
		...
		taucs_datatype * A, int *plda, taucs_real_datatype * beta, taucs_datatype * C, int *pldc)
{
	...
	if (n <= TAUCS_THRESHOLD_HERK_BLAS && k <= TAUCS_THRESHOLD_HERK_BLAS) {
		 ...
		 */
		taucs_herk(uplo, trans, pn, pk, alpha, A, plda, beta, C, pldc);
		return;
	}
	...
	if (k > n) {
		 ...
		 */
		spawn taucs_cilk_herk(uplo, trans, pn, &khalf1, alpha, A, plda, beta, C, pldc);

		sync;
		spawn taucs_cilk_herk(uplo, trans, pn, &khalf2, alpha, A + khalf1 * lda, plda, beta, C, pldc);
		 ...
		 */
		spawn taucs_cilk_herk(uplo, trans, &nhalf1, pk, alpha, A, plda, beta, C, pldc);

		spawn taucs_cilk_gemm("No Transpose", "Conjugate",
				      &nhalf2, &nhalf1, pk,
				      &taucs_minusone_const, A + nhalf1, plda, A, plda, &taucs_one_const, C + nhalf1, pldc);

		spawn taucs_cilk_herk(uplo, trans, &nhalf2, pk, alpha, A + nhalf1, plda, beta, C + nhalf1 * ldc + nhalf1, pldc);
		...
		int *pm, int *pn, taucs_datatype * alpha, taucs_datatype * A, int *plda, taucs_datatype * B, int *pldb)
{
	...
	if (m <= TAUCS_THRESHOLD_TRSM_BLAS && n <= TAUCS_THRESHOLD_TRSM_BLAS) {
		 ...
		 */
		taucs_trsm(side, uplo, transa, diag, pm, pn, alpha, A, plda, B, pldb);
		return;
	}
	...
	if (m >= n) {
		 ...
		 */
		spawn taucs_cilk_trsm(side, uplo, transa, diag, &mhalf1, pn, alpha, A, plda, B, pldb);
		spawn taucs_cilk_trsm(side, uplo, transa, diag, &mhalf2, pn, alpha, A, plda, B + mhalf1, pldb);
		 ...
		 */
		spawn taucs_cilk_trsm(side, uplo, transa, diag, pm, &nhalf1, alpha, A, plda, B, pldb);

		sync;
		spawn taucs_cilk_gemm("No Transpose", "Conjugate",
				      pm, &nhalf2, &nhalf1,
				      &taucs_minusone_const, B, pldb, A + nhalf1, plda, &taucs_one_const, B + nhalf1 * ldb, pldb);
		sync;
		spawn taucs_cilk_trsm(side, uplo, transa, diag,
				      pm, &nhalf2, alpha, A + nhalf1 + nhalf1 * lda, plda, B + nhalf1 * ldb, pldb);
		sync;
		return;
	}
}
...
cilk static void taucs_cilk_potrf(char *uplo, int *pn, taucs_datatype * A, int *plda, int *pinfo)
{
	...
	spawn taucs_cilk_trsm("Right",
			      "Lower",
			      "Conjugate", "No unit diagonal", &nhalf2, &nhalf1, &taucs_one_const, A, plda, A + nhalf1, plda);
	sync;
	/*
	...
	spawn taucs_cilk_herk("Lower",
			      ...
			      A + nhalf1, plda, &taucs_one_real_const, A + nhalf1 + (nhalf1 * *plda), plda);
	sync;
				     ...
				     supernodal_frontal_matrix * mtr, int *bitmap, supernodal_factor_matrix * snL)
{
	...
	if (mtr->sn_size) {
		/*
		 * taucs_potrf ("LOWER", &(mtr->sn_size), mtr->SFM_F1,&(mtr->sn_size), &INFO); 
		 */
		spawn taucs_cilk_potrf("LOWER", &(mtr->sn_size), mtr->SFM_F1, &(mtr->sn_size), &INFO);
	...
	if (mtr->up_size && mtr->sn_size) {
		spawn taucs_cilk_herk("Lower",
				      ...
				      &(mtr->up_size), &(mtr->sn_size),
				      &taucs_minusone_real_const,
				      mtr->SFM_F2, &(mtr->up_size), &taucs_one_real_const, mtr->SFM_U, &(mtr->up_size));
		sync;
	}
	...
	for (j = 0; j < my_mtr->up_size; j++) {
		for (i = j; i < my_mtr->up_size; i++) {
			...
			if (parent_j < parent_mtr->sn_size) {
				...
				(parent_mtr->SFM_U)[(parent_mtr->up_size) * (parent_j - parent_mtr->sn_size) +
						    (parent_i - parent_mtr->sn_size)] =
				    taucs_add((parent_mtr->SFM_U)[(parent_mtr->up_size) * (parent_j - parent_mtr->sn_size) +
								  (parent_i - parent_mtr->sn_size)], v);
			}
		}
	}
...
static int leftlooking_supernodal_front_factor(int sn, int *bitmap, taucs_ccs_matrix * A, supernodal_factor_matrix * L)
{
	 ...
	 */
	if (sn_size) {
		taucs_potrf("LOWER", &sn_size, (L->sn_blocks)[sn], &((L->sn_blocks_ld)[sn]), &INFO);
	}
	...
	if (up_size && sn_size)
		taucs_trsm("Right",
			   ...
			   &up_size, &sn_size,
			   &taucs_one_const,
			   (L->sn_blocks)[sn], &((L->sn_blocks_ld)[sn]), (L->up_blocks)[sn], &((L->up_blocks_ld)[sn]));
	...
	if (!is_root) {
		...
		if (flops > BLAS_FLOPS_CUTOFF) {
			...
			taucs_trsm("Left",
				   ...
				   "No Conjugate",
				   "No unit diagonal",
				   &sn_size, &ione, &taucs_one_const, sn_blocks[sn], &(sn_blocks_ld[sn]), xdense, &sn_size);
			...
			if (up_size > 0 && sn_size > 0) {
				taucs_gemm("No Conjugate", "No Conjugate",
					   ...
					   xdense, &sn_size, &taucs_zero_const, bdense, &up_size);
			}
		...
		if (flops > BLAS_FLOPS_CUTOFF) {
			...
			if (up_size > 0 && sn_size > 0)
				taucs_gemm("Conjugate", "No Conjugate",
					   ...
					   up_blocks[sn], &(up_blocks_ld[sn]),
					   xdense, &up_size, &taucs_one_const, bdense, &sn_size);

			taucs_trsm("Left",
				   "Lower",
				   "Conjugate",
				   "No unit diagonal",
				   &sn_size, &ione, &taucs_one_const, sn_blocks[sn], &(sn_blocks_ld[sn]), bdense, &sn_size);
