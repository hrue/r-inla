...
static void spa_scale_add(spa * s, int j, taucs_ccs_matrix * A, int k, taucs_datatype alpha)
{
	...
	for (ip = (A->colptr)[k]; ip < (A->colptr)[k + 1]; ip++) {
		...
		(s->values)[i] = taucs_add((s->values)[i], taucs_mul(alpha, Aik));
...
taucs_ccs_matrix *taucs_dtl(ccs_factor_llt) (taucs_ccs_matrix * A, double droptol, int modified) {
	...
	for (j = 0; j < n; j++) {
		...
		for (l = rowlist_getfirst(j); l != -1; l = rowlist_getnext(l)) {
			...
			/*
			 * spa_scale_add(s,j,L,k,taucs_neg(Lkj));
			 *//*
			 * L_*j -= L_kj * L_*k 
			 */
		...
		taucs_printf("pivot=%.4e+%.4ei, sqrt=%.4e+%.4ei\n",
			     taucs_re((s->values)[j]), taucs_im((s->values)[j]), taucs_re(pivot), taucs_im(pivot));
#endif
...
taucs_ccs_matrix *taucs_dtl(ccs_factor_ldlt) (taucs_ccs_matrix * A) {
	...
	for (j = 0; j < n; j++) {
		...
		for (l = rowlist_getfirst(j); l != -1; l = rowlist_getnext(l)) {
			 ...
			 */
			/*
			 * spa_scale_add(s,j,L,k,taucs_mul(taucs_neg(Lkj,Dkk));
			 *//*
			 * L_*j -= L_kj * L_*k 
			 */
			spa_scale_add(s, j, L, k, taucs_mul(taucs_neg(taucs_conj(Lkj)), Dkk));	/* L_*j -= L_kj * L_*k */
		}
		...
		for (ip = 0; ip < s->length; ip++) {
			...
			if (i == j) {
				/*
				 * printf(">>> %.8e + %.8ei / %.8e + %.8ei = ", taucs_re(v),taucs_im(v),
