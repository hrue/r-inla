...
static
taucs_ccs_matrix *ccs_syml_to_symu(taucs_ccs_matrix * A)
{
	taucs_ccs_matrix *U;
	int n;
	int *temp;
	...
	/*
	 * taucs_ccs_write_ijv(A,"AA.ijv"); taucs_ccs_write_ijv(U,"UU.ijv"); 
	 */
...
taucs_ccs_matrix *taucs_ccs_factor_xxt(taucs_ccs_matrix * A)
{
	...
	for (j = 0; j < n; j++) {
		...
		for (ip = 0; ip < Aej->length; ip++) {
			...
			for (l = rowlist_getfirst(i); l != -1; l = rowlist_getnext(l)) {
...
int taucs_ccs_solve_xxt(void *vX, double *x, double *b)
{
