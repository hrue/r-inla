...
## 5. The model checks: a Gaussian model with a known answer, rgeneric, and
##    the built-in cgeneric models. Shared with the Windows lane, which runs
##    the same file; see ci/smoke-models.R. When TESTMODEL_DIR is set, the
##    model's input files are exported there (keep=TRUE) so other jobs can
##    run the bare binary on a real model without needing R.
Rscript --vanilla "$ROOT/ci/smoke-models.R" \
    "$INLA_BIN" "${TESTMODEL_DIR:-}" "${INLA_SMTP:-}"
