...
vignette: >
  %\VignetteIndexEntry{Short tutorials from old www-page}
...
has 1 neighbour 1, and the graph looks like this (requires `Rgraphviz` and `graph`)
```{r,plot=TRUE,fig.width=2,fig.align='center'}
if (requireNamespace("Rgraphviz") && requireNamespace("graph")) {
...
```
The marginals are available as \verb|r$marginals.lincomb.derived$...|
...
\verb|inla.make.lincombs()|, where IDX is a matrix. Again, each row
...
found by doing (or give a specific section, see
