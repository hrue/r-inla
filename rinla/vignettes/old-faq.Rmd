...
### mod1.a.stack mimics the code above

mod1.a.stack <- inla.stack(
	data = list(Y = as.matrix(cbind(NA, y1, NA))), 
	A = list(1, 1),
	effects = list(intercept1 = rep(1,n),
			x1 = x1)
)
		
### while mod1.b.stack utilizes the predictor u in the model specification

mod1.b.stack <- inla.stack(
	data = list(Y = as.matrix(cbind(NA, y1, NA))), 
	A = list(1),
	effects = list(u = 1:n)
)
...
copy.stack <- inla.stack(
	data = list(Y = as.matrix(cbind(rep(0,n), NA, NA))),
	A = list(1, 1, - 1),
	effects = list(intercept1 = rep(1,n),
			x1 = x1,
			u = 1:n)
...
all.a.stack <- inla.stack(mod1.a.stack, copy.stack, mod2.stack)
all.b.stack <- inla.stack(mod1.b.stack, copy.stack, mod2.stack)
...
### There are some surprises here. All specifications give similar and acceptable results for fixed effects, 
### but different results with regard to precisions, and none has an approximately correct result, c(10000, 10000). 
### I guess this is due to inner working of the INLA-machine.


### The version r.new.b is faster than r.new.a. This time-saving is LARGE for more complex models with several spde-terms
```
