...
You can skip the next section if already know how to work with 
...
of the terms does not affects anything 
```{r, orderi}
...
dat$b <- factor(dat$b, levels=1:0, labels=c('2nd', '1st'))
```
...
We can now show that when the intercept is not in the formula 
it will takes the first level of the first factor covariate 
in the formula and the oder now matters
```{r orderf}
	coef(lm(y ~ 0 + a + b + d + x, dat))
...
The above scenario was for the fixed effects. 
