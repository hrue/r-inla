...
for(i in 1:k) {
  set.seed(seed_2[[i]])
  locs_mll[[i]] <- sf::st_sample(world_poly_planar, n, type = "random", exact = TRUE)
}
...
```{r, results='hide'}
depth_values_file <- "depth_values.rds"
if(file.exists(depth_values_file)) {
  depth_values <- readRDS(depth_values_file)
} else {
  # Download of depth/bathymetry 
  depth_values <- list()
  for(i in 1:k) {
      depth_values[[i]] <- ggOceanMaps::get_depth(as.data.frame(locs[[i]]))
  }
  saveRDS(
    object = depth_values, 
    file = depth_values_file
  )
}
```
