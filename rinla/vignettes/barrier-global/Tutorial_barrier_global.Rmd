...
depth_pred_file <- "depth_pred_file.rds"
if(file.exists(depth_pred_file)) {
    locs_latlong_pred <- readRDS(depth_pred_file)
} else {    
    locs_latlong_pred <- ggOceanMaps::get_depth(data = as.data.frame(
                                                    st_coordinates(grid_ll[[1]])))
    saveRDS(
        object = locs_latlong_pred,
        file = depth_pred_file
    )
}
