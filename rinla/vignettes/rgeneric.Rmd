...
`n_chars`, `n_mats` and `n_smats`.  The predefined named arguments in
...
```	
where \texttt{A} and \texttt{B} are what is defined in the
\texttt{inla}-argument \texttt{num.threads}. Note that the
\texttt{cgeneric} function needs to be thread-safe, and that number of
threads used within this function should be \texttt{data->threads.inner}.

You may store cached variables in \texttt{data}-struct
```{r eval=FALSE}
    void *cache;
```	
The macros
...
```
can be used to define threads spesific cache, where
\texttt{CGENERIC\_CACHE\_LEN} gives the require length, and
\texttt{CGENERIC\_CACHE\_ASSIGN\_IDX} will give the index (or assign an
index variable to the index depending on the current and parent
thread.
...
Dense matrices are stored row wise (as natural in C), which is NOT how
it is stored in `R`.
```{r eval=FALSE}
/*
...
Here is an example of reimplementing `generic0` using `cgeneric`. The
C-code is in `cgeneric-demo.c`, and the following R-code compare the
builtin and cgeneric implementation
```{r eval=FALSE}
library(mvtnorm)
n <- 10
