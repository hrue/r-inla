    ...
    if (nhyper == 0L) {
        if (!is.null(hyper) && !identical(hyper, list())) {
            stop(inla.paste(c(
                ...
                inla.paste(list(hyper))
            ), sep = " "))
        }
        if (!is.null(initial) && length(initial) > 0L) {
            stop(inla.paste(c(
                "Model", model, "[", section, "], has none hyperparameters, but 'initial' is ",
                initial
            ), sep = " "))
        }
        if (!is.null(fixed) && length(fixed) > 0L) {
            stop(inla.paste(c(
                "Model", model, "[", section, "], has none hyperparameters, but 'fixed' is",
                fixed
            ), sep = " "))
        }
        if (!is.null(param) && length(param) > 0L) {
            stop(inla.paste(c(
                "Model", model, "[", section, "], has none hyperparameters, but 'param' is",
                param
            ), sep = " "))
        }
        if (!is.null(prior) && length(prior) > 0L) {
            stop(inla.paste(c(
                "Model", model, "[", section, "], has none hyperparameters, but 'prior' is",
                param
            ), sep = " "))
        }
        return(hyper.new)
        ...
        for (nm in names(hyper)) {
            ...
            if (!any(inla.strcasecmp(nm, valid.keywords))) {
                stop(paste("Unknown keyword in `hyper' `", nm,
                    "'. Must be one of ", inla.paste(valid.keywords), ".",
                    sep = " "
                ))
            }
        }
    ...
    for (ih in 1L:nhyper) {
        ...
        for (key in keywords) {
            ...
            if (debug) {
                ...
                cat(inla.paste("*** key =", inla.ifelse(
                    is.null(key.val) || (is.numeric(key.val) && length(key.val) == 0),
            ...
            if (!is.null(read.only) && read.only == TRUE) {
                if (!is.null(h[[key]]) && h[[key]] != hyper.new[[idx.new]][[key]]) {
                      stop(paste("Setting hyperparameter `", name,
                          ...
                          "' is 'read-only', cannot change to `", h[[key]], "'.",
                          sep = ""
                      ))
                  }
                ii <- idx.new + off[[key]]
                if (!is.null(key.val) && !is.na(key.val[ii])) {
                    if (key.val[ii] != hyper.new[[idx.new]][[key]]) {
                        stop(paste("Setting hyperparameter `", name,
                            "'. Key `", key, "' = '", hyper.new[[idx.new]][[key]],
                            "' is 'read-only', cannot change to `", key.val[ii], "'.",
                            sep = ""
                        ))
                    }
                }
            ...
            if (is.na(test.val) || test.val) {
                ...
                if (len != 0L) {
                    ...
                    if (length(idxx) > 0L) {
                        if (debug) {
                            cat(inla.paste(c(
                                "*** Replace hyper.new|", idx.new, "|", key, "|", idxx, " with h|", key, "|", idxx,
                                "=", h[[key]][idxx]
                            )), "\n")
                        }
        ...
        if (key != "param" || (key == "param" && !skip.final.check)) {
            if (!is.null(key.val) && (length(key.val) > off[[key]])) {
                stop(paste("Length of argument `", key, "' is ", length(key.val),
                    ", does not match the total length of `", key, "' in `hyper' which is ", off[[key]],
                    sep = ""
                ))
            }
        }
