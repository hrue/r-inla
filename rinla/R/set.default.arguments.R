...
`inla.set.control.compute.default` <-
    function(...) {
        ...
        list(
            ...
            control.gcpo = list(enable = FALSE, group.size = -1, strategy = c("posterior", "prior"), groups = NULL, selection = NULL, verbose = FALSE, epsilon = 0.001, correct.hyperpar = TRUE, keep = NULL, remove = NULL, remove.fixed = TRUE)
        )
...
`inla.set.control.inla.default` <-
    function(...) {
        ...
        ans <- list(
            ...
            ## :ARGUMENT: int.strategy  Character The integration strategy to use; one of 'auto' (default),  'ccd', 'grid', 'eb' (empirical bayes),  'user' or 'user.std'. For the experimental mode,  then 'grid' equal 'ccd' for more than two hyperparameters.
            int.strategy = "auto",
