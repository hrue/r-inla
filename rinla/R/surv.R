...
`inla.surv.1` = function(time, event, time2, truncation)
{
    ...
    if (sum(time[!is.na(time)]<0)>0) stop("Negative times are not allowed")
    if (!missing(time2))
    {
        if (sum(time2[!is.na(time2)] < 0) != 0)
    ...
    else if (sum(is.na(event))>0)
    {
        ...
        ##observed failures (give a warning)
        event[is.na(event)] = 1
        warning("Some elements in `event' are missing: assuming all are observed failures")
    }
      
    ##check that event is 0,1,2,3
    if (sum(event==0 | event==1 | event==2 | event==3)!=nn)
...
`plot.inla.surv.2` = function(x, legend=TRUE,...)
{
    ...
    for(i in 1 :nn)
    {
        ...
        points(time[subject==i & event==1], rep(i, length(time[subject==i & event==1])),         
               pch=8,col="red")
        points(time[subject==i & event==0], rep(i, length(time[subject==i & event==0])), pch=10, 
               col="green")
