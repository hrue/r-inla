...
##!   \item{hyper.user.scale}{Logical. If \code{TRUE} then values of the hyperparameters are given in the user scale (for example \code{precision}). If \code{FALSE} then values of the hyperparameters are given in the internal representation (for example \code{log(precision)}).}
##!   \item{use.improved.mean}{Logical. If \code{TRUE} then use the marginal mean values when constructing samples. If \code{FALSE} then use the mean in the Gaussian approximations.}
##!}
##!\value{
