...
`inla.upgrade` = function(lib = NULL, testing = FALSE, ask = TRUE)
{
    repo=c(CRAN = "https://cran.rstudio.com",
        INLA = paste("https://www.r-inla-download.org/inla/R/",
    ...
    if (require("INLA", quietly = TRUE, lib.loc = lib,
        ...
        if (new.pack) {
            if (.Platform$OS.type == "windows") {
                ...
                if (testing) {
                    cat("\n     install.packages(\"INLA\", repos=\"https://www.r-inla-download.org/inla/R/testing\")")
                } else {
                    cat("\n     install.packages(\"INLA\", repos=\"https://www.r-inla-download.org/inla/R/stable\")")
                }
                cat("\n *** and then restart R.", "\n")
            } else {
