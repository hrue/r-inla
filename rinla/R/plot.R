    ...
    function(x,
            ...
            if (!postscript && !pdf) {
                ...
                while (!found) {
                    filename <- paste(prefix,
                        ...
                        figure.count,
                        inla.ifelse(postscript, ".eps", ".pdf"),
                        sep = ""
                    )
            ...
            if (nf > 0) {
                ...
                for (i in 1:nf) {
                    ...
                    if (!all(is.na(fix[[i]]))) {
                        ...
                        my.plot(m,
                            type = "l", main = paste("PostDens [", inla.nameunfix(labels.fix[i]), "]", sep = ""),
                            sub = sub, xlab = "", ylab = ""
                        )

                        if (plot.prior) {
                            xy <- (inla.get.prior.xy(
                                section = "fixed", hyperid = labels.fix[i],
                                        ...
                                        if (tp == "s") {
                                            ...
                                            if (!is.null(x$.args$data$baseline.hazard.strata.coding)) {
                                                rep.txt <- inla.paste(c(
                                                    rep.txt, "[",
                                                    x$.args$data$baseline.hazard.strata.coding[r.rep], "]"
                                                ), sep = "")
                                            }
                                        } else {
                                            xval <- rr[, colnames(rr) == "ID"][idx]
                ...
                for (i in 1:nhyper) {
                    ...
                    if (!is.null(hh)) {
                        ...
                        if (plot.prior) {
                            ...
                            if (length(id) > 0) {
                                xy <- (inla.get.prior.xy(
                                    section = tolower(id[2]), hyperid = id[1],
        ...
        if (plot.cpo && !is.null(x$cpo)) {
            if (!(is.null(x$cpo$pit) || length(x$cpo$pit) == 0L) || !(is.null(x$cpo$cpo) || length(x$cpo$cpo) == 0L)) {
                ...
                ## do one plot for each likelihood
                fam.names <- x$.args$family
                fam <- sort(unique(x$misc$family))
                len <- length(x$cpo$cpo)
                ...
                for (ifam in fam) {
                    ...
                    fam.name <- fam.names[ifam]
                    idx <- which(x$misc$family == ifam)
                    ok <- (!is.na(x$cpo$failure[idx]) & !is.na(x$cpo$pit[idx]) & !is.na(x$cpo$cpo[idx]))
                    failure <- (x$cpo$failure[idx])[ok]
                    pit <- (x$cpo$pit[idx])[ok]
                    cpo <- (x$cpo$cpo[idx])[ok]
                    fail <- (x$cpo$fail[idx])[ok]
                    ii <- 1:len
                    ii <- (ii[idx])[ok]
                    ...
                    if (!(is.null(pit) || length(pit) == 0L)) {
                        ...
                        if (inla.model.properties(fam.name, section = "likelihood")$discrete) {
                            discrete <- TRUE
                            m <- "Modified PIT-values"
                            p <- pit - 0.5 * cpo
                        } else {
                            discrete <- FALSE
                            m <- "PIT-values"
                        ...
                        hist(p,
                            main = main.txt, xlab = "Probability",
                    ...
                    if (!(is.null(cpo) || length(cpo) == 0L)) {
                        n.fail <- sum(failure != 0.0)
                        main.txt <- paste("CPO-values", ", nfail", n.fail, ", ", fam.name, "[", ifam, "]", sep = "")
                        ccpo <- rep(NA, len)
                        ccpo[ii] <- cpo
                        if (discrete) {
                              ...
                              len = 10000, range, intern = FALSE) 
{
    ...
    my.pcfgnh <- function(theta, param, log = FALSE) {
        ...
        d.spline <- function(H.intern, ...) {
            return(exp(log.d.spline(H.intern, ...)))
        }
        d.spline.deriv <- function(H.intern, ...) {
            return(d.spline(H.intern, ...) * log.d.spline(H.intern, deriv = 1L))
        }
