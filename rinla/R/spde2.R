...
inla.spde2.generic <- function(M0, M1, M2, B0, B1, B2, theta.mu, theta.Q,
...
inla.internal.spde2.matern.B.tau <- function(precision.fcn,
                                             n.theta, delta = 1e-3, ...) {
...
#'
#'
#' @param mesh An [fmesher::fm_mesh_1d()],  [fmesher::fm_mesh_2d()], or
...
inla.spde2.matern.sd.basis <- function(mesh,
                                       B.sd,
                                       B.range,
                                       method = 1,
...
inla.internal.test.spde2.sd.basis <- function(k = 1,
                                              dth = 0.1,
                                              r = 1000,
                                              globe = 25,
                                              compensate = FALSE) {
    ...
    sd.basis0 <- inla.spde2.matern.sd.basis(
        mesh,
        B.sd = B.sd,
        B.range = B.range,
        method = 0,
        alpha = alpha,
        local.offset.compensation = compensate
    )
    sd.basis1 <- inla.spde2.matern.sd.basis(
        mesh,
        B.sd = B.sd,
        B.range = B.range,
        method = 1,
    ...
    op <- par(mfrow = c(2, 2))
    on.exit(par(op))
    plot(asin(mesh$loc[, 3]), rowSums(sd.basis0$B.kappa[, 1 + k, drop = FALSE]),
        pch = 20,
        ylim = (range(c(
            0, range(rowSums(sd.basis0$B.kappa[, 1 + k, drop = FALSE])),
    ...
    plot(asin(mesh$loc[, 3]), v0.1 - v0.0,
        col = 2, pch = 20,
    ...
    plot(asin(mesh$loc[, 3]), v0.1 / v0.0 - 1,
        col = 2, pch = 20,
...
#'
#'
#' @param mesh The mesh to build the model on, as an [fmesher::fm_mesh_2d()]
#' object, or other `fmesher` mesh supporting [fmesher::fm_dof()],
#' [fmesher::fm_manifold_dim()] and [fmesher::fm_dof()].
...
#'
#' @param mesh The mesh to build the model on, as an [fmesher::fm_mesh_1d()],
...
#' Default `FALSE`. For [fmesher::fm_collect] meshes, the constraint is
#' applied separately for each sub-mesh.
...
inla.spde2.matern <- function(mesh,
    ...
    if (n.iid.group == 1) {
        ...
        spde <-
            inla.spde2.generic(
                M0 = kronecker(Diagonal(n.iid.group), M0),
            ...
            if (inherits(mesh, "fm_collect")) {
                A.constr_ <-
                    as.matrix(Matrix::sparseMatrix(
                        i = rep(seq_along(mesh$fun_spaces),
                                vapply(
                                    mesh$fun_spaces,
                                    function(x) fmesher::fm_dof(x),1L)
                        ),
                        j = seq_len(fmesher::fm_dof(mesh)),
                        x = 1,
                        dims = c(length(mesh$fun_spaces), fmesher::fm_dof(mesh))
                    ))
                A.constr <- rbind(
                    A.constr,
                    kronecker(
                        matrix(1 / n.iid.group, 1, n.iid.group),
                        as.matrix(A.constr_ %*% fem$c1)
                    )
                )
                e.constr <- rbind(
                    e.constr,
...
#' @param mesh The mesh to build the model on, as an [fmesher::fm_mesh_2d()]
#'   object, or other `fmesher` mesh supporting [fmesher::fm_dof()],
#'   [fmesher::fm_manifold_dim()] and [fmesher::fm_dof()], such as
#'   [fmesher::fm_mesh_1d()], [fmesher::fm_mesh_3d()], or
#'   [fmesher::fm_collect()].
...
inla.spde2.iheat <- function(mesh.space,
                             mesh.time,
                             order = 2,
                             param = NULL,
                             constr = FALSE,
    ...
    #     mesh.space, c("fm_mesh_2d", "fm_mesh_1d", "fm_mesh_3d"),
    #     "'mesh.space'"
    # )

    # Still requires fm_mesh_1d for time:
    inla.require.inherits(
    ...
    ## TODO: the rest
    warning("inla.spde2.iheat not fully implemented")
    if (alpha == 2) {
        B.phi0 <- param$B.tau
        B.phi1 <- 2 * param$B.kappa
        M0 <- fem$cc
...
inla.spde2.theta2phi2 <- function(spde, theta) {
    ...
    if (spde$param.inla$transform == "identity") {
        ...
        warning(paste("Unknown link function '",
            ...
            "' phi2.  Using identity link instead.",
            sep = ""
        ))
        return(phi)
    }
...
inla.spde2.precision <- function(spde, theta = NULL,
