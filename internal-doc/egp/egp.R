...
c012 <- function(eta, y, xi, kappa, alpha) {
...
F <- function(y, eta, xi, kappa,  alpha = 0.9) {
    q <- exp(eta)
    sigma <- xi * q / ((1-alpha^(1/kappa))^(-xi) -1)
    yy <- 1 + xi * y / sigma
    return ((1- yy^(-1/xi))^kappa)
}

lf <- function(y, eta, xi, kappa, alpha = 0.9) {
    return (log(f(y, eta, xi, kappa, alpha)))
}

f <- function(y, eta, xi, kappa, alpha = 0.9) {
    h <- 1e-5
    return (((F(y+h, eta, xi, kappa, alpha) - F(y-h, eta, xi, kappa, alpha)) / (2*h)))
}

llf <- function(y, eta, xi, kappa, alpha = 0.9, no.check = FALSE) {
    ...
    if (!no.check && xi < 0.0) {
        ...
        if (length(idx) >0) {
            ee <- eta[idx]
            eeta <- eta.L + (eta.H - eta.L) * (1-exp(-sqrt(eta.L - ee)))
            cc012 <- c012(eeta, y, xi, kappa, alpha)
            values[idx] <- cc012$c0 + cc012$c1 * (ee - eeta) + 0.5 * cc012$c2 * (ee - eeta)^2
        }
        return (values)
    }
    yy <- 1 + xi * y / sigma
    ...
    lyy <- log(yy)
    lsigma <- log(sigma)
    val <- log(kappa) + (kappa-1) * log1p(-yy^xii) - lsigma + (xii-1) * lyy
    val[idx] <- -Inf
    return (val)
}
...
##lf(y, eta, xi, kappa) - llf(y, eta, xi, kappa)

yy <- llf(y, eta, xi, kappa, no.check = TRUE)
idx <- which(is.finite(yy))
fun.orig <- splinefun(eta[idx], yy[idx])
fun <- splinefun(eta, llf(y, eta, xi, kappa))
par(mfrow = c(1, 2))
plot(eta,  fun.orig(eta), ylim = c(-300, 0))
lines(eta,  fun(eta))
plot(eta,  fun.orig(eta, deriv = 2), ylim = c(-2000, 0))
lines(eta,  fun(eta, deriv = 2))
