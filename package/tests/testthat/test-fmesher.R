test_that("Spherical harmonics", {
    ...
    sph0 <- fmesher::fm_raw_basis(m, type = "sph.harm", rot.inv = TRUE)
    
    expect_equal(dim(sph0), c(12, 4))
    expect_equal(sph0[1, ], c(1, 1.732050808, 2.2360679775, 2.645751311))
    
    sph <- fmesher::fm_raw_basis(m, type = "sph.harm", rot.inv = FALSE)
...
test_that("Barrier domain construction", {
    ...
    m <- fmesher::fm_rcdt_2d_inla(cbind(0,0), extend = list(offset = 1))
